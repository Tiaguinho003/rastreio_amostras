...
- `src/sale-contracts/` — dominio do contrato de venda "Mercado a vista" (Fase B). `sale-contract-support.js` (PURO: normalizadores dos termos da venda, calculo financeiro, numero `NNNN/AA`, snapshots, view) + `sale-contract-service.js` (gestao listar/detalhar; acesso **ADMIN+CADASTRO**). O contrato NASCE junto da venda a vista (`createSampleMovement`), nao por um create proprio — ver skill `prisma` (`appendEventBatch` + `beforeCommit`)
