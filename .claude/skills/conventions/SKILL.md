...
- `src/sale-contracts/` — dominio do contrato de venda "Mercado a vista" (Fase B). `sale-contract-support.js` (PURO: normalizadores dos termos da venda + da etapa 2, calculo financeiro com agio/desagio, numero `NNNN/AA`, snapshots das partes/banco/armazens, view) + `sale-contract-service.js` (gestao **ADMIN+CADASTRO**: listar/detalhar + **`emitSaleContract`** [salva etapa 2 + `EM_ABERTO`/`CONFERIR`→`CONFERIR`, snapshots, recalcula total, grava `SaleContractExport`, D48] + **`confirmSaleContract`** [`CONFERIR`→`CONFIRMADO`] + `listContractLookups` [as 3 listas, autenticado]). Emitir/Confirmar sao **CRUD direto** (`SaleContract` nao tem trigger de UPDATE) com concorrencia otimista por `version`. O contrato NASCE junto da venda a vista (`createSampleMovement`), nao por um create proprio — ver skill `prisma` (`appendEventBatch` + `beforeCommit`)
