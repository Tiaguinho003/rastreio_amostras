...
- **Arquitetura:** `app/dashboard/page.tsx` renderiza `DashboardMobile` + `DashboardDesktop` JUNTOS (nao-prospector); o CSS alterna por `@media (min-width:901px)` (`.dashboard-desktop`/`.dashboard-mobile` viram `display:contents`/`none`). O mobile vive so das props (`data`/`salesData` do `useDashboardData`); o desktop tem UM fetch proprio (`getDashboardRecentSends`, que alimenta os DOIS cards de envio "Amostras enviadas"/"Aprovacoes enviadas" — guard `matchMedia` + listener `change` + throttle 30s, so o breakpoint ativo busca). CSS do desktop e **desktop-first na base** + mobile em `@media (max-width:900px)`.
- **Chrome (DSB-D6, 2026-07-12):** a navegacao desktop dos 5 papeis nao-PROSPECTOR migrou da sidebar esquerda para uma **top bar horizontal branca** (`.topbar` + `.topbar-nav`): **logo colorido** (`/logo-safras-color.png`) a esquerda, **itens so com NOME (sem icones)** centralizados (`grid-template-columns: 1fr auto 1fr`), **avatar de perfil** a direita. Item ativo = **sublinhado verde** (`.topbar-nav-link.is-active`, `border-bottom`). O shell virou **coluna unica**; `:has(.app-sidebar)` reativa o grid de 2 colunas SO pro **PROSPECTOR**, que mantem a **sidebar vertical verde** de antes (`.app-sidebar`: logo branco + Inicio + "Sair"). Layout grid + sticky, NAO `position:fixed` (o PageTransition aplica transform e quebraria fixed). **"Sair" saiu da navegacao** — logout so pelo dropdown do avatar (`.topbar-profile-menu` "Sair", presente em todas as paginas desktop). Mobile inalterado (topbar verde + MobileTabbar + hero).
- **Saudacao no desktop: REMOVIDA (DSB-D6).** O bloco `.topbar-greeting` ("Bom dia," + nome grande + tipo de usuario) saiu da faixa branca — a faixa agora hospeda a navegacao. O **hero mobile** (`DashboardMobile`) mantem a saudacao. **Busca de lote** segue removida da faixa branca no desktop (`.topbar-search-slot{display:none}` >=901px; mobile mantem).
...
- **Grid principal — `.dd-content-grid`** (DSB-D3, 2026-07-12: **DUAS LINHAS**, `flex: 1`; o shell-lock de 100vh continua, sem scroll de pagina): **TOP ROW `.dd-top-row`** (DSB-D5: **3 colunas** `minmax(0,0.8fr)/1.1fr/1.1fr` — donut mais estreito) = "Lotes disponiveis" (`SalesAvailabilityCard` `compact`) + **"Amostras enviadas"** + **"Aprovacoes enviadas"** (dois `RecentSendsCard`) **lado a lado**; **EMBAIXO** = card **"Eventos"** (`EventsCalendarCard`) ocupando a **largura toda** (horizontal), mais alto que a top row (`grid-template-rows: 0.85fr 1.15fr`). _(Antes de DSB-D3 eram 2 colunas com `.dd-left-col`/`.dd-left-stack`; a StatCards `.dd-summary-row` saiu em DSB-D2; a top row era `1fr/1fr` com um unico "Ultimos envios" ate DSB-D5.)_ Animacoes de entrada nos FILHOS (top-row 0.2s, eventos 0.3s). Sombra de todos os cards via var **`--dd-card-shadow`** em `.dashboard-page` (tunavel num lugar so).
- **Cards de envio** (`components/dashboard/RecentSendsCard.tsx`, desktop-only): UM componente parametrizado (`title`/`emptyLabel`/`items`/`variant`) usado em **dois cards** na top row. **DSB-D8 (2026-07-12): viraram TABELA horizontal compacta** — cabecalho de colunas **sticky** (`.dd-send-head`) + **1 linha por envio** (`.dd-send-row`), com classes proprias `.dd-send-*` (**desacoplado do `.spv2-card`** — a variante `is-static` era exclusiva daqui e saiu). Colunas por variante (`.dd-sends-list.is-samples`/`.is-approvals` definem o `grid-template-columns`): `samples` = **Lote · Destinatario · Tipo · Tempo**; `approvals` = **Contrato · Comprador · Tempo** (sem Tipo — o titulo ja diz). `Tipo` = texto **cinza neutro** (`.dd-send-type`), uma palavra: `PHYSICAL_SAMPLE`→"Fisico", `REPORT`→"Descricao" (⚠️ "Descricao" diverge de "Laudo" do resto do app — decisao do usuario, so aqui). Shell branco radius 20 + header (`.dd-sends-title` + tile `.dd-sends-icon`) + `.dd-sends-list` com **scroll interno**. Celulas: `.dd-send-code` (lote/n contrato + `BlendBadge`) · `.dd-send-party` (destinatario/comprador — **ellipsis + `title`** no hover) · `.dd-send-time` (relativo `formatRelativeTime`/refresh 60s, alinhado a direita, `title` com data exata; SEM icone). Cards **INERTES** (sem clique). Cancelado = `.dd-send-row.is-cancelled` (esmaecido) + numero **riscado** (`.dd-send-code-text` line-through) — sem tag. Semantica de tabela (`role=table/row/columnheader/cell`). Dados: `GET /dashboard/recent-sends` devolve `{ sampleItems, approvalItems }` (top-40 cada — amostra = `PHYSICAL_SAMPLE_SENT`+`REPORT_EXPORTED`, aprovacao = `approval_label_log`).
- **Card "Eventos"** (`components/dashboard/EventsCalendarCard.tsx`, desktop-only; detalhe em `docs/Dashboard-Visao-Geral.md` §7.3): **DSB-D4/D7 (2026-07-12)** — de 2 semanas para **1 SEMANA de DIAS ÚTEIS (seg–sex, 5 células)**. A janela BUSCADA segue dom–sáb (7 dias); o backend rola os eventos de fim de semana pro dia útil vizinho (`rollWeekendToWeekday`, sáb→sex/dom→seg) e `buildBusinessDays` filtra os 5 renderizados (helpers date-only BRT em `lib/dashboard-calendar.ts`: `buildWeek`/`buildBusinessDays`/`computeWeekStart`/`CALENDAR_WEEK_DAYS` — NAO unificar com o weekly-report segunda-based). Header = titulo + rotulo do periodo ("7 – 11 de julho", seg–sex) + nav ◀ `Hoje` ▶ (passo **7 dias**, passado/futuro livres; deslize horizontal 200ms `dd-events-slide-*`, coberto no reduced-motion). Grade `.dd-events-grid` = **5 colunas × 1 linha** (dias úteis, DSB-D7) de celulas **altas** que preenchem a altura do card. Cada celula (`.dd-events-day`, agora um **container `<div>`**, nao mais botao selecionavel) = numero no topo (`.dd-events-day-number`; "1 ago" na virada de mes) + **`.dd-events-day-list`** (`flex:1; overflow-y:auto` — **rola POR DENTRO** quando o dia tem muitos eventos). **Sem painel** de dia selecionado e **sem dots**. Cada evento = **`.dd-events-chip`** (Link ou span): borda-esquerda colorida por **ESTADO** (var `--chip-color` via `data-state`, DSB-D10) + rotulo truncado; **navegacao PURA** → a sub-aba dona via `navTabForEvent` (`/contratos?tab=financeiro|embarque|contratos&highlight={contractId}`; DSB-D11: `eventHref` recebe `navigableTabs` do papel e deixa o chip INERTE quando a aba-dona não é visível — faturamento→Contratos p/ operacional); tipo desconhecido/inerte = chip so-rotulo (span). **Cores por estado `.dd-events-chip[data-state]`** (DSB-D10 — **3 cores só**; o **nome do tipo** no rotulo `tipo · nº · comprador` diferencia os eventos): azul `#2563eb` = **previsto** / vermelho `#dc2626` = **atrasado** / verde `#15803d` = **realizado**. Vars `--state-previsto/--state-atrasado/--state-realizado` no `.dd-events-card`; **legenda** `.dd-events-legend` (bolinhas por `data-state`). O estado tambem vai no `aria-label`/`title` do chip (a11y — nao depende so da cor). Hoje = anel `inset var(--brand-green)` + fundo `#f2f8f4` + `aria-current="date"`; **fins de semana OCULTOS** (DSB-D7 — só seg–sex; sáb/dom rolam pro dia útil vizinho no backend). **3 feeds mesclados no `DashboardDesktop`** via `onWindowChange` (janela de **7 dias**): pagamento (`/dashboard/payment-events`, ADMIN+COMMERCIAL/`canPay`) + embarque (`/dashboard/shipment-events`, **auth-only**) + faturamento (`/dashboard/invoice-events`, **auth-only**; DSB-D11 — chip → aba Contratos, com pisca via `useContractHighlight`). _(O feed de **aprovacao** — lembrete "a enviar", `contract_approval_due` — foi **REMOVIDO** em DSB-D9/2026-07-12: a data era imprecisa e fazia fan-out. A aba Aprovacoes e o card "Aprovacoes enviadas" seguem.)_ A prop `events` = `Record<'YYYY-MM-DD', DashboardCalendarEvent[]>`. Card **desktop-only** (o `DashboardMobile` nao o renderiza).
