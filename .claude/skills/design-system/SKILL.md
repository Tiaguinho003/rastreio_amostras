...
- **Informativo de mercado** (`InformativoFormSheet` + `InformativoForm`, opcao "Informativo" do leque): BottomSheet (`.is-informe.is-informativo`) em **2 etapas** — 21 campos → previa da peca + baixar. Gera um PNG 1080×1920 (story 9:16) desenhado em `<canvas>` no cliente; **nada e salvo** (sem rota de API, sem banco) e o `shareOrDownloadFile` entrega (share sheet no celular, download no PC). CSS proprio no prefixo **`.ifm-`**, herdando o achatamento de cards do `.is-informe`. As cores do toggle de direcao (`.ifm-dir.is-alta/.is-baixa`) sao hex da **peca**, nao tokens do app — a fonte-da-verdade e o `COLORS` de `lib/informativos/mercado-layout.ts` (mexeu num, mexeu nos dois). Especificacao visual completa em `docs/Informativos-Plano-de-Trabalho.md` §5.
