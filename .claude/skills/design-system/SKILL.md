...
- **Variante `.is-operations` (lista de pendencias do dashboard):** compartilha a regra de altura-por-conteudo da `.is-menu` (mesmo seletor agrupado). Usada pelo `OperationModal` ("Amostras pendentes", `components/dashboard/OperationModal.tsx`), que migrou do antigo modal central `.app-modal.is-themed.app-modal-dashboard` pro BottomSheet. A lista interna (`.app-modal-list`) tem o `max-height`/`overflow` proprios neutralizados (`.bottom-sheet.is-operations .app-modal-list`) pra o scroll ficar so no `.bottom-sheet-body` (sem duplo scroll). Cards reusam `.app-modal-card*`.
