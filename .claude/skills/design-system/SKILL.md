...
- **Arquitetura:** `app/dashboard/page.tsx` renderiza `DashboardMobile` + `DashboardDesktop` JUNTOS (nao-prospector); o CSS alterna por `@media (min-width:901px)` (`.dashboard-desktop`/`.dashboard-mobile` viram `display:contents`/`none`). O twin desktop busca seu dado extra (Vendas e perdas) so no proprio breakpoint (guard `matchMedia` + listener `change` que re-busca ao entrar no breakpoint num resize — senao o card travava no skeleton); o mobile hoje vive so das props (`data`/`salesData`). CSS do desktop e **desktop-first na base** + mobile em `@media (max-width:900px)`.
...
- **1a linha — 4 `StatCard`** (`components/dashboard/StatCard.tsx`; horizontais, icone verde em tile claro `rgba(31,93,67,.1)`): Classificacao pendente / Cadastros pendentes / Lotes registrados hoje / Envios concluidos hoje. Os 2 de pulso tem delta **"% vs ontem"** (`formatDelta`): `.dd-stat-delta` `is-up` verde / `is-down` vermelho / **`is-flat` cinza** (0% OU igual a ontem — nunca verde); a linha some quando ontem=0.
- **Linha 2 — `.dd-content-grid`** (cols 1fr/1fr, uma linha): "Lotes disponiveis" (`SalesAvailabilityCard` `compact`: donut + legenda + botao "Ver disponiveis" **full-width no rodape**) e "Vendas e perdas" (`CommercialTrendCard`: grafico de **linha SVG** na mao, vendas verde / perdas vermelho, em **sacas**, ultimos **7 dias uteis**) lado a lado 50/50 — layout provisorio pos-remocao do card "Ultimas atividades" (2026-07-06; a informacao substituta sera definida). Sombra de todos os cards/containers via var **`--dd-card-shadow`** em `.dashboard-page` (tunavel num lugar so).
