...
- **Card "Eventos"** (`components/dashboard/EventsCalendarCard.tsx`, desktop-only, F0 — decisoes E1-E19 em `docs/Eventos-Dashboard-Plano-de-Trabalho.md`): calendario de **2 semanas domingo-first** (helpers date-only BRT em `lib/dashboard-calendar.ts` — NAO unificar com o weekly-report segunda-based). Header = titulo + rotulo do periodo ("7 – 20 de julho") + nav ◀ `Hoje` ▶ (passo 14 dias, passado/futuro livres). Grade `.dd-events-grid` 7 colunas: quadrados-`<button>` com numero + dots por tipo (max 3 + `+N`; cor por tipo entra com o catalogo), fds APAGADOS (`.is-weekend` opacity .45), hoje = anel `inset var(--brand-green)` + `aria-current="date"`, selecionado = fundo `#e8f1ec` + `aria-pressed`, roving tabindex com setas, "1 ago" na virada de mes. Painel do dia protagonista (~60%, `.dd-events-panel` flex:1 scroll interno) com titulo "9 de julho — quinta-feira"; vazio da F0 = "Nenhum evento para este dia." + nota de futuro (E19, temporaria). Deslize horizontal de 200ms na navegacao (keyframes `dd-events-slide-*`, coberto no reduced-motion). SEM fetch na F0 — seam da F1 e a prop `events` (`Record<'YYYY-MM-DD', stub[]>`).
