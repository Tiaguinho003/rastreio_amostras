...
- **Card "Eventos"** (`components/dashboard/EventsCalendarCard.tsx`, desktop-only, F0+F1 — decisoes E1-E27 em `docs/Eventos-Dashboard-Plano-de-Trabalho.md`): calendario de **2 semanas domingo-first** (helpers date-only BRT em `lib/dashboard-calendar.ts` — NAO unificar com o weekly-report segunda-based). Header = titulo + rotulo do periodo ("7 – 20 de julho") + nav ◀ `Hoje` ▶ (passo 14 dias, passado/futuro livres). Grade `.dd-events-grid` 7 colunas: quadrados-`<button>` com numero + dots por tipo (max 3 + `+N`; **1ª entrada do catalogo viva na F1: `.dd-events-dot[data-type]` âmbar `#eab308` (`contract_payment_due`) / verde `#15803d` (`contract_payment_paid`)**), fds APAGADOS (`.is-weekend` opacity .45), hoje = anel `inset var(--brand-green)` + `aria-current="date"`, selecionado = fundo `#e8f1ec` + `aria-pressed`, roving tabindex com setas, "1 ago" na virada de mes. Painel do dia protagonista (~60%, `.dd-events-panel` flex:1 scroll interno) com titulo "9 de julho — quinta-feira"; vazio = "Nenhum evento para este dia." (a nota de futuro E19 SAIU na F1). **F1 (E21-E27/D138): cada item do painel virou ACORDEAO** (`.dd-events-panel-item`, estado `expandedId` — 1 aberto por vez): header `.dd-events-item-head` (label "nº · comprador" + chevron) → detalhe `.dd-events-item-detail` (`<dl>` Contrato/Comprador/Vendedor/Status) + acoes `.dd-events-item-actions`: **"Pago"** `.dd-events-item-btn-primary` (so `status==='FATURADO'` && `canManage`, reusa o `SaleContractLifecycleDialog`/D137 no pai via `onPagar`) + **"Ver contrato"** (`<Link href="/contratos?details={contractId}">`). Deslize horizontal de 200ms na navegacao (keyframes `dd-events-slide-*`, coberto no reduced-motion). **Fetch REAL na F1**: a prop `events` (`Record<'YYYY-MM-DD', DashboardCalendarEvent[]>`, tipo promovido pro `lib/types.ts`) e populada pelo `DashboardDesktop` via `GET /api/v1/dashboard/payment-events` da quinzena que o card emite por **`onWindowChange`**; so ADMIN+COMMERCIAL (E22/`canPay`) buscam.
