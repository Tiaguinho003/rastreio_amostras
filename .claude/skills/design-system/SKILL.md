...
- **Arquitetura:** `app/dashboard/page.tsx` renderiza `DashboardMobile` + `DashboardDesktop` JUNTOS (nao-prospector); o CSS alterna por `@media (min-width:901px)` (`.dashboard-desktop`/`.dashboard-mobile` viram `display:contents`/`none`). Cada twin so busca no SEU breakpoint (guard `matchMedia` + listener `change` que re-busca ao entrar no breakpoint num resize — senao os cards travavam no skeleton). CSS do desktop e **desktop-first na base** + mobile em `@media (max-width:900px)`.
...
- **Linhas 2-3 — `.dd-content-grid`** (cols 1.5fr/2.5fr): esquerda = "Lotes disponiveis" (`SalesAvailabilityCard` `compact`: donut + legenda + botao "Ver disponiveis" **full-width no rodape**) + "Vendas e perdas" (`CommercialTrendCard`: grafico de **linha SVG** na mao, vendas verde / perdas vermelho, em **sacas**, ultimos **7 dias uteis**); direita = "Ultimas atividades" (`RecentActivityList`, **8 linhas fixas**, nomes de coluna no **cabecalho FIXO** — nao vaza ao rolar; venda/perda cancelada esmaecida `.is-cancelled` + card maior `.is-cancellation`). Sombra de todos os cards/containers via var **`--dd-card-shadow`** em `.dashboard-page` (tunavel num lugar so).
...
- ⚠️ **CSS orfao legado:** ha um cluster de classes mortas do dashboard antigo em `globals.css` (`.dashboard-secondary-grid`, `.dashboard-section-column/-link/-subtitle`, `.dashboard-op-print/-progress*`, variantes `.dashboard-skeleton-line-md/-lg/-full/-xs`+`circle`/`content`, `.dd-activity-count`, `.sales-total-number`) **INTERLEAVADAS com classes VIVAS** (`.dashboard-section-heading/-title` e `.dashboard-operations-grid` sao usadas pelo PROSPECTOR; `.dashboard-skeleton-line-sm`/`-card`/base vivas). Remover SO classe-a-classe com grep antes — delecao por faixa de linha QUEBRA o prospector.
