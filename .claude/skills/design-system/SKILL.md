...
| `.sdv-card-themed`          | Card com header verde (gradient `--brand-green`) + body branco — Endereco fiscal / Filiais (detalhe do cliente). O container **Informacoes** usa o padrao branco `.sdv-info-compact` (abaixo).                                                                                                                             |
| `.sdv-info-compact`         | Container branco padrao (SEM header verde): `.sdv-card-header` (titulo cinza + hairline full-bleed) + editar minimalista `.sdv-edit-btn` (lapis + "Editar") + `.sdv-info-grid` (2 colunas). Containers Informacoes/Classificacao/Resumo do detalhe da amostra e Informacoes do detalhe do cliente.                         |
