...
- A rota `/informe` virou a pagina **"Relatorios"** que adapta o conteudo ao papel (`app/informe/page.tsx`); `/resumo` redireciona pra ela (server-side). Guard `INFORME_ROLES` = ADMIN/COMMERCIAL/REGISTRATION/**CADASTRO**. Branches:
  - **ADMIN/CADASTRO** (`isVisitReportViewer`) → `components/informe/RelatoriosViewer.tsx`: feed de TODOS (`scope=all`, 3 tipos) + curadoria de vinculo, shell `.sdv-page`/`.rsm-content` (ver §1 item 5). **ADMIN** tambem cria — recebe `canCreate` e renderiza o `InformeCreateFab` ancorado em `.rsm-fab-anchor` (vars do leque fora da `.informe-commercial-page`).
  ...
  - **REGISTRATION** → placeholder `.informe-placeholder`.
  - **CLASSIFIER** NAO acessa: o item Relatorios (`/informe`) e **filtrado** da tabbar dele (sobra Inicio/Lotes/Camera/Clientes — 4 itens). **CADASTRO** (viewer) mantem Relatorios no 5o slot. _(A pagina **/metrics** e a logica `isMetricsNavRole` foram **REMOVIDAS** em 2026-06-23 — era so um placeholder "Em construcao"; nao ha mais item Metricas em navbar/menu/sidebar.)_
...
- O viewer Relatorios (`RelatoriosViewer`, ADMIN/CADASTRO) consome o mesmo feed (scope=all) e renderiza os 3 tipos: `VisitReportCard typeBadge="Prospecção"`, `CommercialVisitCard` (twin do `VisitReportCard` — mesmo card + curadoria, ver abaixo), `WeeklyReportCard`
