...
- **Ultimas atividades**: card REMOVIDO em 2026-07-06 (componentes, CSS, rota `/api/v1/dashboard/recent-activity` e backend sairam; a informacao que entrara no lugar sera definida). O sheet do admin mobile agora termina em "Lotes disponiveis". De `lib/dashboard-activity.ts` sobrou so `formatRelativeTime` (usado pelo modal de Detalhes do contrato)
...
- ⚠️ **CSS orfao legado:** ha um cluster de classes mortas do dashboard antigo em `globals.css` (`.dashboard-secondary-grid`, `.dashboard-section-column/-link/-subtitle`, `.dashboard-op-print/-progress*`, variantes `.dashboard-skeleton-line-md/-lg/-full/-xs`+`circle`/`content`, `.sales-total-number`) **INTERLEAVADAS com classes VIVAS** (`.dashboard-section-heading/-title` e `.dashboard-operations-grid` sao usadas pelo PROSPECTOR; `.dashboard-skeleton-line-sm`/`-card`/base vivas). Remover SO classe-a-classe com grep antes — delecao por faixa de linha QUEBRA o prospector.
