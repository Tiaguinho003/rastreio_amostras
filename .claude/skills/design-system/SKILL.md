...
- **Informativos** (`InformativoFormSheet` + `InformativoForm`, opcao "Informativo" do leque/faixa): **side-sheet de DUAS colunas** (`.fv-panel-sheet side-sheet informativo-sheet` + seta ← edge-back, ~1000px no desktop; RD16 §2.10 R10) — formulario a esquerda, **previa 1080×1920 AO VIVO** a direita com abas **Mercado | Meteorologico**. Saiu o wizard de 3 fases: o mercado (21 campos, obrigatorio) e o meteorologico (6 campos, **OPCIONAL** via toggle `set-inclui-meteo`) convivem numa superficie so; a previa substitui a "revisao". Gera PNGs 1080×1920 (story 9:16) desenhados em `<canvas>` no cliente; **nada e salvo** (sem rota de API, sem banco). **As duas canvases ficam MONTADAS** (a inativa `display:none`, mas viva pro `toBlob`); a previa repinta **DEBOUNCED (160ms)** via `useDebouncedValue`, com o print da previsao injetado AO VIVO no paint (debounca-lo desalinharia 160ms apos colar); o **download pinta com os dados vivos** (`renderToBlob`) pra bater com o ultimo digito. Sem fase de revisar, a validacao (INF28) migra pro clique de baixar, **por secao**. **Footer**: "Baixar mercado"/"Baixar meteorologico" (nao fecham) + "Baixar os dois" (fecha), ou "Baixar informativo" com o meteo desligado. O `shareOrDownloadFiles` entrega as duas (um share sheet no celular, 2 downloads no PC); cada peca e um gesto proprio, entao nao dispara o aviso de "varios downloads" do Chrome.
  ...
  - CSS proprio no prefixo **`.ifm-`** (workspace 2-col, abas da previa, switch do meteo, palco das canvases). Os campos trocaram os cards `.inf-card` pelo shell institucional **`.fv-form-heading`** (`.ifm-group`), mantendo o kit de INPUT `.inf-field`/`.inf-input`/`.ifm-pair`/`.ifm-quad`/`.ifm-dir`/`.ifm-drop`. As cores do toggle de direcao (`.ifm-dir.is-alta/.is-baixa`) sao hex da **peca**, nao tokens do app — a fonte-da-verdade e o `COLORS` de `lib/informativos/story-layout.ts` (mexeu num, mexeu nos dois). **Legado pos-R10**: o `.bottom-sheet.is-informe` (achatamento) e as classes do wizard antigo (`.ifm-preview-*`/`.ifm-revisao*`/`.ifm-footer-actions`) viraram CSS morto — varredura na consolidacao pos-device.
