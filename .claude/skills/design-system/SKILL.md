...
> Componente: `components/DetailOverlay.tsx` (casca fina sobre o `BottomSheet` com `className="detail-overlay"`, `manageHistory={false}`, `dragToDismiss={false}`). E o contentor canonico de DETALHE do redesign (RD5): a pagina de detalhe morre e o conteudo abre como overlay **dirigido por URL** sobre a pagina de lista. Primeiro uso: detalhe do cliente em `/cadastros?cliente=<id>` (F1, 2026-07-20). Replicar este molde ao migrar outros detalhes.
