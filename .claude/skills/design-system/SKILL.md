...
- **SEM navbar**: o PROSPECTOR nao renderiza a MobileTabbar (`hideMobileTabbar` no AppShell). No lugar do botao da camera fica o botao central `.prospector-fab` — mesma posicao/gradiente (`#1a6b2e→#0d4a1a`) do pill da camera, porem **quadrado arredondado** (`border-radius clamp(18-22px)`) e ligeiramente menor (`clamp(4.2-4.9rem)`), com icone "+"; abre o formulario num bottom sheet `.is-informe`; some quando ha sheet/modal aberto. Em `/profile` o prospector ganha seta de voltar (sem navbar nao haveria caminho de volta); o menu do avatar dele nao mostra "Metricas"
