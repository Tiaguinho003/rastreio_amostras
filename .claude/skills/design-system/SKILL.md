...
- A rota `/informe` virou a pagina **"Relatorios"** que adapta o conteudo ao papel (`app/informe/page.tsx`); `/resumo` redireciona pra ela (server-side). Guard `INFORME_ROLES` = ADMIN/COMMERCIAL/REGISTRATION/**CADASTRO**. Branches:
  ...
  - **CLASSIFIER** NAO acessa: ve **Métricas** na NAVBAR no lugar de Relatorios (`isMetricsNavRole` + nao-viewer; rota `/metrics`). **CADASTRO** (que virou viewer) mantem Relatorios no 5o slot da tabbar e acessa Metricas pelo menu do avatar.
