...
- **Arquitetura:** `app/dashboard/page.tsx` renderiza `DashboardMobile` + `DashboardDesktop` JUNTOS (nao-prospector); o CSS alterna por `@media (min-width:901px)` (`.dashboard-desktop`/`.dashboard-mobile` viram `display:contents`/`none`). Ambos os twins vivem so das props (`data`/`salesData` do `useDashboardData`) — o fetch dedicado do desktop saiu junto com o card "Vendas e perdas" (DSH-D3, 2026-07-07). CSS do desktop e **desktop-first na base** + mobile em `@media (max-width:900px)`.
...
- **Linha 2 — `.dd-content-grid`** (cols 1fr/1fr): so "Lotes disponiveis" (`SalesAvailabilityCard` `compact`: donut + legenda + botao "Ver disponiveis" **full-width no rodape**) na coluna ESQUERDA; a coluna direita esta **vazia** — layout provisorio apos as remocoes de "Ultimas atividades" (2026-07-06) e "Vendas e perdas" (`CommercialTrendCard` + rota `commercial-timeseries` + CSS `dd-trend-*`, removidos em 2026-07-07, DSH-D3); a informacao substituta sera definida. Sombra de todos os cards/containers via var **`--dd-card-shadow`** em `.dashboard-page` (tunavel num lugar so).
