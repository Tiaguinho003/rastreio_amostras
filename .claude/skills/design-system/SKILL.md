...
`/contratos`+`/financeiro` → globais). Mobile de cada pagina entra na E2 dela.
...
- Chrome: **sidebar UNICA** (logo completo centralizado + itens com sub-itens `?tab=` +
  ...
  Vale pra todas as paginas desktop nao-PROSPECTOR. (AppShell segue **sem** chamar
  `useSearchParams`: `activeSubTab` chega por prop. **Desde a F2 do ciclo SN quem le o `?tab=` e o
  `app/(app)/layout.tsx`**, dentro do `<Suspense>` dele — nao mais cada pagina. O AppShell valida
  esse valor cru contra os `subItems` da rota ativa e cai no primeiro quando nao casa.)
...
>
> _(Sairam da lista: `/relatorios` na RD16 §2.10, `/users` na §2.11, `/profile` na §2.12,
> `/contratos` na RC-F6 (1a rodada) e `/financeiro` na 2a (RC-D92..D95). Nas duas de contrato, o
> **miolo do card mobile** (`.ctr-card`/`.fin-card`) ainda e legado — a moldura em volta e kit.)_
>
...
> ⚠️ **Nao existe mais header verde POR PAGINA no mobile.** Cada pagina desenhava o seu
> (`.samples-page-v2-header`, `.clients-v2-header`, `.sdv-header-top`, o bloco de avatar do
...
- E `position: fixed`; **quem reserva o espaco e o `padding-top` do `.app-shell-main`** (token
  ...
  repete `env(safe-area-inset-top)`** — a faixa ja o consome.
- Toda regra de reserva pende de **`.app-shell-root:has(.fv-mtopbar)`**: o PROSPECTOR nao recebe a
  faixa (app restrito) e compartilha `.dashboard-hero`, o `.sdv-header` do `/profile` e a variante
  `.is-dashboard-route` com os demais papeis.
- A unica faixa `.sdv-header-top` que sobrou e a do `/profile` **do PROSPECTOR** (sem tabbar e sem
  header do shell, ele precisa do back).
- Detalhe de lote/cliente segue com `.sdv-header` proprio: vive dentro do `DetailOverlay`, alguns
  niveis abaixo do `.app-shell-page-content`, e nao tem faixa acima.

Decisoes e commits: `docs/Redesign-Plano-de-Trabalho.md` RD16 + §2.8.

### Sheet de Conteudo (area bege)

- Fundo quente: `linear-gradient(180deg, #fdf9ec 0%, #f4f0e7 100%)`. **Excecoes aprovadas (fundo claro frio)**: (1) dashboard mobile do ADMIN — **BRANCO `#ffffff`** no sheet (2026-06-22, `.dashboard-mobile .dashboard-sheet`; era `#f4f6f5`); o PROSPECTOR segue em `#f4f6f5` (`.prospector-dashboard .dashboard-sheet`, divergiu do admin); (2) pagina de **Lotes `/samples`** — **BRANCO puro `#ffffff`** no sheet **e** nos cards (`.spv2-card-wrap`, sai o gradiente quente) e o recuo lateral redistribuido (pouco no sheet, mais no `.spv2-list-scroll`) (2026-06-16, escopado `.samples-page-v2:not(.informe-commercial-page)` — NAO afeta o `InformeCommercialPage`/shell reusado). **A "sombra reforcada" dos cards MORREU no RD16 M2 (2026-07-22)**: o card da lista de lotes e institucional — `var(--fv-surface)` + `1px solid var(--fv-line)` + `var(--fv-radius-lg)` + `box-shadow: 0 1px 2px rgba(0,0,0,.04)` — quem define a borda sobre o branco e a hairline, nao a sombra (ver §0 e `data-tables` §9). Na rodada 2 o MIOLO foi junto: saiu a **barra de status lateral** (paleta paralela a do chip) e sairam os icones da linha de dados; a tipografia passou a ser a da linha da tabela. `.spv2-card-bar` e `.spv2-card-sep` seguem no CSS para o `RelatedSampleRow`, que nao entrou no ciclo (o picker de lote do contrato saiu da lista na RC-D49 e foi apagado na RC-D69, quando o lote virou campo do formulario). As demais paginas deste bullet seguem na sombra reforcada; (3) pagina de **Clientes** (`.clients-page-v2`, hoje renderizada em `/cadastros`) — **BRANCO puro `#ffffff`** no `.clients-v2-sheet` **e** nos cards (`.cv2-card`, sai o gradiente quente). **No RD16 (2026-07-23) o card virou institucional** (escopado `.clients-page-v2`, espelhando os Lotes): `var(--fv-surface)` + `1px solid var(--fv-line)` + `var(--fv-radius-lg)` + `box-shadow: 0 1px 2px rgba(0,0,0,.04)` — saiu a sombra reforcada; na rodada de ajustes finos (2026-07-23) saiu tambem a **barra de status lateral** (a regra `.cv2-card::before`/`.is-incomplete::before` foi apagada em `.clients-page-v2`; o status agora vem do chip `.fv-chip`). O indicador alfabetico (`.cv2-section-divider`), que de 2026-06-18 ate o RD16 foi um **chip flutuante** branco com sombra/cantos/`z-index`, virou na mesma rodada uma **letra nua, estatica e minimalista** (so a letra, menor, `font-weight:500`, cinza suave — sem contorno, sem sombra, sem sticky); antes (2026-06-17, batch de alinhamento de /clients ao /samples) era faixa branca de largura total. (4) _(MORTA)_ pagina de **Informe** comercial (`.informe-commercial-page`) — a pagina foi apagada na unificacao de 2026-07-15 e o CSS dela saiu na consolidacao de 2026-07-27; o que valia aqui migrou pro item (5). (5) pagina de **Relatórios** (`/relatorios`, ex-`/resumo`; informes de visita) — sheet **BRANCO** (`.sdv-content.informe-content.rsm-content`) + recuo lateral redistribuido (pouco no sheet, mais no `.rsm-feed`), mantem o shell `.sdv-page`. **No RD16 §2.10 R2 (2026-07-23) o card virou institucional** (base `.rsm-card`: `var(--fv-surface)` + `1px solid var(--fv-line)` + `var(--fv-radius-lg)` + `box-shadow: 0 1px 2px rgba(0,0,0,.04)`, espelhando Lotes/Clientes) — saiu o gradiente quente + a sombra tripla, e o override branco de `.rsm-content .rsm-card` foi removido; o MESMO card base serve o feed do dashboard do prospector. (6) pagina de **Perfil** (`/profile`) — **BRANCO puro `#ffffff`** no sheet (`.sdv-content.stg-content`), mantendo o shell `.sdv-page`. **No RD17 P1 (2026-07-27) o card virou institucional** (`.sdv-card.stg-card`: `var(--fv-surface)` + `1px solid var(--fv-line)` + `var(--fv-radius-lg)` + `box-shadow: 0 1px 2px rgba(19,35,24,.05)`, espelhando Lotes/Clientes/Relatorios) — saiu a **sombra reforcada** de tres camadas; na mesma passada os campos (`.stg-field-row`) sairam do cinza-frio `#f4f5f7` para `var(--fv-canvas)` + hairline `var(--fv-line)` + `var(--fv-radius-lg)`. `/samples`, `/clients`, `/informe`, `/resumo`, `/profile` e o **dashboard** usam sheet branco. **2026-06-22: o FUNDO DE PAGINA (`--mobile-page-bg-base` + `html`) virou branco `#fff`** (era gradiente bege), desktop e mobile, a pedido do usuario — a regra "nunca `#fff` puro" foi superada. Sheets de paginas ainda nao migradas seguem no bege quente
...
- **Hero**: saudacao grande em 2 linhas (label `clamp(1.25-1.5rem)` w400; nome `clamp(2.3-3rem)` **w700**) + papel com escudo `#7eccae`; verde liso vindo do shell (sem textura no proprio `.dashboard-hero`, continuo com a area da busca — sem emenda na borda de baixo do hero); **o avatar saiu do hero na M1 do RD16** (2026-07-22): mora na faixa `.fv-mtopbar` do shell. O override translucido `rgba(255,255,255,0.18)` de `.header-avatar-trigger .user-avatar` (`!important` por causa do backgroundColor inline do `UserAvatar`) segue no CSS, hoje so alcancando o dashboard do PROSPECTOR
...
- **Cards de pendencias (mobile): REMOVIDOS em 2026-07-12 (DSB-D2).** Os op-cards "Lotes"/"Clientes" (classificacao pendente + cadastros incompletos) sairam do `DashboardMobile`. "Classificacao pendente" virou a KPI "Aguardando classificacao" na pagina de Lotes (`/samples`) — o card `.spv2-pending-stat`, orfao desde a RD15-F1, foi REMOVIDO (`d136bfc`); "Cadastros pendentes" foi removido por completo. As classes `.dashboard-operation-card`/`.dashboard-operations-grid`/`.dashboard-operation-badge` seguem VIVAS pelo **PROSPECTOR** (mesmo visual, cards de contagem de visitas). Ver `docs/Dashboard-Plano-de-Trabalho.md`.
...
- **Chrome (DSB-D15 + DSB-D16, 2026-07-14):** o desktop dos 5 papeis nao-PROSPECTOR tem **TOP BAR GLOBAL + SIDENAV lateral em 2 colunas + canvas verde-clarinho** (referencia visual do usuario):
  ...
  - **SIDENAV** `.app-sidenav` (RD13 — **painel UNICO branco de altura inteira**; o trilho + painel de 2 colunas do DSB-D15 MORREU e o avatar migrou pra top bar, junto com o dropup `.app-sidenav-profile-menu`. Nao existem mais `.app-sidenav-rail` nem `.app-sidenav-panel`): topo `.fv-sidenav-brand` com o **logo completo** (link pro dashboard, na altura da top bar — as hairlines emendam); miolo `.fv-sidenav-scroll` com o label `.fv-sidenav-heading` "Menu" + `<nav class="app-sidenav-nav">`; rodape `.fv-sidenav-footer` com **Perfil** e **Ajuda** (esta inerte por ora). Item = `.app-sidenav-link`, que serve **Link E button** (reset de button embutido, porque a secao expansivel usa o mesmo visual): altura `--app-nav-row-h`, padding `0 0.6rem`, radius `--fv-radius`, 0.88rem w500 + icone do `renderNavIcon`. **Item ativo = fundo NEUTRO `--fv-active-bg` + w600** (a pilula verde saiu no RD13); hover `--fv-hover` em `@media (hover:hover)`; `:active` so scale (button-press-effect). Coluna = `var(--app-sidenav-w, 252px)` — a var **nao e definida em lugar nenhum**, entao vale o fallback. Secao com sub-itens (`NAV_SUB_ITEMS`) = `.fv-sidenav-section` + toggle com `.fv-sidenav-chevron` e `.fv-sidenav-subnav`/`.fv-sidenav-sublink`. **Ordem dos itens (2026-07-28, pedido do Flavio):** Inicio · Lotes · Contratos · Relatorios · Cadastros · Financeiro (ADMIN) · Usuarios (ADMIN) — montada em `desktopNavItems`, com os condicionais sumindo por papel sem alterar a ordem relativa.
  ...
  - **TITULO DA PAGINA** `.app-page-title` (DSB-D17): `<h1>` no topo do `.app-shell-main` (shell, desktop nao-PROSPECTOR), **dentro da pagina** — texto = rotulo do item de nav ativo, so nas 8 rotas principais (match exato; detalhes/Perfil mantem headers proprios). **Alinhado verticalmente ao botao "Inicio"** via tokens compartilhados `--app-nav-row-top` (0.9rem) + `--app-nav-row-h` (34px) — o painel/links da sidenav usam os MESMOS tokens (mexeu num, mexeu nos dois). Row do titulo segue o box do dashboard (`max-width: var(--content-max-wide)` + padding `clamp(0.85rem, 3vw, 4rem)`); fonte 1.15rem w700 `--ink`. Titulos proprios das paginas ficam escondidos no desktop (hoje so o `.inf-intro-title` do viewer de Relatorios — o `.nsv2-title` dos headers de pagina morreu na M1 do RD16).
  - Grid 2 colunas via `:has(.app-sidenav)` (`--app-sidenav-w`, 276px); chrome **sticky**, NAO `position:fixed` (o PageTransition aplica transform e quebraria fixed). A faixa verde antiga (`.topbar`) **so e montada para o PROSPECTOR** desde a M1 do RD16; para os demais papeis nem chega ao DOM. GOTCHA corrigido no DSB-D17: o main de Lotes/Clientes descontava `4.5rem` hardcoded (top bar do DSB-D6) → `var(--app-topbar-h, 4.5rem)`. `:has(.app-sidebar)` segue reativando o grid do **PROSPECTOR**, que mantem a **sidebar vertical verde** + faixa branca com o avatar e fundo branco (app restrito). **"Sair" segue so no menu do avatar.** Mobile inalterado (topbar verde + MobileTabbar + hero). _(Historico: DSB-D6 (2026-07-12) = top bar de nomes sem icone, ativo sublinhado — superada, CSS removido.)_
- **Saudacao no desktop: REMOVIDA de vez.** O bloco `.topbar-greeting` saiu da faixa branca em DSB-D6; o cabecalho do dashboard `.dd-page-header` ("Visao geral" + saudacao/nome + papel + data, classes `.dd-page-*` + `getTodayLong`) saiu em **DSB-D16**. So o **hero mobile** (`DashboardMobile`) mantem saudacao. **Busca de lote**: NAO existe mais em chrome nenhum. O `.topbar-search-slot`/`SampleSearchField` vivia na `.topbar` e era inalcancavel (so montava para nao-PROSPECTOR, dentro de um header que nunca aparecia para eles); saiu na M1 do RD16. Divida aberta: a busca global de lote precisa de casa nova.
- **1a linha — StatCards de pendencia: REMOVIDOS em 2026-07-12 (DSB-D2).** A linha `.dd-summary-row` (Classificacao pendente / Cadastros pendentes) saiu do desktop; o `StatCard.tsx` e o CSS `.dd-stat-*`/`.dd-summary-row` foram **deletados**. "Classificacao pendente" migrou pra pagina de Lotes (`/samples`) como a KPI "Aguardando classificacao"; o card `.spv2-pending-stat` foi REMOVIDO (`d136bfc`); "Cadastros pendentes" foi removido. _(Historico: os StatCards de pulso "Lotes registrados hoje"/"Envios concluidos hoje" e o delta `formatDelta`/`.dd-stat-delta` ja tinham saido em 2026-07-07 com o payload `dailyRegistered`/`dailySent` — DSH-D4.)_
...
- **SEM navbar**: o PROSPECTOR nao renderiza a MobileTabbar (`hideMobileTabbar` no AppShell). O botao `.prospector-fab` — gradiente do pill da camera (`#1a6b2e→#0d4a1a`), **quadrado arredondado** (`border-radius clamp(18-22px)`, `clamp(4.2-4.9rem)`), icone "+" — fica fixo no rodape **a direita, alinhado ao centro do avatar do hero** (right = padding do hero + meio avatar − meio botao); abre o formulario de visita (desde 2026-07-23 o mesmo **painel lateral** da pagina Relatorios, nao mais o bottom sheet `.is-informe`); some quando ha sheet/modal aberto. Em `/profile` o prospector ganha seta de voltar no canto esquerdo (`.stg-header .sdv-header-top` com `width: 100%` — sem isso o align-items:center do stg-header encolhe o header-top)
- _(SUPERADO em 2026-07-23, RD16 §2.10 R3)_ o sheet `.is-informe` do prospector: o formulario de visita virou o **painel lateral** unificado (`CommercialVisitFormSheet`), o mesmo da pagina Relatorios. A `.inf-intro` (e o `.inf-intro-title`) foram removidas do JSX e do CSS.

### Barra de navegacao inferior (MobileTabbar)

Pill flutuante (`.mobile-tabbar-inner`) renderizada via Portal no `body` (`components/MobileTabbar.tsx`); todos os papeis EXCETO PROSPECTOR renderizam — `hideMobileTabbar` no `AppShell` e **so** `isProspector(role)`, nao ha lista de rotas (os detalhes de lote e de cliente viraram OVERLAY sobre a lista na F1/F2 do redesign e escondem a tabbar via `body.is-bottom-sheet-open`, nao por rota). **5 itens FIXOS** (2026-07-28), na mesma ordem relativa da sidenav desktop sem as duas ADMIN-only: **Inicio, Lotes, Contratos, Cadastros, Relatorios**. Nao ha mais slot por papel — o antigo 4o slot alternava Relatorios/Perfil pra completar 4 itens, mas desde o acesso unificado `INFORME_ROLES === NON_PROSPECTOR_ROLES`, entao o ramo Perfil nunca renderizava. **Perfil, Usuarios e Financeiro sao os que ficam no menu do avatar** (`HeaderAvatarMenu`) — quem esta na barra NAO se repete no menu. _(Historico: o slot central da camera saiu na CAM-P3, 2026-07-16, junto com o conceito `emphasis`/`is-primary`; "Clientes" saiu na F1 do redesign 2026-07-20 — `/clients` e redirect e Cadastros vale pra todo nao-PROSPECTOR.)_ A camera abre por um **icone no header** (cluster `[camera][avatar]` do `HeaderAvatarMenu`) → bottom sheet global `CameraSheet`. **Desde a M1 do RD16 (2026-07-22) esse cluster e montado uma unica vez, na faixa `.fv-mtopbar` do `AppShell`** — antes cada pagina montava o seu. Estilos em `app/globals.css` (`.mobile-tabbar*`, `.header-camera-trigger`, `.camera-sheet-stage`).

- **Superficie BRANCA** (redesign 2026-06-15, mockup): `.mobile-tabbar-inner` `--fv-surface`, hairline `--fv-line` + sombra reforcada em camadas (`0 4px 12px /.14` + `0 14px 34px /.22`) pra destacar do fundo (era barra verde-escura `#0b4a04→#073603` com icones brancos)
- **Estados por item** (paleta trocada na M1 do RD16, 2026-07-22 — ver §0): inativo `--fv-chip-gray-fg` (icone via `currentColor` + label w500); **ativo `--fv-cta`** (icone + label w600) com **indicador = traco curto arredondado ABAIXO do label** (`::before`, `bottom`, `border-radius: 999px`). Superficie e hairline da pill saem de `--fv-surface`/`--fv-line`. _(Antes: inativo `#74837a`, ativo verde-grama `#1e8540`, hex soltos.)_
- **Traco fino**: labels leves (w500/w600) e icones com `stroke-width: 0.9`
- **Layout = grid de colunas iguais**: `.mobile-tabbar-inner` usa `grid-auto-flow: column; grid-auto-columns: minmax(0,1fr)` -> os itens ficam exatamente equidistantes seja qual for a quantidade (o CSS **nao conta itens** em lugar nenhum); cada link centra o conteudo na propria coluna
- **Altura escalada 2x por -10%**: primeiro a barra inteira (icone/label/paddings internos por 0.9; clamps em `.mobile-tabbar-icon`, `-label`, `-link`), depois so o **icone** na passagem pra 5 itens (`clamp(1.83rem, 8.1vw, 2.19rem)`, ~29.3px no SE ate ~35px no Pro Max). `--mobile-tabbar-clearance` acompanhou as duas (8.9rem -> 8.3rem -> **8.1rem**). Ao mexer na altura da barra, reavaliar a clearance pra nao sobrar/faltar respiro
- **Encaixe horizontal (o que aperta quando entra um item)**: com 5 colunas a largura por item cai de ~69px pra **~57px no pior caso (320px)**, e quem estoura primeiro e o NOME, nao o icone. As 3 alavancas, em ordem de uso: `gap` do `-inner` (0.12rem), padding lateral da `.mobile-tabbar` (`clamp(0.6rem, 3vw, 1.25rem)`) e do `-inner` (`clamp(0.3rem, 1.6vw, 0.5rem)`) — o tamanho do label so depois delas. Referencia do dimensionamento atual: sobra pro nome 50.2px em 320px, 62.9px em 390px; "Relatorios" (o mais largo) mede ~43px em Inter 500 @9.36px
...
| `.sdv-info-value-row`       | Linha de leitura com **botão de copiar**: `<div class="sdv-info-value-row">` com o `.sdv-info-value` (que trunca com reticências) + `.sdv-info-copy` — botão **sem moeda e sem borda**, só o ícone em `--brand-green` (`fill:none`), 1.6rem, afunda no `:active`. Use SEMPRE que a linha for `.sdv-info-*` e precisar copiar; não monte outro em cima do `.fv-iconbtn` (foi o erro da §2.11 U3, corrigido em `258ac62`). Consumidores: detalhe do cliente (`ClientDetailView`) e o painel de `/users` (e-mail/telefone).                                                                      |
...
- **Variante `.is-menu` (altura por conteudo):** `className="is-menu"` troca a altura fixa alta por `height: auto` + `max-height: min(72dvh, 30rem)`, pro sheet encolher ao conteudo (poucas linhas em vez de ocupar quase a tela). Usada pelo menu da conta no header mobile (`components/HeaderAvatarMenu.tsx`): botao de avatar (`.header-avatar-trigger`, mobile-only, substituiu o antigo sino) que abre um launcher com resumo (`UserAvatar` md a esquerda + coluna nome/cargo, classe `.header-avatar-menu-summary-text`) + linhas Perfil/Usuarios(adm)/Financeiro(adm)/Sair — cada linha fecha o sheet e navega. **Regra do menu (2026-07-28): so entra aqui o que NAO esta na tabbar** — Cadastros e Contratos saíram quando viraram abas. _(O "Resumo" migrou pra Relatorios na unificacao do `/informe`; a row "Metricas" foi removida em 2026-06-23 junto com a pagina.)_ **Excecao ao chrome do BASE:** sem titulo verde visivel (`title=""`) — o resumo ja encabeca o menu; o `<h3>` fica vazio mas presente, entao o X segue a direita (header `space-between`) e a altura nao muda; nome acessivel do dialog via `ariaLabel`.
- **Variante `.is-fit-content` (altura por conteudo, teto do BASE):** `className="is-fit-content"` so faz `height: auto` (NAO reduz o `max-height`, que segue o do BASE — ~tela cheia menos o `--bottom-sheet-top-gap`, hoje 4rem apos RD16 M3). Diferente da `.is-menu`/`.is-operations` (que apertam o `max-height` pra menus curtos), e pra sheets curtos que devem encolher ao conteudo mas ainda crescer se preciso: o sheet encolhe (footer logo abaixo do ultimo campo, sem o vao do `flex:1`) e rola so se estourar. Usada por `NewSampleModal` (Nova Amostra, form longo), `SampleLossSheet` (perda) e `SampleLabelPrintSheet` (etiqueta). **NAO** usar em sheet com dropdown absoluto inline (lookup de cliente, `.client-lookup-dropdown` = `position:absolute`): num sheet que abraca o conteudo o `overflow` do body corta o dropdown — esse sheet fica de altura cheia (ex.: painel de envio, meta-step da camera).
...
- **Modal de novo cliente (`.client-quick-create-sheet`, `ClientQuickCreateModal`) — superficie BRANCA** (`#fff`, pedido do usuario 2026-06-23): nao e uma variante de altura, e o BASE com override de cor — `.bottom-sheet.client-quick-create-sheet` sobrescreve o creme do `.bottom-sheet` base **e** o `.bottom-sheet-footer` (que tem fundo creme proprio, nao herda) pra a faixa das acoes (Cancelar/Cadastrar) tambem ficar branca. Os campos migraram pro kit `.app-modal-input` (fundo glass cinza translucido; RD16 2026-07-23) e os `.chip-select` foram alinhados a ele — se definem sobre o branco pelo fundo + borda. Vale mobile (sheet) E desktop, onde o mesmo elemento e **painel lateral** (`side-sheet`), nao modal central. Espelha is-menu/is-operations.
...
- **Variante `.is-informe` — LEGADO/morta (pos-RD16 §2.10):** achatava as secoes `.inf-card` e dava padding lateral ao `.inf-form`. **Nao ha mais consumidor**: a visita e o Semanal migraram pro `.fv-panel-sheet`+`.fv-form-*` (R3/R4) e o Informativo idem (R10, ultimo a sair). O CSS `.bottom-sheet.is-informe .inf-*` foi **removido** na consolidacao pos-device (2026-07-27). (Nao confundir com `.hero-search-wrap.is-informe` e `.cv2-fab.is-informe-fab`, que sao VIVOS e de outro contexto — o FAB/leque.)
...
- **Mobile**: sheet de TELA CHEIA (`--bottom-sheet-top-gap: 0`, sem raio, sem padding lateral — o conteudo carrega o proprio padding). **Desktop (>=901px)**: painel lateral DIREITO `min(620px, 92vw)`, desliza da direita. O backdrop nasceu transparente e **atravessavel** (lista viva; clicar noutro card trocava o item aberto), mas **cliente e lote sao BLOQUEANTES hoje** — ver `containers` §2. So o detalhe de contrato ainda e peek. CSS no bloco `.detail-overlay` do `globals.css`.
...
> NAO e componente novo: e uma **classe CSS** aplicada no `className` de um `BottomSheet` comum (ex.: `className="is-fit-content side-sheet"` no `NewSampleModal`; `client-quick-create-sheet side-sheet` no `ClientQuickCreateModal`). No **mobile nada muda** (sheet de baixo, como sempre); em **desktop (>=901px)** o CSS transforma o sheet num **painel lateral DIREITO** com a MESMA geometria do peek do DetailOverlay: `width: min(620px, 92vw)`, ancorado top/right/bottom, sem raio, desliza da direita (`translate3d(100%,0,0)` → `(0,0,0)` no `.is-open`), `border-left` hairline + `--shadow-xl`. Bloco `.bottom-sheet.side-sheet` no `globals.css`. Em producao desde 2026-07-20 (commits `875c777`/`7a2abbe`, extensao da F2 do redesign). **Excecoes de largura, todas no `@media (min-width: 901px)`**: `.informativo-sheet` ~**850px** (duas colunas: form + previa 1080×1920 ao vivo) e `.ctr-contract-sheet` **min(700px, 92vw)** (RC-D61 — 26 campos, linhas de 3 colunas e uma folha A4 no ultimo passo). 🔴 **O seletor da excecao PRECISA citar `.side-sheet`** (`.bottom-sheet.side-sheet.X`, 0,3,0): a regra dos 620px e `.bottom-sheet.side-sheet` (0,2,0) e mora **depois** no arquivo — com 0,2,0 a excecao perde por ordem, sem erro nenhum.
...
| ---------------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Backdrop desktop | **ESCURO BLOQUEANTE** nas paginas do ciclo FV (so contrato segue peek) | **ESCURO BLOQUEANTE** (o padrao do sheet): form tem estado sujo, lista nao clicavel |
...
### Pagina `/relatorios` = "Relatorios" (unifica os antigos /informe e /resumo)

- A pagina **"Relatorios"** e a rota **`/relatorios`** (`app/(app)/relatorios/page.tsx`); `/informe` e `/resumo` redirecionam pra ela (server-side). **ACESSO UNIFICADO (2026-07-15):** guard `INFORME_ROLES` = `NON_PROSPECTOR_ROLES` — **todo papel nao-PROSPECTOR** (ADMIN, CLASSIFIER, REGISTRATION, COMMERCIAL, CADASTRO) entra como VIEWER unico; o antigo ramo "meus" do COMMERCIAL foi aposentado.
  - **Viewer (todo nao-PROSPECTOR, `isVisitReportViewer`)** → `components/informe/RelatoriosViewer.tsx`: feed de TODOS (`scope=all`), shell `.sdv-page`/`.rsm-content` — no DESKTOP (RD §2.10 v2, 2026-07-23; ajustes 2026-07-24) ganhou a **faixa FV** (no **desktop = 2 cards**: o **gráfico de tendência semanal** em card próprio [`VisitsTrendChart`: 13 semanas ≈ 90 dias, meses/ticks/tooltip; prop `viewBoxWidth=460` p/ ficar ~50% mais largo SEM subir de altura] + **"Visitas esta semana"** com número grande e mini-métrica **▲/▼ N vs. semana passada** [`.fv-kpi-delta` is-up/is-down, molde /cadastros; Δ = visitsThisWeek−visitsLastWeek], **centralizado no vão** até os botões; no **mobile = 1 card só**, com o **número no cabeçalho ao lado do título** [`.rsm-week-headline`] e o gráfico no corpo — + 3 botões de criação) + **toolbar** (só busca debounced) + **chips de tipo** fixos no topo do feed (Todas/Visitas/Semanal, substituem o antigo painel de Filtros; menores, à esquerda, selecionado = verde sólido `--fv-cta` + texto branco) + cartão de altura cheia; no mobile a faixa/toolbar/chips entram na rolagem pelo kit. Todos criam (`canCreate` fixo) — ver **Criação** logo abaixo (botões da faixa no desktop + FAB "leque" no mobile). **Cards do feed em LINHA COLUNAR no desktop desde a R16 (2026-07-27)**: reflow so de CSS escopado em `.rsm-content` (`display: contents` achata os wrappers numa grid de 7 tracks — Autor · Data·hora · chip · Cliente/Semana · chevron); o JSX dos cards NAO muda, por isso o feed do dashboard do prospector (mesmos `.rsm-card`, fora de `.rsm-content`) fica intocado.
  - _(REMOVIDO 2026-07-15)_ o ramo **COMMERCIAL** `InformeCommercialPage.tsx` (feed `scope=mine`, shell `.samples-page-v2.informe-commercial-page`) foi apagado; o comercial usa o viewer acima. Na consolidacao de 2026-07-27 sairam as regras que EXIGIAM a classe (`.samples-page-v2.informe-commercial-page`, `.informe-commercial-page .rsm-card`, o `.hero-search-wrap.is-informe`); os **7 `:not(.informe-commercial-page)`** que qualificam regras VIVAS de `/samples` ficaram de proposito — tirar `:not(.classe)` baixa a especificidade de (0,2,0) pra (0,1,0) e pode virar a cascata de uma pagina fora da vez. Saem na proxima passada de `/samples`.
...
- **Formulario de Visita (`CommercialVisitFormSheet`)** — painel lateral `.fv-panel-sheet` + kit `.fv-form-*` (submit no footer, check terminal, descarte `.is-scrim-none`) desde RD16 §2.10 R3; o corpo `CommercialVisitForm` foi absorvido no sheet. As duas pilhas de "Identificacao do cliente" terminam num `Client` do cadastro. "Ja cadastrado" = `ClientLookupField`; "Cliente novo" = os campos de anotacao (nome/cidade/telefone) + CTA `.inf-newclient-cta` que abre o `ClientQuickCreateModal` (prefill do que foi digitado, `initialPersonType="PF"`). Depois de criar, o CTA da lugar ao chip inerte `.inf-newclient-linked` (nome canonico + codigo + "×" pra soltar). **O modal e renderizado FORA do `<form>`** — o BottomSheet usa portal, mas eventos de portal sobem pela arvore React e o submit dele chegaria ao `onSubmit` da visita (mesmo arranjo do `NewSampleModal`).
- **Criação (RD §2.10 v2, 2026-07-23):** o estado dos 3 BottomSheets de criação (Visita + Semanal + Informativo, ciclo open/mounted) vive no hook `components/informe/useInformeCreateSheets.tsx` — UMA fonte servindo as DUAS portas: os **botões da faixa** no desktop e o **FAB "leque"** (`InformeCreateRadialFab`) no mobile (`.relatorios-page .rsm-fab-anchor` é `display:none` ≥901px; escopado pra não apagar o FAB do dashboard do prospector, mesma classe). O antigo `InformeCreateFab` (só o FAB) foi absorvido e **removido**.
- **FAB radial de LAPIS** (`InformeCreateRadialFab`): usa o MESMO **leque** (`.fab-fan-*`) da /samples, com 3 opcoes, nesta ordem — **Informativo** (imagem/montanha) na posicao `is-lote` (acima do FAB, 1º do leque), **Visitas** (prancheta-check) na `is-liga` (diagonal a 45°) e **Relatório** (calendario) na `is-aprovacao` (esquerda). Diferenca vs /samples: a variante `.cv2-fab.is-informe-fab` mantem o **icone LAPIS** (DOIS svgs empilhados lapis ↔ × em **crossfade**, neutraliza o rotate 45° do "+"). Escurece a tabbar via `body.is-fab-fan-*` (igual /samples). Vars `--fab-*`/`--fan-*` em `.samples-page-v2.informe-commercial-page` (`--fab-right` = right real do FAB do informe, que NAO e reposicionado como o do Lotes). O drawer antigo `.fab-menu-*` foi removido. **O leque SEMPRE abre**: Visitas e Informativo valem p/ todo nao-PROSPECTOR, entao ha no minimo 2 opcoes — o atalho antigo (`!canCreateWeekly` abria a visita direto, sem leque) saiu quando o Informativo entrou. **Pegadinha:** era esse atalho que escondia o "Relatório" de quem nao e autor do Semanal; hoje a condicao e EXPLICITA no JSX, senao um CLASSIFIER veria um botao que o backend recusa com 403.
- **Informativos** (`InformativoFormSheet` + `InformativoForm`, opcao "Informativo" do leque/faixa): **side-sheet de DUAS colunas** (`.fv-panel-sheet side-sheet informativo-sheet` + seta ← edge-back, ~850px no desktop — afinado, ver nota 2026-07-27; RD16 §2.10 R10) — formulario a esquerda, **previa 1080×1920 AO VIVO** a direita. **SEM abas** (RD16 §2.10, pedido do Flavio): a superficie e uma PILHA de DOIS blocos (`.ifm-block`), cada peca FIXA (sticky) ao lado do SEU proprio formulario — bloco 1 = mercado, bloco 2 = meteorologico (opcional). Saiu o wizard de 3 fases: o mercado (21 campos, obrigatorio) e o meteorologico (6 campos, **OPCIONAL** via toggle `set-inclui-meteo`) convivem numa superficie so; a previa substitui a "revisao". Gera PNGs 1080×1920 (story 9:16) desenhados em `<canvas>` no cliente; **nada e salvo** (sem rota de API, sem banco). **A canvas do mercado fica sempre montada; a do meteo monta com o toggle ligado** (ambas vivas pro `toBlob` quando o download roda — e "Baixar os dois" so aparece com o meteo ligado); a previa repinta **DEBOUNCED (160ms)** via `useDebouncedValue`, com o print da previsao injetado AO VIVO no paint (debounca-lo desalinharia 160ms apos colar); o **download pinta com os dados vivos** (`renderToBlob`) pra bater com o ultimo digito. Sem fase de revisar, a validacao (INF28) migra pro clique de baixar, **por secao**. **Footer**: "Baixar mercado"/"Baixar meteorologico" (nao fecham) + "Baixar os dois" (fecha), ou "Baixar informativo" com o meteo desligado — os tres numa LINHA no desktop (`.ifm-footer-dl` = grid 3 colunas ≥901px; no mobile as duas secundarias dividem a 1a linha e a primaria ocupa a 2a). O `shareOrDownloadFiles` entrega as duas (um share sheet no celular, 2 downloads no PC); cada peca e um gesto proprio, entao nao dispara o aviso de "varios downloads" do Chrome.
  - **`PrevisaoPicker`** (`.ifm-drop-*`): a previsao do tempo e um **print colado** pelo usuario (colar/arrastar/escolher), nao uma API — ver `docs/Informativos-Plano-de-Trabalho.md` §4.3. E um **quadradinho** (`.ifm-drop` `aspect-ratio:1`) que divide a linha da Pluviosidade **metade a metade** (`.ifm-pair.ifm-rain-row`), NAO um grupo full-width: vazio = icone + "Colar print"; preenchido = **selo "✓ print colado"** (`.ifm-drop-sealed`/`.ifm-drop-check`, **SEM miniatura** — a previa 9:16 ao lado ja mostra a imagem toda) + Trocar/Remover (`.ifm-drop-link`). **Armadilha:** arquivo solto FORA da caixa faz o browser navegar ate ele e os campos digitados evaporam; o picker recusa `dragover`/`drop` no `window` enquanto vive.
  - CSS proprio no prefixo **`.ifm-`** (workspace = pilha de blocos `.ifm-block` form|previa, switch do meteo, palco da canvas). Os campos trocaram os cards `.inf-card` pelo shell institucional **`.fv-form-heading`** (`.ifm-group`), mantendo o kit de INPUT `.inf-field`/`.inf-input`/`.ifm-pair`/`.ifm-dir`/`.ifm-drop` **porem com a COR dos modais** (`.app-modal-input` — vidro cinza, compacta, **escopada em `.informativo-sheet`**; a unidade foi pro rotulo, sem `.ifm-unit`). O mercado futuro/CPR foi reorganizado **POR ANO** (`.ifm-year-cols`/`.ifm-year-col`/`.ifm-mp-row` — campos do mesmo ano juntos, sem "Linha N") e a variacao virou UMA linha (`.ifm-var-row` = pontos + Alta/Baixa). As cores do toggle de direcao (`.ifm-dir.is-alta/.is-baixa`) sao hex da **peca**, nao tokens do app (**SEMPRE tingido**: apagado quando nao selecionado, vibrante no `.is-selected`) — a fonte-da-verdade e o `COLORS` de `lib/informativos/story-layout.ts` (mexeu num, mexeu nos dois). **Legado pos-R10**: o `.bottom-sheet.is-informe` (achatamento) e as classes que ficaram sem consumidor (`.ifm-preview-*` do wizard, `.ifm-quad`, `.ifm-row-group`/`-legend`, `.ifm-revisao*`, `.ifm-footer-actions`, o kit `.inf-card*`/`.inf-choice*`/`.inf-form`) foram **removidos** na consolidacao pos-device (2026-07-27). **Modal afinado + blocos alinhados (2026-07-27):** o sheet caiu de 1080 pra **~850px** (`.informativo-sheet` no `@media≥901px`) — os campos sao curtos (valor/mes/ano/preco), entao o form (`1fr`) fica em ~420px (tamanho suficiente pro conteudo) sem largura desperdicada. Os **DOIS blocos** (mercado, meteo) compartilham o MESMO `.ifm-block` = `minmax(0,1fr) 340px`, entao formularios E previas ficam **ALINHADOS** na pilha (NAO ha mais override so-meteo — o `.ifm-block-meteo` foi removido; alinhar as duas previas reverteu a "previa maior" do meteo). O print da previsao e um quadrado (`.ifm-rain-row` = par Pluviosidade|previsao; selo "✓ print colado" — as regras da miniatura antiga `.ifm-drop-thumb`/`-dim`/`-preview`/`-title`/`-hint` sairam). O rotulo do registro e **DINAMICO**: `registroEmHoras(hoje)` (segunda-feira = **72h**, demais dias = 24h; `informativo-draft.ts`) alimenta o `<h3>` do form e o `REGISTRO EM {h}h` da peca (`MeteoData.registroHoras`, dado nao literal — layout puro).
...
- Feed: cards `rsm-*`. **No RD16 §2.10 R2 (2026-07-23) a base `.rsm-card` virou institucional** (`var(--fv-surface)` + `1px solid var(--fv-line)` + `var(--fv-radius-lg)` + `box-shadow: 0 1px 2px rgba(0,0,0,.04)` — saiu o gradiente quente + a sombra tripla; tap `scale(0.98)`, SEM hover de cor, chevron verde ao expandir). O tipo/estado agora e um **chip `.fv-chip.is-sm`** (Visita verde / Semanal azul / Cancelado cinza — posicionado por `.rsm-card-head > .fv-chip`, recua da lixeira via `.has-quick-delete`; o antigo `.rsm-type-badge` foi removido). Lixeira + confirm central; "Carregar mais" (mecanica rsm — o espelhamento da /samples e o shell visual, nao o cursor/sentinel). A base serve tanto /relatorios quanto o feed do dashboard do prospector.
- Relatorio semanal (`WeeklyReportFormSheet`, painel lateral `.fv-panel-sheet` + `.fv-form-*` desde RD16 §2.10 R4): campo read-only `.informe-week-label` ("Semana de DD/MM a DD/MM", espelho client-side `lib/weekly-report.ts`); 409 do servidor abre aviso central **sobre o painel** (`.fv-panel-scrim`, "Relatório já enviado", botao unico "Entendi") — regra bloqueante → modal, conforme skill feedback-messages
- O viewer Relatorios (`RelatoriosViewer`, todo nao-PROSPECTOR desde 2026-07-15) consome o feed combinado (`scope=all`) e renderiza **DOIS** tipos: `VisitReportCard` (Visita unificada — chip verde "Visita", ou cinza "Cancelado") e `WeeklyReportCard` (Semanal). O `CommercialVisitCard` foi DELETADO na unificacao.
- **NAO existe mais curadoria de vinculo** (2026-07-15): a Visita nasce vinculada a um `Client` real nos dois `clientKind`, entao sairam do card as acoes `.rsm-link-*`, o badge "Aguardando vinculo" e os modais de Vincular/Remover — junto com `linkCommercialVisitClient` no backend. Sobrou no card o icone de cliente novo/existente + a tag "Cliente novo". _(O CSS orfao dessas classes saiu na consolidacao pos-device de 2026-07-27.)_
