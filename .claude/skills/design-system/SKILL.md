...
- **Cards de pendencias** (sem heading "Operacoes"): labels **"Lotes" / "Clientes"** (dados seguem sendo classificacao pendente e cadastros incompletos; **"Clientes" so pra `CLIENT_MANAGEMENT_ROLES`**, e a `.dashboard-operations-grid` ganha `.is-single` sem ele); card branco flat radius 18px; icon-wrap rounded-square `#e8f1ec` com glifo SOLIDO `#1f5d43` (fenda do grao em stroke claro `#e8f1ec` inline no markup); titulo `#1a1a1a` w700 + subtitulo `#737774` (tom AA; era `#8a8f8c` ate a revisao DSH) empilhados (sem divider/chevron). Numero em CHIP verde `#e8f1ec`/`#1f5d43` **flutuando no canto superior direito** (`position: absolute`, `top` negativo popando pra fora da borda de cima, anel branco 2px + sombra suave), ancorado no card (`.dashboard-operation-card` e `position: relative`) — sai do fluxo pra o titulo+subtitulo ocuparem a largura TODA do card sem cortar "Pendentes" (era CHIP inline estatico que comprimia o texto). Pulsacao (`badge-pulse-soft`, scale 1.13, 2s — keyframe proprio do mobile, perto do `badge-pulse` 1.15 do desktop; a regra mobile sobrescreve o `animation: none` antigo), some quando 0. Identidade verde uniforme nos 2 cards (decisao do usuario — sai o ambar semantico)
...
- **Ultimas atividades**: card REMOVIDO em 2026-07-06 (componentes, CSS, rota `/api/v1/dashboard/recent-activity` e backend sairam; a informacao que entrara no lugar sera definida). O sheet do admin mobile agora termina em "Lotes disponiveis". De `lib/dashboard-activity.ts` sobrou so `formatRelativeTime` (renomeado pra `lib/relative-time.ts` na revisao DSH 2026-07-07; usado pelo modal de Detalhes do contrato)
...
- **Arquitetura:** `app/dashboard/page.tsx` renderiza `DashboardMobile` + `DashboardDesktop` JUNTOS (nao-prospector); o CSS alterna por `@media (min-width:901px)` (`.dashboard-desktop`/`.dashboard-mobile` viram `display:contents`/`none`). O mobile vive so das props (`data`/`salesData` do `useDashboardData`); o desktop tem UM fetch proprio (`getDashboardRecentSends` do card "Ultimos envios" — guard `matchMedia` + listener `change` + throttle 30s, so o breakpoint ativo busca). CSS do desktop e **desktop-first na base** + mobile em `@media (max-width:900px)`.
- **Chrome:** barra **vertical verde fixa** a esquerda (`.app-sidebar`: logo + nav icone+nome + "Sair" no rodape) + **faixa branca** superior (`.topbar` branca, sticky; layout em grid + sticky, NAO `position:fixed` — o PageTransition aplica transform e quebraria fixed). Mobile mantem topbar verde + MobileTabbar.
- **Saudacao na faixa branca** (so dashboard, so desktop): "Bom dia," + **nome grande** + tipo de usuario (escudo), a esquerda, **alinhada a coluna dos cards** (`.app-shell-root:has(.dashboard-page) .topbar-inner` espelha o box do `.dashboard-page`: max-width + margin auto + recuo). **Busca de lote removida** da faixa branca no desktop (`.topbar-search-slot{display:none}` >=901px; mobile mantem).
- **1a linha — ate 2 `StatCard`** (`components/dashboard/StatCard.tsx`; horizontais, icone verde em tile claro `rgba(31,93,67,.1)`): Classificacao pendente / Cadastros pendentes. **"Cadastros pendentes" so aparece pra `CLIENT_MANAGEMENT_ROLES` (ADMIN + CADASTRO)** e leva a `/cadastros?incomplete=true`; sem ele a `.dd-summary-row` ganha `.is-single` (1 coluna) pra o card restante nao deixar meia linha vazia — vale tambem pro esqueleto de carregamento. Grid `repeat(2, minmax(0, 1fr))` (decisao do usuario, DSH-D4 — os StatCards de pulso "Lotes registrados hoje"/"Envios concluidos hoje" e o delta `formatDelta`/`.dd-stat-delta` foram REMOVIDOS em 2026-07-07 junto com o payload `dailyRegistered`/`dailySent` do `dashboard/pending`).
- **Grid principal — `.dd-content-grid`** (cols 1fr/1fr de pagina inteira, `flex: 1`; o shell-lock de 100vh continua, sem scroll de pagina): coluna ESQUERDA = **`.dd-left-col`** (flex column) com a linha de pendencias (`.dd-summary-row`, agora `repeat(2, 1fr)` DENTRO da metade esquerda — largura de card igual a do antigo grid de 4 na pagina inteira) em cima e a pilha **`.dd-left-stack`** (rows `1fr/1fr`) embaixo — "Lotes disponiveis" (`SalesAvailabilityCard` `compact`) + **"Ultimos envios"** (`RecentSendsCard`, DSH-D5), MESMO tamanho. Coluna DIREITA inteira = card **"Eventos"** (`EventsCalendarCard`, DSH-D6/E20), do topo (altura das pendencias) ate a base. Animacoes de entrada nos FILHOS (stat-cards nth 1/2, stack 0.2s, eventos 0.3s). Sombra de todos os cards/containers via var **`--dd-card-shadow`** em `.dashboard-page` (tunavel num lugar so).
- **Card "Ultimos envios"** (`components/dashboard/RecentSendsCard.tsx`, desktop-only): shell branco radius 20 + header (titulo `#1a2e1f` + tile de icone verde `.dd-sends-icon`) + lista `.dd-sends-list` com **scroll interno**; minicards INERTES reusam o visual `.spv2-card is-static` do modal de Lotes pendentes (lote `spv2-card-code` + `BlendBadge` + pill do tipo `.dd-send-kind` `is-physical` verde / `is-report` azul + destinatario ATUAL em `spv2-card-owner` + tempo relativo `formatRelativeTime` com refresh 60s e `title` com a data exata). Cancelado = `.is-cancelled` esmaecido + tag `.dd-send-cancelled-tag`. Dados: `GET /dashboard/recent-sends` (40 itens, `PHYSICAL_SAMPLE_SENT`+`REPORT_EXPORTED`).
- **Card "Eventos"** (`components/dashboard/EventsCalendarCard.tsx`, desktop-only, F0+F1 — decisoes E1-E27 em `docs/Eventos-Dashboard-Plano-de-Trabalho.md`): calendario de **2 semanas domingo-first** (helpers date-only BRT em `lib/dashboard-calendar.ts` — NAO unificar com o weekly-report segunda-based). Header = titulo + rotulo do periodo ("7 – 20 de julho") + nav ◀ `Hoje` ▶ (passo 14 dias, passado/futuro livres). Grade `.dd-events-grid` 7 colunas: quadrados-`<button>` com numero + dots por tipo (max 3 + `+N`; **catalogo `.dd-events-dot[data-type]`: âmbar `#eab308` (`contract_payment_due`) / verde `#15803d` (`contract_payment_paid`) / vermelho `#dc2626` (`contract_payment_overdue` — pagamento vencido, E29) / laranja `#f97316` (`contract_approval_due`, F2) / azul `#2563eb` (`contract_shipment` — embarque agendado, EMB10) / azul-escuro `#1e40af` (`contract_shipment_done` — embarcado, EMB17) / vermelho `#dc2626` (`contract_shipment_overdue` — embarque atrasado, EMB24)**), fds APAGADOS (`.is-weekend` opacity .45), hoje = anel `inset var(--brand-green)` + `aria-current="date"`, selecionado = fundo `#e8f1ec` + `aria-pressed`, roving tabindex com setas, "1 ago" na virada de mes. Painel do dia protagonista (~60%, `.dd-events-panel` flex:1 scroll interno) com titulo "9 de julho — quinta-feira"; vazio = "Nenhum evento para este dia." (a nota de futuro E19 SAIU na F1). **Item do painel ramifica por `typeKey` (Revisao do Pagamento, E28):** evento de **pagamento** (`contract_payment_*`) = **navegacao PURA** — `.dd-events-item-link` (`<Link href="/contratos?tab=financeiro&highlight={contractId}">`, label "nº · comprador" + seta `.dd-events-item-go`), SEM acordeao/acoes (o "Pago" e o portao moram no Financeiro). Evento de **aprovacao** (`contract_approval_due`) = **navegacao PURA** → `<Link href="/contratos?tab=aprovacoes&highlight={contractId}">` (AP29 reverteu o acordeao/"Gerar aprovacao" da AP7 — a geracao mora so na sub-aba Aprovacoes; o dot laranja + o lembrete ficam). Evento de **embarque** (`contract_shipment*`, EMB26) = **navegacao PURA** → `<Link href="/contratos?tab=embarque">` (a worklist + o [Confirmar embarque] moram na sub-aba). **Os 3 tipos agora UNIFICADOS** num so branch nav-pura (`navTabForEvent` → a sub-aba dona; fallback so-rotulo p/ tipo desconhecido); o acordeao morto (`dd-events-item-head`/`-detail`/`-chevron`/`-btn`) foi REMOVIDO (AP29, 2026-07-10). Deslize horizontal de 200ms na navegacao (keyframes `dd-events-slide-*`, coberto no reduced-motion). **Fetch REAL na F1**: a prop `events` (`Record<'YYYY-MM-DD', DashboardCalendarEvent[]>`, tipo promovido pro `lib/types.ts`) e populada pelo `DashboardDesktop` via `GET /api/v1/dashboard/payment-events` da quinzena que o card emite por **`onWindowChange`**; so ADMIN+COMMERCIAL (E22/`canPay`) buscam os pagamentos. **3 feeds mesclados no `DashboardDesktop`:** pagamento (`/dashboard/payment-events`, role-gated) + aprovacao (`/dashboard/approval-events`) + embarque (`/dashboard/shipment-events`) — os 2 ultimos **auth-only** (todos os nao-PROSPECTOR). Card **desktop-only** (o `DashboardMobile` nao o renderiza).
- **Modal "Lotes pendentes"** (`.bottom-sheet.is-operations`, `OperationModal`): branco + cards brancos com sombra de borda reforcada (ver `### Variantes do BottomSheet` / `.is-operations`) — mobile + desktop. Tipo dos itens = `DashboardPendingSample` (projecao parcial, NAO `SampleSnapshot`).
- **CSS orfao legado: LIMPO na revisao DSH (2026-07-07).** O cluster de classes mortas do dashboard antigo (`.dashboard-secondary-grid`, `-section-column*/-link/-subtitle`, `-secondary-panel*`, `-operations-panel`, `-search-section`, `-op-print/-progress*`, `-mobile-hero*/-welcome*`, `-total-today*`, `-view-all-link`, `-action-link*/-icon/-label`, `-empty-state`, `-muted-text`, skeleton `-md/-lg/-full/-xs`+`circle`/`content`, `.sales-total-number`) foi removido classe-a-classe com grep. Continuam VIVAS: `.dashboard-section-heading/-title`, `.dashboard-operations-grid` (PROSPECTOR), `.dashboard-skeleton-line`/`-sm`/`-card`/`-icon-wrap`. A regra segue valendo pra limpezas futuras: **so classe-a-classe com grep antes** — delecao por faixa de linha QUEBRA o prospector. Novidade da mesma revisao: `.dashboard-error-banner` (banner de erro de carregamento, ver skill `feedback-messages` §4).
...
- A rota `/informe` virou a pagina **"Relatorios"** que adapta o conteudo ao papel (`app/informe/page.tsx`); `/resumo` redireciona pra ela (server-side). Guard `INFORME_ROLES` = ADMIN/COMMERCIAL — exatamente os dois branches abaixo, sem terceiro caso. Branches:
  - **ADMIN** (`isVisitReportViewer`, unico viewer) → `components/informe/RelatoriosViewer.tsx`: feed de TODOS (`scope=all`, 3 tipos) + curadoria de vinculo, shell `.sdv-page`/`.rsm-content` (ver §1 item 5). Tambem cria — recebe `canCreate` e renderiza o `InformeCreateFab` ancorado em `.rsm-fab-anchor` (vars do leque fora da `.informe-commercial-page`).
  - **COMMERCIAL** → `components/informe/InformeCommercialPage.tsx` com o shell da `/samples` (`.samples-page-v2.informe-commercial-page` + header + `.hero-search-wrap.is-informe` SEM barra de busca — so um respiro verde — + `.samples-page-v2-sheet`): feed dos PROPRIOS (`scope=mine`) + FAB de criar.
  - **CLASSIFIER, CADASTRO e REGISTRATION** NAO acessam: o item Relatorios (`/informe`) e **filtrado** da tabbar deles, que recebe **Perfil** no 5o slot (5 itens, como os demais). CADASTRO saiu em 2026-06-28; REGISTRATION em 2026-07-10 (tinha acesso a um placeholder vazio, hoje removido junto com o `.informe-placeholder`). _(A pagina **/metrics** e a logica `isMetricsNavRole` foram **REMOVIDAS** em 2026-06-23 — era so um placeholder "Em construcao"; nao ha mais item Metricas em navbar/menu/sidebar.)_
...
- O FAB de criar (Visita comercial + Relatorio semanal) foi extraido pra `components/informe/InformeCreateFab.tsx`, reusado por COMMERCIAL e ADMIN.
