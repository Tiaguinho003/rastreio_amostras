...
- **Grid principal — `.dd-content-grid`** (DSB-D3, 2026-07-12: agora **DUAS LINHAS**, `flex: 1`; o shell-lock de 100vh continua, sem scroll de pagina): **TOP ROW `.dd-top-row`** (2 colunas `1fr/1fr`) = "Lotes disponiveis" (`SalesAvailabilityCard` `compact`) + **"Ultimos envios"** (`RecentSendsCard`, DSH-D5) **lado a lado**; **EMBAIXO** = card **"Eventos"** (`EventsCalendarCard`) ocupando a **largura toda** (horizontal), mais alto que a top row (`grid-template-rows: 0.85fr 1.15fr`). _(Antes de DSB-D3 eram 2 colunas: `.dd-left-col`/`.dd-left-stack` empilhava donut sobre envios e o Eventos ficava vertical na coluna direita; a linha de StatCards `.dd-summary-row` saiu em DSB-D2.)_ Animacoes de entrada nos FILHOS (top-row 0.2s, eventos 0.3s). Sombra de todos os cards via var **`--dd-card-shadow`** em `.dashboard-page` (tunavel num lugar so).
