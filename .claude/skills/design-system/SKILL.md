...
- **Card "Eventos"** (`components/dashboard/EventsCalendarCard.tsx`, desktop-only; detalhe em `docs/Dashboard-Visao-Geral.md` §7.3): **DSB-D4/D7 (2026-07-12)** — de 2 semanas para **1 SEMANA de DIAS ÚTEIS (seg–sex, 5 células)**. A janela BUSCADA segue dom–sáb (7 dias); o backend rola os eventos de fim de semana pro dia útil vizinho (`rollWeekendToWeekday`, sáb→sex/dom→seg) e `buildBusinessDays` filtra os 5 renderizados (helpers date-only BRT em `lib/dashboard-calendar.ts`: `buildWeek`/`buildBusinessDays`/`computeWeekStart`/`CALENDAR_WEEK_DAYS` — NAO unificar com o weekly-report segunda-based). Header = titulo + rotulo do periodo ("6 – 12 de julho") + nav ◀ `Hoje` ▶ (passo **7 dias**, passado/futuro livres; deslize horizontal 200ms `dd-events-slide-*`, coberto no reduced-motion). Grade `.dd-events-grid` = **5 colunas × 1 linha** (dias úteis, DSB-D7) de celulas **altas** que preenchem a altura do card. Cada celula (`.dd-events-day`, agora um **container `<div>`**, nao mais botao selecionavel) = numero no topo (`.dd-events-day-number`; "1 ago" na virada de mes) + **`.dd-events-day-list`** (`flex:1; overflow-y:auto` — **rola POR DENTRO** quando o dia tem muitos eventos). **Sem painel** de dia selecionado e **sem dots**. Cada evento = **`.dd-events-chip`** (Link ou span): borda-esquerda colorida por tipo (var `--chip-color`) + rotulo truncado; **navegacao PURA** → a sub-aba dona via `navTabForEvent` (`/contratos?tab=financeiro|embarque&highlight={contractId}`); tipo desconhecido = chip so-rotulo (span). **Catalogo de cores `.dd-events-chip[data-type]`** (mesmo dos antigos dots): âmbar `#eab308` (`contract_payment_due`) / verde `#15803d` (`contract_payment_paid`) / vermelho `#dc2626` (`contract_payment_overdue`) / azul `#2563eb` (`contract_shipment`) / azul-escuro `#1e40af` (`contract_shipment_done`) / vermelho `#dc2626` (`contract_shipment_overdue`). Hoje = anel `inset var(--brand-green)` + fundo `#f2f8f4` + `aria-current="date"`; **fins de semana OCULTOS** (DSB-D7 — só seg–sex; sáb/dom rolam pro dia útil vizinho no backend). **2 feeds mesclados no `DashboardDesktop`** via `onWindowChange` (agora janela de **7 dias**): pagamento (`/dashboard/payment-events`, ADMIN+COMMERCIAL/`canPay`) + embarque (`/dashboard/shipment-events`, **auth-only**). _(O feed de **aprovacao** — lembrete "a enviar", `contract_approval_due` — foi **REMOVIDO** em DSB-D9/2026-07-12: a data era imprecisa e fazia fan-out. A aba Aprovacoes e o card "Aprovacoes enviadas" seguem.)_ A prop `events` = `Record<'YYYY-MM-DD', DashboardCalendarEvent[]>`. Card **desktop-only** (o `DashboardMobile` nao o renderiza).
