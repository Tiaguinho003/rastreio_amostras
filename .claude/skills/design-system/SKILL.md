...
- **Scroll simples da pagina** (so o admin mobile): hero (saudacao + avatar), busca e sheet vivem TODOS dentro do `.dashboard-scroll` (`flex: 1; min-height: 0; overflow-y: auto`, momentum + `overscroll-behavior: contain`) e **rolam juntos** — nada fica fixo no topo. A busca e o sheet ficam no fluxo normal (`.dashboard-hero-search` sem `sticky`; `.dashboard-sheet` `overflow: visible; flex: 1 0 auto` pra preencher a tela quando o conteudo e curto). O `.dashboard-page` continua `overflow: hidden; height: 100%` (so o `.dashboard-scroll` rola). _(Substituiu o antigo efeito "cobrir a busca" — busca sticky + sheet por cima + recorte arredondado do scroll —, removido a pedido do usuario em 2026-06-15.)_
- **Cards de pendencias** (sem heading "Operacoes"): labels **"Lotes" / "Clientes"** (dados seguem sendo classificacao pendente e cadastros incompletos); card branco flat radius 18px; icon-wrap rounded-square `#e8f1ec` com glifo SOLIDO `#1f5d43` (fenda do grao em stroke claro `#e8f1ec` inline no markup); titulo `#1a1a1a` w700 + subtitulo `#8a8f8c` empilhados (sem divider/chevron). Numero em CHIP verde `#e8f1ec`/`#1f5d43` **flutuando no canto superior direito** (`position: absolute`, `top` negativo popando pra fora da borda de cima, anel branco 2px + sombra suave), ancorado no card (`.dashboard-operation-card` e `position: relative`) — sai do fluxo pra o titulo+subtitulo ocuparem a largura TODA do card sem cortar "Pendentes" (era CHIP inline estatico que comprimia o texto). Sem pulse, some quando 0. Identidade verde uniforme nos 2 cards (decisao do usuario — sai o ambar semantico)
