...
- **Linha 2 — `.dd-content-grid`** (cols 1fr/1fr, `flex: 1` — preenche a altura restante da viewport; o shell-lock de 100vh continua, sem scroll de pagina): pilha **`.dd-left-stack`** na coluna ESQUERDA com rows `1fr/1fr` — "Lotes disponiveis" (`SalesAvailabilityCard` `compact`: donut + legenda + botao "Ver disponiveis" full-width no rodape) em cima e **"Ultimos envios"** (`RecentSendsCard`, DSH-D5) embaixo, MESMO tamanho — e o card **"Eventos"** (`EventsCalendarCard`, DSH-D6/F0) na coluna DIREITA inteira, esticado na mesma altura da pilha. Sombra de todos os cards/containers via var **`--dd-card-shadow`** em `.dashboard-page` (tunavel num lugar so).
