...
**FAB de criar da /samples = LEQUE (speed-dial)** (`SampleCreateRadialFab`, modo `idle`, 2026-06-16): tap no "+" abre 2 opcoes circulares — **Lote** (grao de cafe) ACIMA do FAB e **Liga** A ESQUERDA — que emergem dele (classes `.fab-fan-*`; `scale 0.2→1` com `transform-origin` no lado do FAB + stagger). A pagina escurece com scrim no **tier de modal** (`.fab-fan-backdrop`, `rgba(0,0,0,0.55)` sem blur, `z: var(--z-modal-backdrop)` — ACIMA da tabbar z-70, bloqueia cliques; tap-fora fecha) e o FAB **encolhe + vira circular** com "×" (`.cv2-fab.is-expanded:not(.is-informe-fab)`, `z: var(--z-modal)`). Opcoes ancoradas nas vars `--fab-*`/`--fan-*` de `.samples-page-v2:not(.informe-commercial-page)` (alinham com o FAB sozinhas). Renomeado "Amostra"→"Lote". **NAO usa `.fab-menu-*`** (essas ficaram do `InformeCreateRadialFab`). Tap-feedback so scale; guard de `prefers-reduced-motion` (so fade).
