...
> Bottom sheets (mobile, slide de baixo) seguem outro padrao — ver `design-system` §8 "Bottom Sheet". Detalhes-como-overlay (painel lateral desktop / sheet mobile dirigido por URL, ex.: detalhe do cliente em `/cadastros?cliente=`) usam o `DetailOverlay` — ver `design-system` §8 "DetailOverlay". Esta skill cobre apenas modais centrais.
