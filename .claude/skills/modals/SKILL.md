...
> **Central SOBRE painel — a conferencia do contrato (RC-D27/D28, 2026-07-28):** o `ContractDocumentConfirmModal` (`components/contracts/ContractDocumentConfirmModal.tsx`) e a **fase 2** da emissao: o [Emitir] do `SaleContractEtapa2Modal` deixou de emitir — ele pede o PDF de previa ao servidor (`POST /sale-contracts/preview/pdf`, que reusa o `_resolveEmitData` e o `renderContractPdf` da emissao de verdade) e abre este modal; quem emite e o [Confirmar e emitir]. [Voltar] devolve o painel intacto. Vale nos 3 modos (a vista, futuro, editar). `.app-modal is-themed is-action ctr-doc-modal ctr-confirm-doc`, portal, `useFocusTrap`. ⚠️ **O backdrop leva `is-stacked` obrigatoriamente**: o painel do formulario e `stacked` na criacao a vista (600/610) e um `.app-modal-backdrop` normal fica em 400 — a conferencia abriria ATRAS dele. O `is-stacked` sobe o tier E tira o backdrop da regra "scrim nao se soma" (que apagaria justamente o escurecimento de que o documento precisa); uma regra escopada `:has(> .ctr-confirm-doc)` poe em `--z-modal-stacked + 20` pra desempatar contra o sheet, mesmo tier do `.is-scrim-none`. **Duas particularidades:** (1) usa **backdrop CHEIO**, nao `.fv-panel-scrim` — excecao registrada em `containers` §2, porque o objeto da decisao e uma folha A4 que precisa da tela inteira; (2) o PDF vem **rasterizado no cliente** por `pdfjs-dist` (import dinamico → `<canvas>` → `<img>` por pagina em `.ctr-doc-pages`/`.ctr-doc-page`), nao num `<iframe>` como a previa do Espelho (`EspelhoCorretagemModal`, `.ctr-doc-frame`) — o iframe de PDF nao renderiza em parte dos aparelhos, e ali o contorno e o `.ctr-doc-hint` ("use Exportar ou Baixar"), que numa tela de CONFIRMACAO nao serve. O scroll e das paginas, nao do modal: o rodape com Voltar/Confirmar fica sempre visivel. **Se a rasterizacao falhar** (aparelho antigo, worker bloqueado), o [Confirmar] fica DESABILITADO e o estado de erro oferece [Baixar o PDF para conferir] — confirmar sem ter visto o documento derrota a razao da tela; o download reabilita. Numero **provisorio** enquanto a alocacao real nao acontece (so na tx sob advisory lock) — rotulado no subtitulo e num `.ctr-doc-hint`.
