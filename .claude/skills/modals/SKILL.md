...
> **Central SOBRE central (portao do embarque, EMB28, 2026-07-10):** o `ShipmentConfirmationModal` (`components/contracts/ShipmentConfirmationModal.tsx` — confirmacao de embarque: resumo + data max-hoje + upload opcional 0..10 fotos + aviso terminal; `.app-modal is-themed is-action sample-detail-compact-modal`, portal) e alcancado por 2 portas: a worklist da sub-aba Embarque E o **portao do pagamento** — no `SaleContractLifecycleDialog` (action='pay'), um 422 `CONTRACT_SHIPMENT_REQUIRED` (lido de `ApiError.details.code`) renderiza o modal de confirmacao **por cima** do dialog de pagar (2 `.app-modal` portalados a `document.body`; o renderizado depois pinta em cima). Ao confirmar, o submit e re-chamado e o pagamento passa (o confirm NAO bumpa `version`). Read-only da galeria de fotos = seção "Embarque" do `SaleContractDetailsModal` + lightbox `.emb-lightbox` (portal, z acima do BottomSheet). **Mesma tecnica no portao do faturamento (aprovacao AP18, 2026-07-10):** um 422 `CONTRACT_APPROVAL_REQUIRED` no `SaleContractLifecycleDialog` (action='invoice') abre o `ApprovalLabelModal` (novo prop `onSent`, disparado so no sucesso) POR CIMA do dialog; ao enviar, refatura no `onClose` (o envio NAO bumpa `version` → mesma `expectedVersion`). O mesmo `ApprovalLabelModal` e a porta [Gerar] da sub-aba Aprovacoes (`AprovacoesPanel`, refetch no `onSent`).
