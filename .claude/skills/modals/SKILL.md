...
> - **Edicao MINUSCULA de um item de lista** (1–2 campos, sem confirmacao) → **dropdown INLINE** ancorado no proprio card, nao painel: economiza codigo e espaco (decisao do Flavio 2026-07-21). Unico caso hoje: **editar envio** na timeline do lote (`SampleMovementsPanel`) — o `SampleSendFlow` perdeu a entrada EDIT por causa disso. O CANCELAR do mesmo card, por ser destrutivo, segue central.
...
| /samples ?lote=         | Imprimir etiqueta = painel; **editar envio = dropdown INLINE** abaixo do card da timeline                               | painel / inline                      | ✅ F3 + `53933c8`      |
