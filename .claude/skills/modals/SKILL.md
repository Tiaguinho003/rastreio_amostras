...
> **Visual de ACAO compartilhado (`.is-action`, 2026-06-18):** modificador additivo/opt-in sobre `.is-themed` que troca o header verde pelo **visual de acao** — header branco, titulo verde a esquerda, X quadrado claro (`#eef1ee`) e **backdrop escuro sem blur** (`rgba(0,0,0,.55)` via lista `:has(.is-action)`); tambem recolore `.app-modal-description` e o back-arrow `.type-modal-back` pra escuro (senao sumiriam no header claro). Especificidade `.app-modal.is-themed.is-action` (0,4,0) vence o chrome base (0,3,0) e **nao** toca no verde dos modais de AVISO/confirm. Use em modais centrais que sao FORM/confirm que o usuario opera. Ja aplicado em todos os modais do **detalhe da amostra** (`/samples/[sampleId]`): cabecalho + container "Informacoes" — **editar informacoes** (`.sample-detail-reg-edit-modal`), **enviar amostra** (`.sample-detail-lookup-modal`), **imprimir etiqueta** (`.sample-detail-print-modal`), **invalidar** (`.sample-detail-invalidate-modal`), **reverter liga** (`.blend-revert-modal`); container "Classificacao" — **detalhe da classificacao** (`.cld-modal`), **confirmar salvar**, **gerar laudo** (`.sample-detail-lookup-modal`), **safra do laudo** (`.report-harvest-select-modal`) e **reclassificar** (`.sample-detail-reclassify-modal`); container "Resumo comercial" — **cancelar movimentacao** (`.sample-detail-compact-modal`) e **cancelar envio** (`.sample-detail-compact-modal`) _(o painel comercial do detalhe e SO LEITURA desde a migracao pra lista: **venda/perda/novo envio** e **atribuir dono a liga** rodam pelo card de `/samples`, nao pelo detalhe — o `SampleMovementModal` e as classes `.sample-detail-movement*` foram DELETADOS em 2026-07-21)_; **avisos reativos** — **propagacao de safra/proprietario** (`.blend-harvest-propagation-modal`) e **invalidacao bloqueada por liga** (`.sample-invalidate-blocked-modal`). Tambem aplicado no **fluxo de classificacao por foto (CameraSheet global)**: os modais `Classification*` (erros/avisos `*Mismatch`/`*Failed`/`NotFound`/`StatusInvalid`/`ExtractionError`/`ManualConfirm`, os de acao `Type`/`Classifier` com **back-arrow**) — o `Success` foi DELETADO em 2026-07-22: o sucesso da classificacao virou o **check canonico `SuccessCheckOverlay`** (`fixed`, portalado no body), sem card do lote nem "Ver detalhes"; o `SampleLookupResultModal` (`.app-modal-lookup-result`) tambem e `.is-action` (era precursor manual; adotou a classe + ESC interno no ciclo CAM 2026-07-16). 🪦 **A "Etiqueta enviada" NAO EXISTE MAIS (RC-D127, 2026-07-30).** Era o ultimo consumidor do check antigo `.sample-created-*` — e ele saiu junto: a etiqueta deixou de ser modal e virou o conteudo da **aba Aprovacao** do detalhe do contrato (`ApprovalLabelForm`). **A licao vale para o proximo caso:** o check central existia para dizer "acabou, pode fechar" — e numa ABA nao ha o que fechar. Sucesso em conteudo que fica na tela e **toast**, nao overlay terminal; overlay terminal pressupoe uma superficie saindo de cena. **Com isso o detalhe da amostra E o fluxo `/camera` nao usam mais header verde em NENHUM modal.** O verde `.is-themed` segue como padrao de aviso/confirm nas telas ainda nao migradas (ex.: `/users` e o resto de `/clients`). **2026-06-19, editar/filial/conta supersedidos no redesign FV de `/cadastros` (RD16):** no detalhe do cliente (`ClientDetailView` no overlay de `/cadastros`), o modal de **status do cliente** (inativar/reativar, com cascata) segue **central `.is-action`** (`.client-detail-status-actions`/`-status-msg` vivos). Ja **editar informacoes** virou a sub-pagina em FLUXO `.fv-cd-editor` e **filial/conta** (criar e ver+editar) + **preview de anexo** viraram **side-sheets STACKED** (`.client-panel-sheet`/`.fv-cd-editor-sheet` — ver skill `containers`). As classes de molde CENTRAL desses dialogos (`.client-detail-edit-modal`, `.cudm-modal`, `.client-unit-modal`, `.cap-modal`) foram REMOVIDAS na consolidacao; o conteudo interno (`.cudm-info-grid`, `.cudm-header-eyebrow`, `.client-unit-modal-body`, `.fv-cd-editor-*`, `.cap-body/-footer/-preview`) segue vivo dentro do side-sheet. Os containers brancos (`.sdv-info-compact`) de **Informacoes**, **Filiais** e **Endereco fiscal** (PJ) acompanham. O restante de `/clients` (cascade, status de filial) segue verde. Excecoes no detalhe (NAO migram, nao sao dialog): os overlays de EFEITO — X de invalidacao (`.sdv-x-effect`) — e o `PhotoZoomViewer` (foto full-screen). _(O carimbo `.sdv-stamp` foi deletado em 2026-07-21 com o `SampleMovementModal` e VOLTOU em 2026-07-22 como variante `stamp-loss` do `SuccessCheckOverlay` — agora dentro do veu branco do overlay, sem o `.sdv-stamp-overlay` proprio.)_ _(O precursor escopado `.samples-filter-modal` foi tocado e **morreu**: filtro de lista virou painel lateral — ver a lápide no §11.)_ CSS em `globals.css`, secao "Modais CENTRAIS de ACAO".
...
> 🪦 **Central SOBRE central — os dois portoes MORRERAM (RC-D65/RC-D66, 2026-07-28) e os dois modais foram apagados** (`ShipmentConfirmationModal` na RC-D65, `ApprovalLabelModal` na RC-D127). Nao ha mais nenhum caso de central-sobre-central no projeto. **O que fica da licao:** quando um dialogo precisa abrir outro, o segundo **substitui** o primeiro em cena (`{!needsX ? createPortal(...) : null}`) — **um `.app-modal-backdrop` de cada vez**, porque dois backdrops portalados escurecem em dobro; e o estado do primeiro tem de viver no COMPONENTE, nao no portal, pra sobreviver ao hand-off. **E a licao maior, que a RC-D66 escreveu:** o portao so se justifica quando o passo seguinte e impossivel sem o anterior. Os dois daqui travavam atos de ESCRITURACAO (faturar, pagar) — e o que se fez foi apagar os atos, nao o portao. _Registro do que existia:_ o `ShipmentConfirmationModal` (`components/contracts/ShipmentConfirmationModal.tsx` — confirmacao de embarque: resumo + **transporte "Pela empresa | Por terceiros" + responsavel via `UserSelect` quando "Pela empresa" (EMB30)** + data `shippedAt` (max-hoje, default no ultimo dia util, BRT; recusa fds; EMB34) + upload opcional 0..10 fotos com **recusa >12 MiB no cliente** (EMB34) + aviso terminal + "fotos ficam 15 dias" (EMB31); `.app-modal is-themed is-action sample-detail-compact-modal`, portal) e alcancado por 2 portas: o [Confirmar embarque] da secao **Embarque do `SaleContractDetailsModal`** (RC-D25 — era a worklist da sub-aba Embarque, morta com a `/embarques`) E o **portao do pagamento** — no `SaleContractLifecycleDialog` (action='pay'), um 422 `CONTRACT_SHIPMENT_REQUIRED` (lido de `ApiError.details.code`) abre o modal de confirmacao. **⚠️ EMB33 (2026-07-16) reverteu o empilhamento:** o dialog principal **SAI DE CENA** enquanto o portao esta aberto — `{!needsShipment && !needsApproval ? createPortal(...) : null}` — **UM `.app-modal-backdrop` de cada vez, NAO dois portalados** (o backdrop duplo escurecia demais). O estado (`date`) vive no COMPONENTE, nao no portal, entao persiste no hand-off; ao confirmar, um **toast "Embarque confirmado"** + o submit re-chamado paga (o confirm NAO bumpa `version`). Read-only da galeria = seção "Embarque" do `SaleContractDetailsModal` + lightbox `.emb-lightbox` (portal, z acima do BottomSheet). **Mesma moldura no portao do faturamento (aprovacao AP18):** 422 `CONTRACT_APPROVAL_REQUIRED` (action='invoice') abre o `ApprovalLabelModal` (prop `onSent`) — **tambem com o dialog base fora de cena** (o gate `!needsShipment && !needsApproval` cobre os dois portoes); ao enviar, refatura no `onClose` (envio NAO bumpa `version` → mesma `expectedVersion`). O mesmo `ApprovalLabelModal` e a porta [Gerar etiqueta] da secao Aprovacao do `SaleContractDetailsModal` (RC-D25; o prefill vem de `getApprovalLabelPrefill`, e ao concluir o detalhe se recarrega). **Enquanto ele — ou o `ShipmentConfirmationModal` — esta aberto, o `dismissGuardRef` do `DetailOverlay` fica marcado**, senao o ESC fecharia o overlay por baixo do modal.

> 🪦 **Central SOBRE painel — a conferencia do contrato: NAO EXISTE MAIS (RC-D53, 2026-07-28).** O `ContractDocumentConfirmModal` foi **deletado**; a conferencia virou o **2o passo do painel** do formulario (`ContractDocumentStep`, sem portal, sem backdrop, sem header e sem rodape proprios) — ver `containers` §1-A. Com ela morreu a unica excecao de "backdrop CHEIO sobre painel" e as classes `.ctr-confirm-doc`. **A licao, que era o ponto do verbete:** quando um dialogo nao cabe na largura do painel que o abriu, a pergunta certa nao e qual backdrop usar — e se aquilo e mesmo um dialogo. Preencher e conferir sao dois momentos do MESMO ato. _(O que continua valendo e esta documentado no componente: a previa vem do servidor pelo mesmo `_resolveEmitData`/`renderContractPdf` da emissao real, o PDF e **rasterizado no cliente** com `pdfjs-dist` → `<canvas>` → `<img>` por pagina — nao `<iframe>`, que nao renderiza em parte dos aparelhos —, o [Confirmar] so libera depois que o documento apareceu ou foi baixado, e o numero e **provisorio** ate a alocacao na tx sob advisory lock.)_ ⚠️ **Atualizado na RC-D125/D130 (2026-07-30):** o `EspelhoCorretagemModal`, dono daquelas classes, tambem morreu — o espelho virou **aba** do detalhe —, e com ele `.ctr-doc-modal`, `.ctr-doc-content` e `.ctr-doc-actions`. 🪦 **`.ctr-doc-frame` foi junto na RC-D130:** `<iframe src={blobUrl}>` nao mostra o PDF, mostra o PDF **dentro do visualizador do navegador** (barra escura com zoom/girar/imprimir, miniaturas, fundo cinza) — chrome que nao se tematiza, muda entre navegadores e no iOS as vezes nao renderiza. **Nao ha mais nenhum `<iframe>` de PDF no dominio:** o contrato, cada bloco do espelho e a releitura do Historico usam as paginas **rasterizadas** do `ContractDocumentView`, as mesmas da conferencia da emissao. Sobreviveu so o `.ctr-doc-hint`, o aviso ao lado do documento. `.ctr-details-doc-frame` era o mesmo desenho com outro nome (ver `css-architecture`).
...
- Altura: teto `max-height: min(84dvh, 40rem)` no `.app-modal` base (o corpo e `height:auto` ate ai, entao ja adapta ao conteudo — o que limita e o teto). Modais orientados a DADOS que crescem com N sobem o teto pra `min(88dvh, 44rem)`: `.blend-harvest-propagation-modal`, `.sample-invalidate-blocked-modal` (RD16 M3). Subir um teto de `max-height` nunca forca altura — os confirmes curtos seguem no tamanho do conteudo.
...
| Placeholder vermelho | `.client-quick-create-sheet .app-modal-input.has-error::placeholder` (era `.cqc-input-error`) |
...
| ClassificationDataMismatchModal    | `components/samples/ClassificationDataMismatchModal.tsx`    | Sub-caminho 4 (sacas/safra)         | `is-themed` (38rem; era `is-wide`, encolhido RD16 M3) |
...
| ---------------------- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ClientQuickCreateModal | `components/clients/ClientQuickCreateModal.tsx` | **MIGRADO pra BottomSheet** (2026-06: slide de baixo + `stacked`) — NAO e mais modal central. Os CAMPOS migraram pro kit `.app-modal-field`/`.app-modal-label`/`.app-modal-input` (RD16, 2026-07-23; igual ao BrokerFormModal e aos paineis de cliente) — restam classes `client-quick-create-*` so no LAYOUT (form/body/grid/actions); "Descartar cadastro?" e o **confirm de descarte padronizado** (`.is-scrim-none` + `.is-compact`, portalado pro body) — o MESMO dialogo do "Descartar lote?" do NewSampleModal desde 2026-07-21; o overlay interno `.client-quick-create-discard-overlay` foi **removido** na consolidacao pos-device (2026-07-23). Ver o §3. **Aberto por:** NewSampleModal, SampleMovementModal, SaleContractEtapa2Modal, RelatoriosViewer (curadoria) e CommercialVisitForm (opcao "Cliente novo"). **Pegadinha:** ele tem `<form>` proprio; se o chamador tambem for um `<form>`, renderize o modal FORA dele — o BottomSheet usa portal, mas eventos de portal sobem pela arvore React e o submit do modal dispararia o `onSubmit` do chamador. |
...
- ~~`cdm-modal`~~ — **MORREU na §2.11 U3**: os três modos de `/users` (ver/editar/criar) viraram UM
  ...
  `.fv-btn` e as 9 regras restantes saíram na P3 (`deebcbf`) — o namespace `.cdm-*` não existe
  mais.**
...
> **🪦 `samples-filter-modal` ACABOU (2026-07-28, RC-D47).** Era o filtro central de `/samples` e,
> por reuso do CSS keyed por classe, de `/clients` e `/contratos`. As três páginas migraram para o
> painel lateral `.side-sheet.fv-filter-sheet` (`/samples` na RD16 M2, `/clients` na RD16 C2,
> `/contratos` por último) e o bloco CSS inteiro foi removido junto com o último consumidor. **Filtro
> de lista não abre mais em modal central** — a regra está em `containers` §1. Fica registrado porque
> o nome ainda aparece em comentários antigos: **não recriar.**

> **Perda = painel lateral; venda e edicao MORRERAM (2026-07-21):** o `SampleMovementModal` (central, ~900 linhas) servia venda a vista, edicao de movimentacao e perda. A venda migrou pra /contratos e a edicao virou dropdown inline no card da timeline; sobrou a PERDA, que e um formulario de 3 campos — e ficou sendo a unica das tres acoes do menu ⋯ da linha (enviar, imprimir, perda) que ainda abria central. Hoje e o **`SampleLossSheet`** (`.fv-panel-sheet side-sheet sample-loss-sheet`, campos no kit `.fv-form-*`, submit no footer): motivo + data + sacas, ou — numa liga — o bloco de total (perda e 100%) com a pre-validacao da cascata. Vieram junto, sem mudanca de regra, o nudge de liga sem dono e o aviso de amostra que e origem de ligas ativas. **Atribuir dono** continua central sobre o painel, com `.fv-panel-scrim` (`.sample-loss-owner-modal`). Deletados na migração: `SampleMovementModal`, `.sample-detail-movement-*`, `.smm-scroll`. O **carimbo** `.sdv-stamp*` saiu junto, mas **VOLTOU em 2026-07-22** como variante `stamp-loss` do `SuccessCheckOverlay`: a perda registrada mostra o carimbo vermelho "Perda registrada" cobrindo o painel (`success` prop, no lugar do toast do host), no tempo `SUCCESS_CHECK_MS`. O **cancelar movimentacao/envio** segue central (`.sample-detail-compact-modal`, 28rem, acoes 50/50 com `.app-modal-submit.is-danger`) — e confirm destrutivo, nao formulario. **Cancelar envio** mostra, ao confirmar, o **X vermelho** (`SuccessCheckOverlay variant="x"` "Envio cancelado") cobrindo o modal — o `.sample-cancel-send-modal` ganha `position:relative` pro overlay ancorar nele, nao no backdrop; e reversao, por isso X e nao check verde (no lugar do toast antigo). Nao confundir com o `.sdv-x-effect` full-screen do deletar/reverter, que redireciona pra lista.
...
```tsx
<section className="app-modal is-themed sample-detail-invalidate-modal">
```

E definir overrides no CSS pelo seletor combinado:

```css
.sample-detail-invalidate-modal .app-modal-header {
  /* override pontual */
}
```
