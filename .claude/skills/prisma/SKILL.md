...
- **PJ admite no maximo 1 branch ATIVA** (F7.1B): `enforce_pj_single_active_branch` + trigger `trg_enforce_pj_single_active_branch` BEFORE INSERT OR UPDATE em `client_branch` rejeita criar/reativar uma 2a branch ATIVA quando o `Client.personType = 'PJ'`. Escape valve `SET LOCAL app.allow_split_wizard = 'on'` libera a regra na tx (usada pelo wizard de consolidacao e seu undo). PF nao e afetado pela regra.
