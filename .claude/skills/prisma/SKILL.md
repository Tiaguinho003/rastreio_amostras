...
- **VisitReport** — informe de visita (formulario da pagina `/informe`). 1 row imutavel por envio, sem update/delete via API; `userId` e `createdAt` carimbados no servidor. Identificacao por `clientKind` (enum `VisitClientKind`): `EXISTING` vincula `clientId` (FK em Client, exige ACTIVE), `NEW` guarda `newClientName` (obrigatorio) + `newClientCity`/`newClientPhone`. Respostas objetivas: `farmSize` (enum `VisitFarmSize`), `interestLevel` (enum `VisitInterestLevel`), `sellsCurrently` + `sellsToWhom` (texto livre, persiste so quando `sellsCurrently=true`); observacoes opcionais por pergunta. Service em `src/visits/visit-report-service.js` (create = qualquer autenticado; list = admin-only, alimenta a pagina `/resumo`).
