...
- **VisitReport** — informe de visita (formulario EXCLUSIVO do prospector, no sheet do dashboard dele; a pagina `/informe` e placeholder de formularios por papel). 1 row por envio, sem update via API; DELETE: **ADMIN exclui qualquer** (curadoria do /resumo) e **o autor exclui o proprio** (lixeira do dashboard do prospector; alheio = 404), hard delete via `DELETE /visit-reports/:id`; `userId` e `createdAt` carimbados no servidor. Campo discursivo livre `generalNotes` (campo 5, opcional). Identificacao por `clientKind` (enum `VisitClientKind`): `EXISTING` vincula `clientId` (FK em Client, exige ACTIVE), `NEW` guarda `newClientName` (obrigatorio) + `newClientCity`/`newClientPhone`. `newClientNameNormalized` e coluna **GERADA pelo banco** (LOWER + `immutable_unaccent` de `newClientName` — nunca escrever via Prisma) para a busca acento-insensitive do GET (`search`, que tambem cobre cliente cadastrado via `client.search_normalized`). Respostas objetivas: `farmSize` (enum `VisitFarmSize`), `interestLevel` (enum `VisitInterestLevel`), `sellsCurrently` + `sellsToWhom` (texto livre, persiste so quando `sellsCurrently=true`); observacoes opcionais por pergunta. **Offline**: `capturedAt` (nullable) = hora local do preenchimento quando o envio veio da fila offline (IndexedDB em `lib/offline/`), validada como nao-futura; `createdAt` segue sendo o "recebido em" do servidor. O POST e idempotente via header `Idempotency-Key` (scope `CREATE_VISIT_REPORT` em `idempotency-helper.js`) — replay da fila nao duplica. Service em `src/visits/visit-report-service.js` (create = qualquer autenticado; list = `VISIT_REPORT_VIEWER_ROLES` — ADMIN + COMMERCIAL + CADASTRO, espelho `isVisitReportViewer` em `lib/roles.ts` — alimenta a pagina `/resumo`; PROSPECTOR lista informes de todos os autores PROSPECTOR; stats `getMyVisitReportStats` sempre do proprio ator).
