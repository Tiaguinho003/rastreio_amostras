...
export class SaleContractService {
  ...
  // 🪦 listApprovalContracts (AP25-AP28) — a worklist da sub-aba de Aprovacoes.
  // Sem consumidor desde a RC-D2 (a sub-aba morreu com a /embarques), ficou de pe
  // "esperando a RC-F3". Apagada em 2026-07-30 porque a espera nao se sustentava:
  //   1. o grupo `a_enviar` dela e a MESMA query do `getDashboardAvisos` logo abaixo
  //      (mesmo WHERE, mesma ordem) — a fila urgente ja esta no ar, no card de Avisos;
  //   2. a lista de /contratos ja tem busca, paginacao keyset e filtro no servidor
  //      (RC-D45/D117), entao o recorte que a RC-F3 quiser nasce LA;
  //   3. e nao nasce como 5o cartao: os quatro estados de `contractStateWhere` sao uma
  //      PARTICAO dos tres status (por isso somam o total), e "aprovacao a enviar"
  //      cruza os estados em vez de particiona-los — e outro controle, nao mais um.
  // Foram junto: a rota GET /sale-contracts/approvals, o handler, o `listApprovals` do
  // api-client, os 4 tipos e os 5 testes de integracao.
