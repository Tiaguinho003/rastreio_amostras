...
export class EventContractService {
  ...
  appendEvent(eventDraft, options = {}) {
    ...
    if (hasIdempotency) {
      const idKey = idempotencyCompositeKey(event.sampleId, event.idempotencyScope, event.idempotencyKey);
    ...
    return this.store.transaction((tx) => {
      ...
      if (sample && mutatesSample) {
        ...
        if (sample.version !== expectedVersion) {
          throw new HttpError(409, `Version conflict. expected=${expectedVersion} current=${sample.version}`);
        }
      }
