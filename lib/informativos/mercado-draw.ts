 ...
 * drawTrackedText, senao a seta da variacao sai fora do lugar. */
...
/** Carrega o lockup branco. O decode() e obrigatorio: sem ele o primeiro
 * desenho sai sem o logo (a imagem ainda nao decodificou). */
...
export function drawMercado(
  ...
  const layout = buildMercadoLayout(data, measure);

  ctx.clearRect(0, 0, layout.width, layout.height);
  for (const op of layout.ops) {
    drawOp(ctx, op, family, measure, logo);
  }
  return layout;
}
