...
18. `docs/Informativos-Plano-de-Trabalho.md`
    Uso: documento centralizado da familia de features Informativos — pecas de imagem que o usuario gera preenchendo campos num modal e baixa pronta para publicar nas redes sociais (nada e salvo: sem banco, sem storage, sem historico). Conceito, principios, especificacao visual completa do Informativo de Mercado (1080x1920, story 9:16, zona segura, paleta, tipografia, medidas bloco a bloco), ledger de decisoes e fases. Prefixo de decisoes: INF. Status: IMPLEMENTADO em 2026-07-16 (INF1-INF43, F1-F4) — motor de desenho (layout puro + canvas) e modal de 2 etapas na 3a opcao do leque do FAB da /relatorios; sem migration e sem rota de API; gates verdes (unit 476/476). Pendentes: `npm run build` e a validacao visual (🖥️ peca vs. mockup, 📱 story de teste, unico jeito de confirmar a zona segura). Abertas: Q-C3 (saca em alta resolucao) e Q-F1 (2o tipo de informativo).
