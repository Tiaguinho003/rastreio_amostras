...
> _**Atualização 2026-07-09 — Central de Contratos (F1):** as páginas **Contratos** e **Financeiro** descritas aqui agora são **sub-abas** do hub único em `/contratos` (a rota `/financeiro` **redireciona** para `/contratos?tab=financeiro`; a navegação virou um item só "Contratos", gated ADMIN + COMMERCIAL). A **casca** (página, abas, URL, navegação e acesso por aba) é responsabilidade de **`Central-de-Contratos-Plano-de-Trabalho.md`**; este doc segue dono da **lógica interna** de Contratos e Financeiro._
...
| D139 | **Anexo vinculado à filial (2026-07-09)**                                                       | `ClientAttachment.unitId` **anulável** →`ClientUnit` (`NULL` = anexo do próprio cliente; só PF tem filial). O upload continua **sem vínculo**; vincular é um 2º passo no **modal de preview do anexo** (select só com filiais **ATIVAS**), com **auto-save sem confirmação**. O vínculo é **DEFINITIVO** — não troca de filial nem volta a ser do cliente (`409 CLIENT_ATTACHMENT_ALREADY_LINKED`); corrigir engano = excluir o anexo e subir de novo. **Não move o arquivo** (`storagePath` intacto). Endpoint `PATCH /api/v1/clients/:clientId/attachments/:attachmentId {unitId}` (404 `CLIENT_ATTACHMENT_NOT_FOUND` / 404 `CLIENT_UNIT_NOT_FOUND` — cobre PJ, sem filial / 422 `CLIENT_UNIT_INACTIVE`). A **view expõe só `{id,name,status}`** da filial (select aninhado, nunca `include`). Na grade de anexos (compartilhada entre o card lateral e a aba "Anexos" do modal "Documentos"), **chip com o nome da filial**; esmaecido se ela estiver inativa. **Sem sub-aba de anexos no modal da fazenda** (ideia descartada). Migration `20260709150000_client_attachment_unit_link`.                                                                                                                                                                                                                                                                              |
...
- **`ClientAttachment`** (D27; molde `SampleAttachment`; **independente do contrato**) — `id` uuid · `clientId`→Client · **`unitId?`→ClientUnit** · `storagePath` · `mimeType?` · `sizeBytes?` · `checksumSha256?` · `description?` · `uploadedByUserId?`→User · `createdAt`. `@@index([clientId, createdAt])` + `@@index([unitId])`. Exige +`application/pdf` no allowlist + CLAUDE.md#5/SECURITY. **`unitId` (D139)**: filial/fazenda dona do anexo, anulável (`NULL` = anexo do próprio cliente); só PF tem filial. Vínculo **definitivo** (`PATCH .../attachments/:id` com `unitId`; 409 `CLIENT_ATTACHMENT_ALREADY_LINKED` se já vinculado) e **não move o arquivo** — `storagePath` continua em `clients/<clientId>/attachments/`.
