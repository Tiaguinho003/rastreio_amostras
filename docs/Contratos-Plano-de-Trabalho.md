...
- Os **lotes da etiqueta são EXTERNOS** (não o lote interno do sistema) → **digitados à mão**; os outros 5
  ...
  sacas=`quantitySacks` — o mapeamento fino de produtor/armazém fica pra confirmar).
...
- **A etiqueta avulsa atual (`/samples`) é MANTIDA por ora.**

**Perguntas abertas (a decidir na Fase I):**

1. **1:1 ou 1:N** — um marco único por contrato, ou várias aprovações (várias etiquetas) auditadas?
2. O card **mostra "Aprovado"** (selo + data) ou fica **só no histórico**?
3. Os 5 campos vindos do contrato ficam **travados** ou **editáveis** na impressão?
4. O modal avulso do `/samples`: **remover / manter / substituir** quando a versão do contrato existir?
5. **Quem pode aprovar** — ADMIN (default) ou outro papel?
...
- **Fase I — Aprovação do contrato (etiqueta + auditoria).** _(Nova — S73; **só análise**.)_ Tornar a
  "Etiqueta de Aprovação" um **marco pós-emissão auditado no contrato** (**D107**): ação no card do contrato →
  etiqueta pré-preenchida do contrato + **lotes EXTERNOS manuais** → imprime (**reusa `customPrintJob`**) +
  grava `SaleContractApproval` (ator + quando + payload), no molde `Export`/`AgioLog`. **Não é status**
  (ortogonal). A **etiqueta avulsa atual do `/samples` é mantida** por ora. Restam **Q1–Q5** (ver seção
  "Aprovação do contrato"). _(Implementação em sessão futura.)_
