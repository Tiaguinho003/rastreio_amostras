...
**Próximo passo**: últimas perguntas de **funcionamento** (conteúdo do histórico no Detalhes, resultado da
impressão, selo de status no seletor, nº de cópias, validação do formulário) → depois, plano de
...
- **Fase I — Aprovação do contrato (etiqueta + auditoria).** _(S73 análise; S75 **fluxo fechado** — só
  ...
  **prefill montado no backend**. **Fluxo de UI fechado (S77 → D117)**: seletor só-elegíveis, mais recente
  ...
  sheet**; botão do card = **"Aprovação"**. **Falta antes de implementar: últimas perguntas de
  funcionamento** (histórico, resultado da impressão, cópias, validação). _(Implementação em sessão
  futura.)_
