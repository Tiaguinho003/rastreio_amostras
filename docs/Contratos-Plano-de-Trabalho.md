...
- **Q3 — editáveis**: os 5 campos vindos do contrato abrem **pré-preenchidos e EDITÁVEIS**; os **lotes** são
  ...
  vírgula) — a quebra fica pra **análise campo a campo**. Sem `originLot` (Futuro sem amostra; liga =
...
**Próximo passo (antes de implementar): análise CAMPO A CAMPO** — como cada campo puxa do contrato/amostra:
a quebra do `originLot` nos campos discretos de lote; **qual dos DOIS armazéns** do contrato
(comprador/vendedor, ambos opcionais) preenche o campo único "Armazém"; **"Produtor"** = qual nome do
snapshot do vendedor (e no PF com filial?); `purchaseNumber` **opcional** (Nº compra pode vir vazio).
...
- **Fase I — Aprovação do contrato (etiqueta + auditoria).** _(S73 análise; S75 **fluxo fechado** — só
  ...
  **`ApprovalLabelLog`**). **Não é status** (ortogonal); **sem selo** — só histórico (timeline do Detalhes,
  D108). **Falta antes de implementar: análise campo a campo** (quebra do `originLot`; qual dos 2 armazéns;
  nome do produtor; `purchaseNumber` vazio). _(Implementação em sessão futura.)_
