...
**Próximo passo**: detalhar o **fluxo de UI das duas portas** — a partir da página de amostras
(`/samples`) e a partir da página de contratos (`/contratos`).
...
- **Fase I — Aprovação do contrato (etiqueta + auditoria).** _(S73 análise; S75 **fluxo fechado** — só
  ...
  **prefill montado no backend**. **Falta antes de implementar: detalhar o fluxo de UI das duas portas**
  (`/samples` e `/contratos`). _(Implementação em sessão futura.)_
