...
| D87  | **Ágio/deságio pós-`CONFIRMADO` (botões no card)**                                              | Além do campo no modal de criação (aplicável até `CONFERIR`, via "Editar"), o card de um contrato **`CONFIRMADO`** ganha **botões "Ágio"/"Deságio"** que aplicam o ajuste e **recalculam o contrato** (ação dedicada `applyAgioSaleContract`). Facilita o ajuste de pesagem/entrega, que costuma ser conhecido **após** a confirmação.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| D98  | **Envio do contrato por e-mail (Fase H — desenho)**                                             | Nova função: anexar o PDF do contrato e **enviar por e-mail** ao cliente, direto do app. **Reusa a infra que já existe**: `nodemailer` + `AppEmailService` (`src/email/create-email-service.js`; SMTP em prod / outbox em dev) + o `renderContractPdf` que **já devolve um `Buffer`** on-demand (D32) — anexa direto, sem regenerar/armazenar. **Escopo inicial = só enviar o documento** (assunto/corpo editáveis, histórico no card e envio do Espelho = adiados). _(Desenho na S71; implementação na Fase H, sessão futura.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| D99  | **Remetente configurável — "decidir depois"**                                                   | O app envia por **SMTP configurável** (já suportado), então o remetente é **só config** (host/port/user/pass/from) e **não trava a construção** (dev usa outbox). **Alvo preferido**: **caixa dedicada no domínio da empresa** (`contratos@empresa`) — o admin habilita "SMTP autenticado" + passa uma **senha de app**; plugar = 5 campos, zero código. **Fallback**: endereço que o app controla (Gmail atual/novo) com **display "SAFRAS"** + **Reply-To** pro e-mail real. **Gargalo real**: o e-mail da empresa é **de terceiros** (não controlam a senha) → depende do admin; Microsoft 365 vem restringindo SMTP-básico; **deliverability** (SPF/DKIM/DMARC) se o "De" não bater com o domínio.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| D100 | **Destinatários: só digitados na hora**                                                         | Campo livre, **múltiplos**, **sem** pré-preencher do cadastro (o `Client.email` existe, `schema.prisma:575`, mas fica de fora por ora). Validação de formato + exige ≥1. O modal **mostra os destinatários + o nº do contrato** antes de enviar (mitiga envio ao errado, já que não há pré-preenchimento).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| D101 | **Disponível em 2 pontos**                                                                      | (a) **Modal em sequência** logo após gerar o contrato (o fluxo pedido); (b) **botão "Enviar por e-mail" no card** de qualquer contrato `EMITIDO+` (reenvio / enviar contratos já criados). Mesmo endpoint + mesmo modal de composição. Exige **plumbar o novo `contractId`** do create de volta ao pai (hoje `onSaved()` não devolve o id — `SaleContractEtapa2Modal.tsx:539`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| D102 | **Registro de envio: `SaleContractEmailLog`**                                                   | Nova tabela (migration) espelhando `SaleContractExport` (`schema.prisma:886`) + `recipients` (lista), `subject`, `status` (`SENT`/`FAILED`), `messageId`/`error`, `sentByUserId`, `sentAt`. Gravada **após cada tentativa** (sucesso E falha). Habilita "histórico de envios" no card depois.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| D103 | **Envio SÍNCRONO por endpoint dedicado**                                                        | `POST /sale-contracts/[id]/send-email` (gate **ADMIN**, `SALE_CONTRACT_MANAGE_ROLES`): carrega o contrato → gera o PDF (Buffer) → envia → grava o log → devolve **sucesso/erro** ao modal. **NÃO** o padrão inline-transação dos e-mails atuais (que dão rollback do banco em falha de SMTP) — aqui é ação **isolada, pós-contrato, com feedback imediato**. Botão **desabilita durante o envio** (anti-duplo-clique).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
## Envio do contrato por e-mail (Fase H — desenho, S71)

**Desenho na Sessão 71 (2026-07-02). Só desenho — implementação = Fase H (sessão futura). Decisões D98–D103.**

Ao emitir um contrato, poder **enviar o PDF ao cliente por e-mail** direto do app: um **modal em
sequência** após gerar o contrato (e um **botão no card** para reenvio / contratos já criados) →
digitar os destinatários → **Enviar** → o PDF vai **anexado**, pelo e-mail da empresa. Escopo
inicial = **só enviar o documento**.

**Quase tudo já existe** (é extensão, não construção do zero): o app **já envia e-mail em
produção** (`nodemailer` + `AppEmailService`, `src/email/create-email-service.js`; SMTP em prod /
outbox em dev; DI + health-check) e o **PDF já é um `Buffer`** on-demand (`renderContractPdf` →
`{ buffer }`). Conectar qualquer conta remetente = **só config SMTP** (host/port/user/pass/from),
zero código.

**Arquitetura (para a Fase H):**

- **Backend**: (1) estender os 2 transportes (`SmtpEmailService.sendMail` / `LocalOutboxEmailService.sendMail`)
  pra **repassar `attachments`** — hoje descartam; o outbox deve persistir o anexo pra teste;
  (2) novo `AppEmailService.sendSaleContractPdf(...)`; (3) `SaleContractService.sendSaleContractByEmail(contractId, {recipients}, actor)`
  (injeta `appEmailService` + `saleContractPdfService`; `getSaleContract` → `renderContractPdf` →
  envia → grava `SaleContractEmailLog`); (4) rota `POST /sale-contracts/[id]/send-email` (**ADMIN**);
  (5) migration `SaleContractEmailLog`.
- **Frontend**: (6) **plumbar o novo `contractId`** do `createSpot/createFutureSaleContract` de
  volta ao pai (pra abrir o modal de e-mail após gerar — hoje `onSaved()` não devolve o id);
  (7) modal de composição (destinatários em chips + assunto fixo; preview opcional reusando
  `downloadSaleContractPdf`); (8) botão "Enviar por e-mail" no card (`EMITIDO+`);
  (9) `api-client.sendSaleContractEmail`.

**Gargalos analisados (o ponto central do desenho):**

1. **Remetente / deliverability (o principal)** — o e-mail da empresa é **de terceiros** (não
   controlam a senha) → usar o endereço real depende do admin **habilitar SMTP autenticado +
   fornecer senha de app**; Microsoft 365 vem **restringindo** SMTP-básico (pode exigir OAuth);
   **deliverability** (SPF/DKIM/DMARC) se o "De" não bater com o domínio. **Não bloqueia a
   construção** (config no fim; dev usa outbox). Alvo = **caixa dedicada no domínio** da empresa
   / fallback controlado com **display "SAFRAS" + Reply-To** real.
2. **Anexos descartados** nos 2 transportes → ajuste pequeno e **obrigatório** (sem ele o PDF não vai).
3. **Síncrono, fora de transação** — os e-mails atuais enviam **dentro** da tx (rollback em falha),
   errado aqui; endpoint dedicado com **feedback imediato** (D103).
4. **`contractId` no create** não volta ao pai → sem isso o modal **não abre** pós-geração.
5. **Destinatário errado** (sem pré-preenchimento) → o modal confirma destinatários + nº; o **log**
   registra pra quem foi.
6. **Duplo-clique** → botão desabilita durante o envio.
7. **Outbox** precisa persistir o anexo (teste); PDF de 1 página (**<200 KB**) — sem limite.

**Extensões futuras** (fora do escopo inicial): assunto/corpo **editáveis**; **histórico de
envios** no card (lê o log); **enviar o Espelho de Corretagem** por e-mail (mesma infra). Plano
detalhado em `~/.claude/plans/functional-swimming-pizza.md`.
...
- **Modais** (seletor + formulário): **desktop = centrais** (padrão do modal de emissão); **mobile =
  bottom sheet** saindo de baixo (padrão `NewSampleModal`). _(O modal de criação de contrato, citado como
  referência, é hoje central também no mobile — débito da B.3/skill `modals`; candidato à Fase G.)_
...
- **Linha do histórico** (timeline do modal de Detalhes, D108): **"há X tempo"** (tempo relativo) + **quem
  ...
  drill-down do payload na UI (as linhas impressas ficam guardadas na auditoria, D114).
...
**Próximo passo**: definir o **escopo do histórico na Fase I** (o modal de Detalhes/D108 ainda não existe —
a Fase I constrói o timeline junto, ou grava a auditoria write-only e o timeline vem com o Detalhes?); se o
histórico também aparece em alguma visão geral fora do contrato (o nº do contrato na linha sugere isso) →
depois, plano de implementação.
...
- **Fase G — Revisão de fluxos + design das páginas.** _(Nova — S67; **1ª frente ✅ na S68**.)_ Varredura
  ...
  **responsividade** (**P27**). Reconcilia as pendências abertas da S67 (**P23–P28**) e fecha o **"validar no
  ...
  - **✅ S68 — reconciliação das pendências (P23–P26, P28)**: **D92–D95** + exceção do ágio na D25 (ver log
    ...
    device".
- **Fase H — Envio do contrato por e-mail.** _(Nova — S71; **só desenho**.)_ Anexar o PDF do contrato e
  **enviar por e-mail** ao cliente (modal em sequência pós-criação + botão no card, `EMITIDO+`). **D98–D103**;
  **reusa a infra existente** (`AppEmailService`/SMTP + `renderContractPdf` que já devolve Buffer). Passos:
  migration `SaleContractEmailLog` → transportes repassam `attachments` + `sendSaleContractPdf` →
  `sendSaleContractByEmail` + rota `POST /sale-contracts/[id]/send-email` (**ADMIN**) → frontend (plumbar o
  `contractId` do create, modal de composição, botão no card, api-client) → **config do remetente no fim**
  (só `.env`/Secret Manager, quando a conta estiver definida — não trava). Gargalo principal = **remetente/
  deliverability** (e-mail da empresa é de terceiros). Ver a seção **"Envio do contrato por e-mail"** +
  plano `~/.claude/plans/functional-swimming-pizza.md`. _(Só desenho na S71; implementação em sessão futura.)_
- **Fase I — Aprovação do contrato (etiqueta + auditoria).** _(S73 análise; S75 **fluxo fechado** — só
  ...
  **prefill montado no backend**. **Fluxo de UI fechado (S77 → D117)**: seletor só-elegíveis, mais recente
  ...
  status no seletor; 1 cópia por envio; validação "≥1 campo". **Aberto: escopo do histórico na Fase I**
  (timeline junto do modal de Detalhes/D108, ou auditoria write-only primeiro). _(Implementação em sessão
  futura.)_
...
### 2026-07-02 — Sessão 71 (Envio do contrato por e-mail — desenho)

Desenho da nova função de **enviar o PDF do contrato por e-mail** ao cliente (**D98–D103**, **Fase H**). A
pedido do Flavio, **só análise/decisões — sem código** (registrado aqui + na seção "Envio do contrato por e-mail").

- **Achado-chave (3 agentes)**: a premissa se inverteu — o app **já envia e-mail em produção** (`nodemailer` +
  `AppEmailService`, SMTP/outbox, DI, health-check, senha SMTP já no Secret Manager) e o **PDF já é um `Buffer`**
  on-demand (`renderContractPdf`). É **extensão**, não construção do zero.
- **Decisões**: envio reusa a infra (D98); **remetente configurável, "decidir depois"** — não trava a
  construção, é só config SMTP (D99); **destinatários só digitados** (D100); disponível **pós-criação + card**
  (D101); **log `SaleContractEmailLog`** (D102); **envio síncrono** por endpoint dedicado `POST /…/send-email`
  ADMIN, com feedback (D103).
- **Gargalo principal (discutido com o Flavio)**: o e-mail da empresa é **Microsoft/Outlook controlado por
  terceiros** (não trocam a senha) → usar o endereço real depende do admin **habilitar SMTP autenticado +
  senha de app**. Conectar **do nosso lado é trivial** (5 campos de config); o bloqueio é só o acesso ao
  provedor. Alvo = caixa dedicada no domínio / fallback controlado (display "SAFRAS" + Reply-To).
- **Ajustes obrigatórios mapeados** p/ a Fase H: os 2 transportes hoje **descartam anexos**; o create **não
  devolve o `contractId`** ao pai (necessário pra abrir o modal pós-geração). Plano em
  `~/.claude/plans/functional-swimming-pizza.md`. _(Implementação = Fase H, sessão futura.)_
