...
- [ ] **Etapa 4** — Execução
  ...
  - [ ] Fase Q (em andamento)
    ...
    - [ ] Q.cls.2 ficha unificada (parcial — ficha física `a79626e`, CTA "Classificar" `f505926`, tela da câmera `e37deaa`, IA `864f619`, modal de revisão `a39e305`, modal de tipo `8dbe36f`, avisos 3a/3b + modo manual `983ccc3`, modais de cross-validation/reclassify/classifier `9411ffe`, payload ficha unificada + reason persistido + sub-caminho 5 Flow B `aa7c591`+`1aa4845`+`a2c7594`+`40d91e4`+`2bc3a36`; restam tipo selecionado depois → `CLASSIFICATION_UPDATED` audit, cleanup do `TYPE_CONFIGS`, migration)
...
**Mantidos por escopo (cleanup do TYPE_CONFIGS é frente 2 desta seção):**

- `TYPE_CONFIGS`, `getTypeConfig` (lib), `CLASSIFICATION_TYPE_LABEL`, `ALL_SIEVE_FIELDS`, `SIEVE_FIELDS`, `NUMERIC_FIELDS`, `ALL_NUMERIC_FIELDS` — usados em `app/samples/[sampleId]/page.tsx` (renderização condicional por tipo).
- `peneiraP19` e `safra` no `ClassificationFormState` — referenciados em `renderVal` na detail page (sempre vazios após Q.cls.2.7, mas remoção do tipo quebra os usos).
- Params `classificationType` em `extractFromDetectedForm`/`extractAndPrepareClassification`/`buildClassificationDataPayload`/`validateClassificationForm` — opcional, ignorado, mantido por compat.
