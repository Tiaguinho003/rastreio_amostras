...
- [x] **Impressão (`498e74b`)**: `requestQrPrint`:
  ...
  - **antes** de criar: executa lazy timeout em `expireStalePrintJobs` (60s, status EXPIRED) ✓
  - **bloqueia (409)** se já houver PENDING válido pra essa amostra ✓
  - **remove** parâmetro `printAction` da assinatura ✓
- [x] **Impressão (`498e74b`)**: `requestQrReprint` deletado.
- [x] **Impressão (`498e74b`)**: `recordQrPrinted` atualiza `PrintJob` pra SUCCESS, emite `QR_PRINTED` audit (null/null), removido o hack "se já passou de QR_PENDING_PRINT", sem `expectedVersion`.
- [x] **Impressão (`498e74b`)**: `recordQrPrintFailed` atualiza `PrintJob` pra FAILED com `error`, emite `QR_PRINT_FAILED` audit (null/null), sem `expectedVersion`.
...
- [x] **Sem código morto**: sweep formal feito via `tsc --noUnusedLocals --noUnusedParameters` na detail page — 30+ refs órfãs removidas (imports `ClassificationDataPayload`/`completeClassification`/`uploadClassificationPhoto`/`compressImage`/`mapExtractionToForm`; helpers `formatDateInputLabel`/`buildMismatchMessage`/`isClassificationStatus`/`getOperationalStatusDot*`/`getCommercialStatusDot*`; states `classificationNotice`/`classificationCompleting`/`classificationPhotoUploading`/`fromQrSource`/`labelModalPrintAction`/`classificationShowsWorkspace`/etc; funções inteiras `handleUploadClassificationPhoto`/`handleCompleteClassification`/`startClassificationEdit`/`cancelClassificationEdit`/`handleRequestClassificationUpdate`/`renderClassificationInputField`/`renderClassificationTextareaField`/`handleClassificationPhotoSelected`/`clearClassificationSelectedPhoto`/`updateClassificationField`/`getClassificationFieldState`). Detail page reduzida de 4197 → 3758 linhas. Plus: `submitLabel` removido de `SampleSearchField` (e callers); `loadingUnits` em `SampleMovementModal` virou destructure-only-setter. Q.draft cobriu `classification_draft_*`.

##### Q.7.6. Frontend — modal de feedback do print (auto + manual)

- [x] **Q.auto**: a câmera (`app/camera/page.tsx`) já redireciona pra detail page automaticamente após sucesso (auto-redirect 2s pós `flowState='success'` — comportamento anterior à Q.auto). Auto-print roda síncrono no `completeClassification` antes da response, então quando o frontend volta pra detail, o `latestPrintJob` PENDING já existe e o polling pega.
- [x] **Polling (`3584b73`)**: detail page detecta `PrintJob` PENDING ativo → polling a cada 3s. Para automaticamente em SUCCESS/FAILED/EXPIRED.
- [x] **Feedback Q.print (`3584b73`)**: badge na seção Etiqueta atualiza via polling (Imprimindo... → Impressa / Falhou / Tempo esgotado). Sem modal automático pós-PENDING — decisão atual é manter o feedback direto na seção pra evitar que um modal interrompa o operador. Modal automático fica pra Q.auto se necessário.
- [x] **Modal único por print (`3584b73`)**: o modal de revisão+confirmação aparece quando o user clica "Imprimir", confirma, vai pra step "completed" ("Lote N enviado pra impressao") e fecha. Não reaparece sem nova ação.
- [x] **Print manual (`3584b73`)**: mesmo modal cobre RC e CLASSIFIED — botão "Imprimir etiqueta" vira "Imprimir novamente" quando já há `latestPrintJob`.

##### Q.7.7. Frontend — dashboard

- [x] **Dashboard (`b2253bb`)**: card "Aguardando impressão" deletado em `DashboardDesktop.tsx` + `DashboardMobile.tsx`. `useOperationModal.ts` perdeu `print_pending` panel. Sem volta.
- [x] **Card "Aguardando classificação" (`b2253bb`)**: query continua, conta amostras em `RC`. (`classificationInProgress.total` removido — agora só `classificationPending.total`).
...
- [x] **Tests Q.print (`498e74b`)**: helpers `moveSampleToQrPendingPrint` + `moveSampleToQrPrinted` deletados em `sprint1.integration.test.js` e `backend-api-v1-missing.integration.test.js`. Calls migrados pra `moveSampleToRegistrationConfirmed`. `expectedVersion` ajustado em ~25 lugares (3 → 1, 4 → 2, 5 → 3, 6 → 4) — refletindo que QR_PRINT_REQUESTED/QR_PRINTED não mutam mais. Tests REPRINT (5 em api-v1-missing + 2 em event-contract-db) deletados ou substituídos por novos do fluxo Q.print (1 cobre idempotency+block-INVALIDATED+409 PENDING; outro cobre audit-only). `qrReprintRequestedEvent` builder deletado de `helpers/event-builders.js`.
- [x] `event-contract.test.js` (`498e74b`): `MUTATING_EVENT_TYPES` sem QR\_\*; tests com `expectedVersion` em audit events ajustados; tests "qr reprint success" deletados.
- [x] **Casos novos cobertos (`498e74b`)**: `createSample` emite 1 evento; `requestQrPrint` rejeita INVALIDATED (409); 409 quando PrintJob PENDING já existe; audit-only não muta version/status (testado em `recordQrPrinted` e `recordQrPrintFailed`). Auto-print pós-classificação fica pra Q.auto.
...
| 4   | `refactor(samples): impressao vira acao pura — PrintJob como source of truth (Q.print P1)` | ✅       | `498e74b` — backend command/query/projeção/schemas/tests; `requestQrReprint` deletado, audit-only events, lazy timeout (`expireStalePrintJobs`), 409 em PENDING duplicado                                                                                                                                                                                                  |
| 5   | `feat(samples): impressao automatica apos completeClassification (Q.auto)`                 | ✅       | `completeClassification` dispara `requestQrPrint` best-effort com `idempotencyKey: ${event.idempotencyKey}:auto-print`. Pre-check de idempotency em `requestQrPrint` (antes do bloqueio PENDING) garante retry idempotente. 4 tests novos: PrintJob criado, key derivada dedupa, reclassificação não dispara, falha do print não bloqueia classificação                    |
| 6a  | `feat(samples): remove QR_PENDING_PRINT/QR_PRINTED do frontend (Q.print P2)`               | ✅       | `b2253bb` — api-client (requestQrReprint deletado), types (SampleStatus 3 estados, dashboard simplificado, PrintJobStatus EXPIRED), StatusBadge, dashboard cards, useOperationModal, camera, SampleMovementsPanel                                                                                                                                                          |
| 6b  | `feat(samples): secao Etiqueta no body + polling do PrintJob (Q.print P3)`                 | ✅       | `3584b73` — secao `sdv-print-section` no `sdv-general` (substitui `sdv-print-failed-card`), polling de PrintJob a cada 3s enquanto PENDING, modal step "completed" destaca lote                                                                                                                                                                                            |
| 7   | (incluído em P2)                                                                           | ✅       | dashboard sem "Aguardando impressão" entrou no commit `b2253bb`                                                                                                                                                                                                                                                                                                            |
| 8   | (incluído em P2 + P3)                                                                      | ✅       | revisão da detail page distribuída entre `b2253bb` (limpeza de tipos QR\_\*) e `3584b73` (seção Etiqueta + polling). CTA "Iniciar classificação" fica pra Q.auto                                                                                                                                                                                                           |
...
- [x] **Tipo selecionado depois → `CLASSIFICATION_UPDATED`** (audit) — commits `4e02fe1` (backend tipo-only update) + `15a5a07` (frontend tipo editável no modal de detalhe). Decisão de UX do usuário: tipo é editado **junto com a ação de editar classificação**, sem botão separado. `<select>` nativo na seção "Tipo" antes de "Identificação"; reasonText auto "Tipo alterado de X pra Y".
...
> 6 commits temáticos: `aa7c591` → `1aa4845` → `a2c7594` → `40d91e4` → `df603d9` → `2bc3a36`. Quality gates verdes em cada um (typecheck, lint, format, build, schemas, contracts 22, unit 177, integration:db 144).

**Schema do evento `CLASSIFICATION_COMPLETED.payload.classificationData`:**

- Removidos: `safra` (vive em `sample.declaredHarvest`), `peneirasPercentuais`, `p19`, `classifierUserId`, `classifierName`, `conferredBy`.
- Adicionados sub-objs/array agrupados:
  - `peneiras`: `{ p18, p17, p16, p15, p14, p13, p12, p11, p10, mk: number|null (0-100) }`
  - `fundos`: array top-level com `minItems: 2, maxItems: 2`, items `{ peneira: string|null, percentual: number|null }`
  - `defeitos`: `{ imp, pva, broca, gpi, ap, defeito: string|null }`
- Mantidos flat: `padrao, aspecto, certif, catacao, observacoes, bebida, dataClassificacao`.
- `technical` mantido (alimenta colunas Prisma `latest_type/screen/defects_count/density/notes` usadas em queries).

**Schema do evento `CLASSIFICATION_UPDATED.payload`:**

- `reasonText`: regex `1-10 palavras` → `minLength: 1, maxLength: 500`. Frontend já permite até 500 chars.
- `before`/`after` continuam genéricos (object minProperties 1) — backend valida shape via `parseClassificationUpdatePatch`.

**Backend (`src/samples/sample-command-service.js`):**

- `parseClassificationUpdatePatch` aceita os 3 sub-objs novos (top-level OU dentro de `classificationData`).
- 3 helpers especializados substituem `parseClassificationSievePatch`: `parseClassificationPeneirasPatch`, `parseClassificationFundosPatch`, `parseClassificationDefeitosPatch`.
- `confirmClassificationFromCamera` aceita `reasonCode`/`reasonText` opcionais. Default `'DATA_FIX'` / `'Reclassificacao via foto'` preserva compat com chamadas pré-Q.cls.2.7.
- `CLASSIFICATION_DATA_EDITABLE_FIELDS` reduzido aos 6 flat fields. `CLASSIFICATION_PENEIRA_KEYS` (10) e `CLASSIFICATION_DEFEITO_KEYS` (6) novos.

**Projection (`src/events/event-contract-db-service.js`):**

- `applyClassificationDataPatch` reescrito: flat fields + 3 sub-objs (peneiras/fundos/defeitos). Merge campo a campo em peneiras/defeitos; substituição inteira em fundos.
- `applyLegacyTechnicalPatch` simplificado: só `technical.notes` → `target.observacoes`. `defectsCount → target.defeito` removido (campo flat não existe mais — vive em `defeitos.defeito`).
- Blocos de leitura `classifierUserId`/`classifierName`/`conferredBy` removidos (schema rejeita; nunca alcançados).

**Frontend (`lib/classification-form.ts`, `lib/api-client.ts`, `app/camera/page.tsx`, `app/samples/[sampleId]/page.tsx`):**

- `ClassificationDataPayload` reestruturado pra estrutura agrupada. Tipos novos exportados: `ClassificationPeneirasPayload`, `ClassificationFundoEntry`, `ClassificationDefeitosPayload`. `ClassificationSievePayload` removido.
- `buildClassificationDataPayload` produz a estrutura agrupada. Sub-objs `peneiras`/`fundos`/`defeitos` ficam `null` quando nenhum campo do grupo está preenchido (payload menor).
- `validateClassificationForm` valida APENAS peneiras + percentual dos fundos como número. Os 6 campos de defeitos viraram texto livre (operador pode escrever "12", "ALTO", "0,5").
- `confirmClassificationFromCamera` no api-client + route handler aceitam `reasonCode`/`reasonText` opcionais.
- `app/camera/page.tsx`:
  - `saveClassification` passa `reclassifyReasonCode`/`reasonText` (substitui hardcode no backend).
  - **Sub-caminho 5 (reclassificação) agora funciona no Flow B**: `handleConfirmClassification` detecta `contextSampleStatus === 'CLASSIFIED'` após cross-validation OK e vai pra `overwrite-confirm` em vez de salvar direto. Antes só funcionava no Flow A.
  - `mismatchOverwriteAfter` reativa quando CLASSIFIED + data-mismatch (operador clica "Aplicar" no DataMismatchModal → vai pra ReclassifyModal).
  - `handleConfirmOverwrite` usa `resolvedSample?.id ?? mismatchTargetSampleId` pra funcionar em ambos os flows.
  - `ClassificationReclassifyModal.open` agora aceita Flow B (`!!contextSampleId`); `sampleLot` fallback `resolvedSample?.internalLotNumber ?? contextSampleLot`.
- `app/samples/[sampleId]/page.tsx`:
  - `buildClassificationFormState` lê estrutura agrupada do `latestClassificationData` (peneiras sub-obj + fundos array + defeitos sub-obj).
  - Edição rápida mantém `reasonCode='DATA_FIX'`/`reasonText='Edicao rapida'` hardcoded (decisão do plano — reclassificação completa redireciona pra câmera).

**Cleanup de dead code (commit `2bc3a36`):**

- Imports removidos: `getTypeConfig` em `app/camera/page.tsx`, `NumericField` em `app/samples/[sampleId]/page.tsx`.
- Blocos legacy removidos no `mergeClassificationData`: `classifierUserId`, `classifierName`, `conferredBy`.
- `applyLegacyTechnicalPatch` simplificado.

**Cleanup completo (commits `c18e435` + `6659bd6`):**

- Detail page: 4 condicionais por `classificationType` (BICA/PREPARADO/LOW_CAFF/fallback) substituídas por layout único da ficha unificada (peneiras 2x5 / fundos 4 cols / catação+defeitos 2x3+1 / bebida / observações).
- `TYPE_CONFIGS`, `ClassificationTypeConfig`, `getTypeConfig`, `NumericField`, `ALL_SIEVE_FIELDS`, `ALL_NUMERIC_FIELDS`, aliases `SIEVE_FIELDS`/`NUMERIC_FIELDS` — **removidos**.
- `peneiraP19` e `safra` no `ClassificationFormState` — **removidos**. Detail page usa `renderStatic('Safra', detail.sample.declared.harvest)` em vez de `renderVal('safra', ...)`.
- Params `classificationType` em `validateClassificationForm`/`buildClassificationDataPayload`/`mapExtractionToForm` — **removidos** (todos os campos válidos pra qualquer tipo).
- `EXTRACTION_FIELD_MAP` (renomeado de `UNIVERSAL_EXTRACTION_MAP`) sem chaves `safra`/`p19`.
- `parseNumberInput` deixou de ser exported (uso interno).
- `lib/classification-form.ts` reduziu ~280 linhas.

**Mantidos:**

- `CLASSIFICATION_TYPE_LABEL` — usado no título do modal de classificação na detail page.
- Params `classificationType` em `extractFromDetectedForm`/`extractAndPrepareClassification` (api-client) — opcional, ignorado, mantido por compat (consumers não passam mais).

**Gargalos verificados (sem ação necessária):**

| Gargalo                                                              | Análise                                                                                                                | Decisão               |
| -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------- |
| `technical` redundante com `defeitos.defeito` + `observacoes`        | `technical` ainda alimenta colunas Prisma `latest_type/screen/defects_count/density/notes` usadas em queries/dashboard | Manter                |
| `target.defeito` flat (legacy)                                       | Removido — eventos novos só populam `defeitos.defeito` sub-obj                                                         | Sem dor (prod zerada) |
| `mismatchTargetSampleId` no Flow B                                   | Setado pra `contextSampleId` quando CLASSIFIED + data-mismatch ou direto pra overwrite-confirm                         | Funciona              |
| `applySampleUpdates` rebuild em `handleConfirmOverwrite`             | `mismatchDivergences` + `mismatchChoices` ainda no state — `buildApplySampleUpdatesFromMismatch` reusa                 | Funciona              |
| Reset de `reclassifyReasonCode` ao cancelar                          | Não reseta — operador volta com seleção pré-existente                                                                  | Aceitável             |
| `parseInt` em `buildTechnicalFromClassificationData` com texto livre | Tolera string não-numérica (vira `undefined`)                                                                          | OK                    |
...
| 2026-05-08 | Cleanup de dead code pós-Q.cls.2.7 frente 1 (commit `2bc3a36`)                                                    | Imports não usados (`getTypeConfig` em camera/page.tsx, `NumericField` em detail page). Blocos `classifierUserId`/`classifierName`/`conferredBy` no projection (schema rejeita; nunca alcançados). `applyLegacyTechnicalPatch` simplificado: só `notes → observacoes`; `defectsCount → target.defeito` flat removido (campo não existe na ficha unificada). Cleanup completo do `TYPE_CONFIGS` fica pra próxima frente da Q.cls.2.7.                                                                                                                                             |
| 2026-05-08 | Detail page com layout unificado da ficha (commit `c18e435`)                                                      | Substitui as 4 condicionais por `classificationType` (BICA mostra P17+MK+Imp, LOW_CAFF P15..P10+AP/GPI, PREPARADO P19..P14+MK+Defeito+Imp, fallback typeConfig) por layout único: identificação 3 cols → identificação visual 3 cols → peneiras 2x5 (P18..P14 / P13..P10+MK) → fundos 4 cols → catação+defeitos 2x3+1 → bebida → observações. `renderVal('peneiraP19')` removido; `renderVal('safra')` substituído por `renderStatic('Safra', declared.harvest)`. CSS `.cld-grid-5` adicionado. Espelha o `ClassificationReviewModal`.                                           |
| 2026-05-08 | `lib/classification-form.ts` enxuto após cleanup do TYPE_CONFIGS (commit `6659bd6`)                               | -280 linhas. Removidos: `TYPE_CONFIGS`, `ClassificationTypeConfig`, `getTypeConfig`, `NumericField`, `ALL_SIEVE_FIELDS`, `ALL_NUMERIC_FIELDS`, aliases `SIEVE_FIELDS`/`NUMERIC_FIELDS`, campos `peneiraP19` e `safra` do `ClassificationFormState`. Simplificados: `validateClassificationForm`/`buildClassificationDataPayload`/`mapExtractionToForm` sem `classificationType` (todos os campos válidos pra qualquer tipo). `EXTRACTION_FIELD_MAP` (renomeado de `UNIVERSAL_EXTRACTION_MAP`) sem chaves `safra`/`p19`. `parseNumberInput` deixou de ser exported (uso interno). |
| 2026-05-08 | Tipo é editado junto com a ação de editar classificação (decisão de UX)                                           | Operador não tem botão "Mudar tipo" separado. Tipo vira mais um campo do modal de especificações da classificação na detail page, editável quando o operador clica "Editar". Reduz fricção e mantém a auditoria via `CLASSIFICATION_UPDATED` (já existia o wire backend, faltava conectar frontend).                                                                                                                                                                                                                                                                             |
| 2026-05-08 | Backend `updateClassification` aceita tipo-only update (commit `4e02fe1`)                                         | Antes, mudar só o tipo gerava 409 `'No classification changes detected'` porque `parseClassificationUpdatePatch` exigia ≥1 campo no `after`. Agora detect `typeChanged = newType !== sample.classificationType` antes de parsear; se `afterEmpty && typeChanged` monta payload `{before:{}, after:{}}` manualmente; se mudou tipo, inclui `classificationType` em `before/after` (audit completo + satisfaz schema `minProperties:1`). 3 testes integration novos.                                                                                                               |
| 2026-05-08 | Frontend tipo editável no modal de detalhe (commit `15a5a07`)                                                     | Seção "Tipo" antes de "Identificação" no modal de detalhe da classificação. View mode: badge pill verde com label. Edit mode: `<select>` nativo (BICA/PREPARADO/BAIXO=LOW_CAFF/[ESCOLHA disabled "Em breve"]). `saveClassificationDetail` detecta typeChanged e gera reasonText auto: "Tipo alterado de X pra Y" se mudou, "Edicao rapida" senão. Header do modal usa `classificationDetailType` (reflete edição em andamento). Cancelar restaura tipo original. `lib/api-client.ts updateClassification` + route handler aceitam `classificationType`.                          |
