...
- **~~A imunidade de produção da SN-D15 é acidental~~ — GUARDADA em 2026-07-30.** Ela depende de as 8 rotas continuarem **estáticas**: um `export const dynamic`, um `revalidate = 0` ou um `cookies()` no layout raiz levaria o mismatch para produção, e nada no build reclamaria. Virou gate: **`npm run check:prerender`** (`scripts/check-prerender-invariants.mjs`), rodando no CI logo depois do build. Ele afirma três coisas por rota — a caixa raiz **presente** (`fv-boot=1`, a promessa da SN-D2), o portão **fora** do HTML (`is-hold=0`) e o shell **só no cliente** (`app-shell-root=0`, que é a base da segurança de ler snapshot no inicializador do `useState`). Rota que deixa de ser pré-renderizada falha com a mensagem apontando para esta decisão. _(As rotas de **redirect** — `/forgot-password`, `/informe`, `/resumo` — ficam de fora do controle de propósito: `redirect()` aborta a árvore antes de qualquer pintura, então `fv-boot=0` ali é o resultado certo. Foi o primeiro falso positivo do check.)_
