...
- `app/dashboard/page.tsx:34`, `app/samples/page.tsx:2509`, `app/cadastros/page.tsx:397`, `app/contratos/page.tsx:41`, `app/financeiro/page.tsx:29`, `app/users/page.tsx:959`, `app/relatorios/page.tsx:24`, `app/profile/page.tsx:425`. _(Atualizado 2026-07-27: `/embarques` virou redirect e saiu da lista; `/financeiro` virou página real e entrou — RC-F1/RC-F4.)_
...
**A barra mobile (`components/MobileTabbar.tsx`) confirma o sintoma e a solução.** Ela já é renderizada via **portal no `document.body`** (fora do `PageTransition`, para não herdar o `will-change:transform` que quebrava o `position:fixed`) e já traz o **efeito de seleção pronto** (`.mobile-tabbar-link.is-active` + `aria-current="page"`) — exatamente o feedback IG/WA que o Flavio quer. Mas o componente ainda vive **dentro do `AppShell` montado por página** → remonta a cada navegação, e seu gate `mounted` (`useEffect`) o **zera por um tick** (`return null`) antes de reaparecer = o "pisca". Ou seja: **o efeito de seleção já existe no código; o que o destrói é a remontagem** — precisamente o que o shell persistente (F2) elimina. Abas mobile hoje: **5 fixas** (2026-07-28) — Início `/dashboard`, Lotes `/samples`, **Contratos `/contratos`**, Cadastros `/cadastros`, Relatórios `/relatorios`. Não há mais slot papel-dependente. Perfil, Usuários e Financeiro ficam no menu do avatar.
...
**Os 8 "gates `null`"** (o principal impacto visual — hoje cobertos pelo page loader): `dashboard/page.tsx:29`, `samples/page.tsx:1818`, `cadastros/page.tsx:221`, `contratos/page.tsx:36`, `financeiro/page.tsx:26`, `users/page.tsx:581`, `relatorios/page.tsx:19`, `profile/page.tsx:227` (`if (loading || !session) return null`). Sem o loader e sem outra solução, esse instante vira **tela branca**. _(Continuam 8: `/embarques` saiu — é `redirect()` server-side, sem gate — e `/financeiro` entrou, RC-F1/RC-F4.)_
...
### 5.1 Mapa de rotas (atual → alvo) — **EM DECISÃO** (linhas de contrato **IMPLEMENTADAS** em 2026-07-27)
...
> **2026-07-27 — as 3 linhas de contrato saíram de "a decidir" E JÁ ESTÃO NO CÓDIGO.** O ledger **RC** travou e a **RC-F1 + RC-F4 foram implementadas no mesmo dia** (`Contratos-Plano-de-Trabalho.md` §5.9): `/contratos` é página única, `/financeiro` é página própria só-ADMIN e `/embarques` é `redirect('/contratos')`. Consequência para a SN: a **contagem de itens de nav não mudou** (continuam 2 no bloco de contrato — Contratos e **Financeiro**, no lugar de Contratos e Embarques), e **as sub-abas expansíveis do bloco desapareceram** (`NAV_SUB_ITEMS` perdeu as 2 entradas) — a sidebar simplificou e o 4º slot da tabbar segue livre para decidir. **O inventário abaixo já está reconciliado com esse código.** O que a SN ainda deve reconciliar ao retomar: a RC-F6 (chrome FV de `/contratos` + `/financeiro`) e a RC-F2/F3, que ainda não rodaram.

> **2026-07-28 — a ORDEM das duas barras e o tamanho da tabbar foram decididos e implementados.** Pedido direto do Flavio, fora do ledger SN (é ajuste de nav, não depende das decisões SN-D1..D7). Desktop: Início · Lotes · **Contratos** · Relatórios · Cadastros · Financeiro (ADMIN) · Usuários (ADMIN). **Tabbar mobile: 5 abas FIXAS** — Início · Lotes · **Contratos** · Cadastros · Relatórios; Contratos subiu do menu do avatar, e Cadastros/Contratos saíram do menu para não duplicar caminho (o menu ficou Perfil · Usuários · Financeiro · Sair). **O 4º slot papel-dependente acabou:** ele alternava Relatórios/Perfil, mas desde o acesso unificado `INFORME_ROLES === NON_PROSPECTOR_ROLES` e o ramo Perfil nunca renderizava. Consequência para a SN: o **§5.1 deixa de ter a pergunta "quais são as abas"** — os 5 destinos estão travados; o que resta ao ciclo é o comportamento (SN-D7, estado por aba) e o chrome. A barra foi redimensionada junto (ícone -10%, apertos laterais; ver skill `design-system`, "Barra de navegacao inferior").

**Inventário verificado (2026-07-24; reconciliado com o código em 2026-07-27 após a RC-F1/F4 e em 2026-07-28 após a reordenação das barras):**

- **Abas mobile hoje (`MobileTabbar`, 5 slots fixos):** Início `/dashboard` · Lotes `/samples` · **Contratos `/contratos`** · Cadastros `/cadastros` · Relatórios `/relatorios`. Sem slot papel-dependente. Perfil, Usuários e Financeiro entram pelo menu do avatar — que **não** repete nenhuma aba da barra.
- **Sidebar desktop (`AppShell`):** Início · Lotes · **Contratos** · Relatórios · Cadastros · **Financeiro (só ADMIN)** · Usuários (ADMIN). Sub-abas expansíveis via `?tab=` — restaram **2**: `/samples` (Lotes/Simulador) · `/cadastros` (Clientes/Corretores). As de `/contratos` e `/embarques` **saíram na RC-F1/F4**.
- **Páginas autenticadas reais (8):** `/dashboard`, `/samples` (+ `/samples/[sampleId]`), `/cadastros`, `/contratos`, **`/financeiro` (ADMIN)**, `/relatorios`, `/users` (ADMIN), `/profile`.
- **Redirects/aliases (não são páginas):** `/clients`→`/cadastros` · **`/embarques`→`/contratos`** · `/resumo`→`/relatorios` · `/informe`→`/relatorios` · `/settings`→`/profile` · `/forgot-password`→`/login?modal=forgot-password`. _(`/contratos?tab=financeiro` também rebate para `/financeiro`, por compat de deep-link.)_
...
| `/contratos`                                     | **página única**          | **sim** (3ª aba)  | ✅ **feito** (RC-D1)                  | **IMPLEMENTADO 2026-07-27** (RC-F1). Sem abas; gate inalterado (5 não-PROSPECTOR). Falta a RC-F2: Embarque/Aprovação viram **fases** — hoje já **agem** no detalhe (RC-D25), mas como seções |
| `/financeiro`                                    | **página autenticada**    | não (avatar)      | ✅ **feito** (RC-D1/RC-D3)            | **IMPLEMENTADO 2026-07-27** (RC-F1). Carteira de corretagem, **só ADMIN** — 1ª rota ADMIN-only do domínio. Painel entrou intacto; chrome FV na RC-F6 (RC-D24)                                |
| `/embarques`                                     | **redirect**              | —                 | ✅ **extinta → `/contratos`** (RC-D2) | **IMPLEMENTADO 2026-07-27** (RC-F4). Painéis e cards apagados; saiu da nav. As worklists como **filtro de fase** na lista (RC-D9) ficam para a RC-F3                                         |
| `/relatorios`                                    | feed                      | **sim** (5ª aba)  | _a decidir_                           | absorve `/resumo`, `/informe`                                                                                                                                                                |
| `/users`                                         | lista (ADMIN)             | não               | _a decidir_                           | só ADMIN                                                                                                                                                                                     |
| `/profile`                                       | perfil                    | não (avatar)      | _a decidir_                           | absorve `/settings`; entra pelo menu do avatar                                                                                                                                               |
