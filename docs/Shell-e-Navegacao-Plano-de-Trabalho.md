...
**Inventário verificado (2026-07-24; reconciliado com o código em 2026-07-27 após a RC-F1/F4):**

- **Abas mobile hoje (`MobileTabbar`, 4 slots):** Início `/dashboard` · Lotes `/samples` · Cadastros `/cadastros` · 4º slot papel-dependente = Relatórios `/relatorios` **ou** Perfil `/profile`. Contratos e Financeiro **não** estão na tabbar (entram pelo menu do avatar).
- **Sidebar desktop (`AppShell`):** Início · Lotes · Relatórios · Cadastros · Contratos · **Financeiro (só ADMIN)** · Usuários (ADMIN). Sub-abas expansíveis via `?tab=` — restaram **2**: `/samples` (Lotes/Simulador) · `/cadastros` (Clientes/Corretores). As de `/contratos` e `/embarques` **saíram na RC-F1/F4**.
...
| `/contratos`                                     | **página única**          | não (avatar)      | ✅ **feito** (RC-D1)                  | **IMPLEMENTADO 2026-07-27** (RC-F1). Sem abas; gate inalterado (5 não-PROSPECTOR). Falta a RC-F2: Embarque/Aprovação viram **fases** — hoje já **agem** no detalhe (RC-D25), mas como seções |
