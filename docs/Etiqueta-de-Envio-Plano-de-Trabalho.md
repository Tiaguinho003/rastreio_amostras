...
### 2026-06-18 — Sessão 8 (deploy em produção real)

- **CI**: o push do feature comercial-visit (`b6576e0`, outro agente) deixou o CI **vermelho** — mas só no **Format check** (`modals/SKILL.md` sem prettier), o que **pulou type check/build/testes**. Corrigido com `prettier --write` (commit `5a3e4a0`); CI re-rodou **verde completo** (2m59s, suíte inteira).
- **Deploy canary→prod** (fluxo padrão, imagem pinada na tag `5a3e4a0`): `build-image` → `deploy-cloud --canary` → smoke no canary ✅ → `execute-job migrate` (aplicou a migration `20260618130000` do comercial-visit + pendentes) → `update-traffic --to-latest`. **Produção real em `rastreio-prod-app-00356-vil`** (100% tráfego); smoke na prod ✅.
