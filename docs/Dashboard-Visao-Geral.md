...
### 7.2 Amostras enviadas + Aprovações enviadas (`RecentSendsCard`) — **OS DOIS SAÍRAM DO PRODUTO**
...
>
> **🔴 Atualização de 2026-07-27 — "Aprovações enviadas" também foi REMOVIDO (RC-D26).** A extinção de `/embarques` levou junto a aba que o hospedava. Como ele era o **último consumidor vivo** do `RecentSendsCard`, o componente inteiro foi apagado, com o CSS `.sends-*`, o helper `getApprovalRecentSends` e a rota `GET /sale-contracts/approvals/recent-sends`. **Ficaram órfãos**: a rota `GET /samples/recent-sends` + `getSampleRecentSends` (que têm suíte própria) e o método de service `getRecentApprovalSends` (coberto por teste; o dado é candidato da fase Aprovação na RC-F2). O hook `lib/use-recent-sends-feed.ts` **continua vivo** — o card de Avisos (§7.4) usa o mesmo hook com outro feed.
>
...
- **"Aprovações enviadas"** (`variant="approvals"`) → topo da aba **Aprovações** de `/embarques`, acima da worklist (visão rápida; a worklist com filtro "Enviadas" segue sendo a lista completa — redundância parcial aceita). Colunas **Contrato · Comprador · Tempo**. Dado: `GET /sale-contracts/approvals/recent-sends` (`getApprovalRecentSends` → `getRecentApprovalSends` do contract service, top-40; degrada pra `[]` sem contract service). Refetch extra após gerar etiqueta na própria aba.
...
- **Item:** nº do contrato + comprador + **chip de prazo por proximidade** ("vence hoje / amanhã / esta semana / este mês / em N dias"; **"sem data"** p/ À definir; **"faturamento vencido"** se a data já passou — `formatAvisoDue`). Linha **clicável** → `/contratos?details=<id>&highlight=<id>` (**RC-D23**; era a worklist de Aprovações, extinta). "A ação mora na casa" segue valendo — a casa de "Gerar etiqueta" agora é a seção **Aprovação** do detalhe do contrato (RC-D25).
