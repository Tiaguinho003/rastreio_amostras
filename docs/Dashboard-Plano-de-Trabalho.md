...
O redesenho e os feeds foram implementados mas nunca foram confirmados no aparelho real. Antes (ou durante) as mudanças deste check-up, validar (lista atualizada pós-DSB-D14/D15):

- **Chrome desktop (DSB-D15, cross-página):** sidenav lateral em TODAS as páginas desktop (901–1400px) — trilho (logo quadrado + avatar) + painel branco (ícone + nome, pílula verde no ativo, até 7 itens no ADMIN); a faixa branca do topo sumiu; menu do avatar abre em dropup no trilho (Meu perfil / Sair funcionando); conteúdo das páginas densas (Lotes/Contratos/dashboard 100vh) confortável com ~276px a menos de largura; **PROSPECTOR intacto** (sidebar verde + faixa branca); **mobile intacto** (topbar verde + tabbar).
...
- **DSB-D15 (2026-07-14)** — **Navegação desktop: top bar → SIDEBAR LATERAL ESQUERDA branca em 2 colunas** (mudança **cross-página**, centralizada em `AppShell.tsx`; supera o chrome do DSB-D6). Referência visual: imagem enviada pelo Flavio (sidebar estilo "recruiting app" — desconsiderado o conteúdo, aproveitado o estilo). Decisões do Flavio:
  - **Estrutura em 2 colunas como a imagem:** trilho fino à extrema esquerda (`.app-sidenav-rail`: logo quadrado `icon-safras.png` em cima → `/dashboard`; avatar embaixo) + painel branco (`.app-sidenav-panel`) com a navegação (ícone + nome, botões **pequenos**).
  - **A faixa branca do topo SOME no desktop** (não-PROSPECTOR): logo e avatar/menu (Meu perfil / Sair) migram pra sidebar; o conteúdo ganha a altura toda. O menu do avatar abre em **dropup** ancorado no trilho (mesmo card do dropdown atual, extraído pra componente local).
  - **Item ativo = pílula suave verde** (tint `rgba(31,93,67,0.10)` + texto/ícone `--brand-green`) — adaptação da pílula azul da imagem à paleta; substitui o sublinhado do DSB-D6.
  - **Sem busca na sidebar** (pedido explícito; a busca de lote já estava oculta no desktop desde DSB-D6). Mobile e tabbar **intactos**.
  - **PROSPECTOR fica como está** (sidebar verde própria do app restrito — mesma decisão do DSB-D6).
  - Técnica: grid via `:has(.app-sidenav)` (2 colunas `~276px + 1fr`), sidebar **sticky + 100lvh** (nunca `position:fixed` — PageTransition aplica transform); ícones reusam `renderNavIcon`/`desktopNavItems` (nada novo); CSS morto do DSB-D6 (`.topbar-nav*`, `:has(.topbar-nav)*`, logo `is-color`, chevron) removido.
...
- **2026-07-14** — **DSB-D15 implementada:** navegação desktop dos não-PROSPECTOR virou **sidenav lateral esquerda branca em 2 colunas** (`.app-sidenav`: trilho `#f4f6f5` com logo `icon-safras.png` + avatar/menu dropup; painel branco com ícone+nome e **pílula verde** no ativo) — supera o chrome do DSB-D6 (top bar). Faixa branca do topo **some** no desktop deles (`:has(.app-sidenav)`, conteúdo em altura total); "Sair" segue só no menu do avatar (card extraído pra `ProfileMenuCard`, compartilhado dropdown/dropup; dismiss unificado em `useMenuDismiss`). CSS morto do DSB-D6 removido (`.topbar-nav*`, `:has(.topbar-nav)*`, logo `is-color` — o asset fica, o PDF do contrato usa —, chevron). PROSPECTOR e mobile intactos. Ícones reusam `renderNavIcon` (nada novo). Docs (Auditoria-Navegação canônico, Visão Geral §5, Contratos-Visão-Geral §2) + skill design-system. Gates verdes (typecheck/lint/format/449 unit); build fica pro device (dev ativo). 📱 validar (ver §4).
