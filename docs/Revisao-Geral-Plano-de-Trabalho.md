...
| 9   | CTR    | Contratos          | `/contratos`                                                                  | ⬜     | —        | Reorganizada ANTES da revisão pelo ledger **RC** (`Contratos-Plano-de-Trabalho.md` §5 e **§6**): virou página única e, na **§6 (RC-D62..D68, 2026-07-28)**, deixou de ser máquina de status — três situações + **agenda** derivada; o embarque foi apagado inteiro e a lista mostra o **próximo compromisso**. A revisão CTR só faz sentido sobre a página reorganizada — ver §RC abaixo                                                                                                                                                                                      |
| 10  | FIN    | Financeiro         | `/financeiro` (redirect hoje → página própria pela RC-D1)                     | ⬜     | —        | A rota desta linha estava **stale**: `/financeiro` é redirect para `/contratos?tab=financeiro` desde o split de 2026-07-13 (CC3). A **RC-D1/RC-D3** devolve a página, agora **só ADMIN** — 1ª rota ADMIN-only do domínio                                                                                                                                                                                                                                                                                                                                                      |
| 11  | CAD    | Cadastros          | `/cadastros`                                                                  | ⬜     | —        | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 12  | REL    | Relatórios         | `/informe` (+ redirect `/resumo`)                                             | ⬜     | —        | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 13  | USR    | Usuários           | `/users`                                                                      | 📱     | FV §2.11 | Revisada junto com o redesenho FV (R1/R4/R5/R6 vieram de lá; R2/R3 fechados nesta passada). Decisões D1–D4: tela antecipa a auto-ação do ADMIN, mensagens do backend em pt-BR, detalhe endereçável `?usuario=`, auditoria sem tela mantida. Achados B1–B3/I1/L1 corrigidos, I2 catalogado; `assertAdminInvariant` ganhou 6 testes (545→551). Gate de 2 camadas, não 3 — o middleware não tem regra pra `/users`                                                                                                                                                               |
| 14  | PRF    | Perfil             | `/profile` (+ redirect `/settings`)                                           | 📱     | FV §2.12 | Revisada junto com o redesenho FV (R1/R4/R5/R6 vieram de lá; R2/R3 fechados nesta passada). A página MENOS restrita do app — `useRequireAuth()` sem `allowedRoles`; a única variação por papel é a barra de voltar do PROSPECTOR. Decisões D1b–D3: molde de página do ciclo em largura cheia (a coluna centrada caiu na conferência), edição segue acordeão, cabeçalho mantém a identidade. Achados B1–B4/I1/I2 corrigidos — inclusive o nome do usuário INVISÍVEL entre 901 e 1199px. R4 tirou 66 regras/447 linhas e extinguiu o namespace `.cdm-*`, pendente desde a §2.11 |
| 15  | AUX    | Auxiliares         | `/laudo/[token]` (público), `/offline`, `/maintenance`, redirects `/` e afins | ⬜     | —        | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |

> **Por que não existe linha para `/embarques`.** A rota nasceu no split de 2026-07-13,
> depois desta tabela, e **foi extinta** pela **RC-D2** — redirect para `/contratos`. Das
> duas capacidades que ela hospedava, a **aprovação** foi para o detalhe do contrato
> (revisada na linha **CTR**) e o **embarque foi apagado do produto** (RC-D65).
...
### Contratos (CTR) — ⬜ não iniciada (bloqueada pela RC)
...
> **Bloqueada por decisão, não por falta de vez (2026-07-27).** A página vai ser
> **reorganizada antes de ser revisada** — ledger **RC**, `Contratos-Plano-de-Trabalho.md`
> §5: `/contratos` vira página única, `/embarques` e a sub-aba Aprovações são extintas, o
> processo passa a aparecer **fase a fase dentro do contrato** e a lista absorve as duas
> worklists. Revisar agora seria auditar uma casca que já tem data para morrer. Quando a
> CTR começar, o escopo dela inclui o que veio de `/embarques` (embarque + aprovação).

### Financeiro (FIN) — ⬜ não iniciada (bloqueada pela RC)

> **A rota da tabela estava errada.** `/financeiro` não é página desde o split de
> 2026-07-13 — é redirect para `/contratos?tab=financeiro` (CC3). A **RC-D1** devolve a
> página própria e a **RC-D3** a fecha em **ADMIN**, o que faz dela a primeira rota
> ADMIN-only do domínio de contratos. A revisão FIN espera essa página existir; o recorte
> de papel é o achado principal que ela vai ter de conferir ponta a ponta (rota, service e
> os 3 consumidores de `FINANCEIRO_ROLES`).

### Cadastros (CAD) — ⬜ não iniciada

### Relatórios (REL) — ⬜ não iniciada

### Usuários (USR) — 📱 aguardando validação no device (2026-07-27)

Página revisada **junto com o redesenho FV** (`Redesign-Plano-de-Trabalho.md`
§2.11, fases U1–U8). O FV entregou R1 (mapa), R4 (código morto), R5 (design e
mobile) e R6 (docs); esta passada fechou **R2 e R3** e o que eles acharam.

**Mapa (R1):** o inventário completo está no §2.11 — aqui só o que ele não
diz. Cadeia de dados: `lib/api-client.ts` (`listUsers`/`getUser`/`createUser`/
`updateUser` + `inactivate`/`reactivate`/`unlock`/`resetUserPassword`) → rotas
`app/api/v1/users/*` → `src/api/v1/backend-api.js` → `UserService`
(`src/users/user-service.js`) + normalizadores puros em `user-support.js` →
tabelas `app_user` e `user_audit_event`. Componentes próprios:
`InactivateUserModal` → `InactivateConfirmDialog` (os únicos modais centrais
que sobraram na página).

**Matriz por papel (R2):** a página mais simples do app nesse eixo — é a
**única exceção** do ACESSO UNIFICADO (2026-07-15).

| Papel                                             | Vê                                | Faz                                                                                                | Bloqueado                                                                       |
| ------------------------------------------------- | --------------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| ADMIN                                             | item "Usuários" na nav + a página | listar/buscar · criar · editar (dados e papel) · redefinir senha · inativar/reativar · desbloquear | tirar o **próprio** acesso admin; deixar o sistema sem ADMIN ativo              |
| CLASSIFIER · REGISTRATION · COMMERCIAL · CADASTRO | nada — o item nem aparece na nav  | —                                                                                                  | a rota inteira (guard da página redireciona; backend responde `ROLE_FORBIDDEN`) |
| PROSPECTOR                                        | nada                              | —                                                                                                  | idem, e antes disso a allowlist central (`src/auth/prospector-access.js`)       |

🔴 **O gate tem DUAS camadas aqui, não três.** `middleware.ts` **não** tem
regra por rota para `/users` — ele cobre só o modo manutenção e a allowlist do
PROSPECTOR. Quem barra é o guard da página
(`useRequireAuth({ allowedRoles: ['ADMIN'] })`, `app/users/page.tsx`) e o
backend (`assertAdminActor` em toda operação do `UserService`). O backend é o
que vale: o guard de front é UX.

**Decisões (R2):**

- **USR-D1 (= U-D7)** — **a tela antecipa a auto-ação.** No próprio usuário,
  "Inativar" não renderiza e o seletor de perfil fica travado com nota. A trava
  real segue no backend. O outro ramo do invariante (**último ADMIN ativo**,
  mesmo não sendo você) fica **só no backend** de propósito: a tela precisaria
  de uma contagem que não tem, e o toast do 409 já explica.
- **USR-D2 (= U-D8)** — **mensagens de erro do backend em pt-BR acentuado.**
  Elas sobem direto para o toast (`description: cause.message`), então são
  texto de UI. Escopo: as 16 do `user-service.js`, não só as de `/users` — o
  mesmo arquivo serve `/login` e `/profile` e ficaria com duas grafias.
- **USR-D3 (= U-D9)** — **detalhe endereçável** (`containers` §5):
  `/users?usuario=<id>` sobrevive ao F5 e vira link; criar e busca ficam fora.
- **USR-D4 (= U-D5)** — **auditoria sem tela**, decisão mantida. A rota e o
  `listUserAuditEvents()` continuam vivos e agora **documentados como decisão**
  no `API-e-Contratos`, para a próxima varredura não os tratar como órfãos.

**Achados:**

- **USR-B1** ✅ — a tela oferecia inativar-se e trocar o próprio papel; só
  falhava no envio (409 `LAST_ADMIN_REQUIRED`). Corrigido pela USR-D1.
- **USR-B2** ✅ — `listState.error` era guardado pelo reducer e **nunca lido**:
  falha de rede mostrava "Nenhum usuário encontrado". (Corrigido na U2.)
- **USR-B3** ✅ — `lastLoginAt` vinha no payload e não era renderizado em lugar
  nenhum. Virou a coluna "Último acesso". (U2.)
- **USR-I1** ✅ — 16 mensagens do backend sem acento, visíveis no toast. USR-D2.
- **USR-I2** ⏳ **aberto (catalogado, D7)** — os normalizadores de
  `user-support.js` lançam 422 com mensagem **em inglês** (`${fieldName} is
required`, `... is invalid`). O front valida antes e o campo mostra pt-BR, então
  o inglês só escapa num 422 que a validação de tela não antecipa (ex.: telefone
  que passa na máscara e falha no `normalizePhone`). Fora do escopo desta
  passada: são mensagens de contrato, não de tela, e mexem no domínio inteiro.
- **USR-L1** ✅ — detalhe não endereçável. USR-D3.
- **USR-T1** ✅ — `assertAdminInvariant` (o invariante que impede o sistema de
  ficar sem ADMIN) **não tinha teste**. 6 casos em
  `tests/user-admin-invariant.test.js`.
- **USR-DOC1–4** ✅ (U8) — rota `clients-impact` ausente do `API-e-Contratos`;
  `GET /users` sem os filtros `role`/`status`; nota do RD7 dizendo que o CSS
  `.cdm-*` fica porque `/users` reusa (falso desde a U6); `Shell-e-Navegacao`
  apontando `app/users/page.tsx:702` para o `AppShell`.
- **USR-DOC5** ✅ — `Produto-e-Fluxos` listava "consultar trilha de auditoria"
  como algo que o administrador faz. Não faz: a trilha é gravada e a consulta é
  pelo banco (runbook do `SECURITY.md`).
- **USR-DOC6** ✅ — três docs de SECURITY diziam "13 event types"; são 17.
  Trocado pela referência ao `enum UserAuditEventType`, que não envelhece.

**R3 (fluxos) — conferido, sem achado novo:** busca com debounce e mínimo de 2
caracteres, igual a `/samples` e `/clients`; duplo clique coberto por
`modal.saving` em todos os submits e ações; carregando/vazio/erro os três
renderizados (o erro era o USR-B2); scroll infinito por cursor com `limit=30`
contra `USER_LIST_LIMIT_MAX=60`; o `BottomSheet` é dono único do histórico, e
o back fecha o painel em um toque.

**Resumo do que foi feito:** `d9ef6db` (docs) · `540ee0f` (USR-D1) · `8a6465c`
(USR-D2) · `6169237` (USR-D3) · `64b0bf3` (USR-T1) — mais os 19 commits do FV
(`6fb78dd`..`5f47905`). Gates verdes, **551** unit / 20 contract, build limpo.

**Pendências:** **USR-P1** — validação no device (🖥️📱); só com ela a página
vira ✅. **USR-P2** — o USR-I2 (mensagens de contrato em inglês) fica para uma
passada de domínio, não de página.

### Perfil (PRF) — 📱 aguardando validação no device (2026-07-27)

Revisada **junto com o redesenho FV** (`Redesign-Plano-de-Trabalho.md` §2.12,
fases P1–P3), no mesmo modelo da USR: o ciclo FV entregou R1, R4, R5 e R6, e
esta seção fecha o registro com R2 e R3.

**Mapa (R1):** o inventário está no §2.12 — aqui o que ele não diz. Cadeia:
`app/profile/page.tsx` (árvore ÚNICA — a página **não usa `useIsDesktop`**) →
`lib/api-client.ts` (`getCurrentUser`, `updateCurrentUserProfile`,
`requestCurrentUserEmailChange`, `confirmCurrentUserEmailChange`,
`resendCurrentUserEmailChangeCode`, `changeCurrentUserPassword`) → rotas
`app/api/v1/users/me/*` → `src/api/v1/backend-api.js` → `UserService` — **o
mesmo serviço de `/users`**, o que explica a U-D8 ter acentuado as 16
mensagens de lá pensando também nesta tela. Push por
`lib/push/use-push-notifications.ts`. `/settings` é redirect para cá.

**Matriz por papel (R2):** a página **menos** restrita do app — o oposto da
USR. `useRequireAuth()` **sem `allowedRoles`**: todo autenticado entra no
próprio perfil, e não há operação sobre terceiros (as rotas são `/users/me/*`).

| Papel                                                     | Vê                                              | Faz                                                                                            | Bloqueado             |
| --------------------------------------------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------- | --------------------- |
| ADMIN · CLASSIFIER · REGISTRATION · COMMERCIAL · CADASTRO | avatar + nome + papel; 5 linhas; Notificações   | editar nome/usuário/telefone · trocar e-mail (com código) · trocar senha · ligar/desligar push | nada da própria conta |
| PROSPECTOR                                                | **idem, mais a barra de voltar** (`.nsv2-back`) | idem                                                                                           | idem                  |

🔴 **A única variação por papel é a barra de voltar.** O PROSPECTOR não recebe
o `.fv-mtopbar` do shell nem a tabbar (app restrito), então o `/profile`
mantém a faixa própria só para ele — é a **última** `.sdv-header-top` viva no
app. Por isso `.nsv2-back` sobreviveu à limpeza da P3.

**Decisões (R2/R5):** **PRF-D1b** (= P-D1b) — o desktop usa o **molde de
página do ciclo**, largura cheia, e não a coluna centrada proposta primeiro;
área em branco é espaço reservado, decisão do Flavio na conferência da P1.
**PRF-D2** (= P-D2) — a edição continua **acordeão**, não vira painel lateral:
é um campo por vez, e a árvore do `containers` manda inline nesse caso.
**PRF-D3** (= P-D3) — o cabeçalho mantém avatar + nome + papel.

**Achados:**

- **PRF-B1** ✅ — **o nome do usuário era invisível entre 901 e 1199px.** A
  base pinta `#ffffff` (feito para a faixa verde do mobile) e o override só
  existia a partir de **1200px**; no meio do desktop ficava branco sobre fundo
  claro. Corrigido na P1 subindo as regras para `≥901px`.
- **PRF-B2** ✅ — o mesmo `profileError` era renderizado em **três** linhas do
  acordeão (nome/usuário/telefone compartilham o submit): errar o telefone
  acendia o erro embaixo do nome. Corrigido na P2 (erro por campo).
- **PRF-B3** ✅ — o efeito de `?section=password` **nunca funcionou como
  pretendido**: rolava até a linha da senha, que está **fechada** por padrão. E
  nada no repositório gera o link. Removido na P2.
- **PRF-B4** ✅ — `--i: 3` no card de Notificações, delay de animação de quando
  a página tinha 4 cards; hoje são 2. (P1.)
- **PRF-I1** ✅ — 14 strings sem acento na página e 21 em `lib/form-schemas.ts`,
  que **já tinha duas grafias** entre schemas gêmeos. (P2.)
- **PRF-I2** ✅ — `.stg-field-copy` duplicava `.sdv-info-copy` do kit, com o
  mesmo SVG. Adotada a peça. (P2.)
- **PRF-M1** ✅ (R4) — **66 regras / 447 linhas** de CSS morto, incluindo o
  namespace `.cdm-*` inteiro, que a §2.11 tinha deixado pendente. (P3.)

**R3 (fluxos) — conferido, sem achado novo:** os quatro fluxos (dados,
troca de e-mail com código, senha, push) tratam carregando e erro; **não há
estado "vazio"** — sempre existe um usuário. Duplo clique coberto por
`disabled` nos submits **e** nos inputs durante o envio. Trocar usuário ou
senha revoga a sessão e a página redireciona para `/login?reason=session-ended`
— sem toast, porque o destino já explica. O único modal é central
(`containers` §8: desativar push **fica** central).

**Resumo do que foi feito:** `33bb4d3` (ledger) · `0b1be82` + `f3035db` (P1) ·
`4e7254d` + `a06cf5c` (P2) · `deebcbf` (P3), mais os commits de skills. Gates
verdes, 551 unit / 20 contract.

**Pendências:** **PRF-P1** — validação no device (🖥️📱); só com ela a página
vira ✅.
