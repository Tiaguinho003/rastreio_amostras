...
3. **Autorização por papel (ACESSO UNIFICADO 2026-07-15).** Todo papel **não-PROSPECTOR** vê e opera **tudo** — contratos, Financeiro, Relatórios, Cadastros (own-only já revogado em D140; o acesso unificado estendeu os gates a todos os não-PROSPECTOR: `SALE_CONTRACT_ACCESS_ROLES`/`FINANCEIRO_ROLES`/`INFORME_ROLES`/`CLIENT_MANAGEMENT_ROLES` = `NON_PROSPECTOR_ROLES`). A **única** exceção por papel é `/users` (só ADMIN, `assertAdminActor`). PROSPECTOR é restrito por allowlist central (`src/auth/prospector-access.js`, fail-closed). Endpoints de cliente/amostra exigem apenas autenticação (gate de front é só UX) — o assistente herda exatamente esse modelo. _(Ver `Auditoria-Navegacao-por-Papel.md`; a uniformidade de-risca as permissões do assistente — "o assistente é o usuário" dá o mesmo alcance a todo não-PROSPECTOR.)_
