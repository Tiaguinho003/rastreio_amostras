...
> 2. **Lote de origem → read-only na liga.** Numa liga o `declared.originLot` é **`null` por design** (forçado no `createBlend`; a origem real vive em `SampleBlendComponent`). Mas o input do modal não era gated → dava pra gravar um **lote-fantasma**. Agora o campo é **read-only** no modal quando `isBlend` (nota "A liga não tem lote de origem próprio") e uma **guarda no `updateRegistration`** rejeita **422 `BLEND_ORIGIN_LOT_READ_ONLY`** (a câmera não reconcilia `originLot`, então não precisa de skip lá).
>
