...
> **Revisão "dono fixado" em 2026-07-15** (revoga o "owner null por default" acima): o dono da liga volta a ser **escolhido na criação** (seletor `ClientLookupField` no `BlendConfirmationSheet`), mas agora **fixado** e **desacoplado das origens**. Pré-preenche o **dono unânime** quando as origens batem; quando **divergem, escolher é obrigatório** — um cliente qualquer (inclusive um **terceiro** que não é dono de nenhuma origem) ou explicitamente **"carteira da corretora"**. A escolha grava **`blendOwnerPinned=true`** (`Sample.blend_owner_pinned`, migration `20260715122623_blend_owner_pinned`): a **propagação reativa de dono** (`_buildBlendPropagation`) passa a **respeitar o pin** — só a **safra** continua derivando das origens; o **dono não é mais recalculado/sobrescrito**. **"Carteira da corretora"** = `blendOwnerPinned=true` + `ownerClientId=null` (escolha explícita, distinta de "sem dono" derivado). **Auto-pin** (`updateRegistration`): qualquer edição manual do dono de uma liga — detalhe do lote ou "Atribuir dono" do modal de venda — fixa o dono. (A terceira via, o **sync do contrato de venda** (D146), morreu com a RC-D37 em 2026-07-28.) **Direção assimétrica**: origem→liga propaga **só a safra**; liga→origem **nunca** (as origens guardam o dono real do produtor). **Backfill** (na própria migration): as ligas existentes com dono unânime foram **fixadas**; as sem dono seguem reativas até um dono ser atribuído. A UI mostra **"Carteira da corretora"** (cards/detalhe) e o modal de venda **não pede** "atribuir dono" para uma carteira fixada. Fixa a raiz do gargalo em que um dono setado à mão (ou pelo contrato) era **sobrescrito** pela re-derivação na edição seguinte de uma origem.

> **Revisão "a liga nasce com dono" em 2026-07-28** (RC-D38/RC-D39 do ciclo de contratos, `Contratos-Plano-de-Trabalho.md` §5.11) — **revoga a "carteira da corretora" como opção de CRIAÇÃO**: escolher um cliente dono passou a ser obrigatório. O checkbox saiu do `BlendConfirmationSheet` e o `createBlend` devolve **422** sem `ownerClientId`; o `deriveBlendOwner` saiu do serviço (a unanimidade das origens virou **pré-preenchimento da tela**) e segue servindo só o `blend-backfill`. Toda liga nova nasce **fixada** (`blendOwnerPinned=true`), então o `false` passou a marcar exclusivamente liga legada. **O estado "carteira da corretora" continua existindo** em ligas antigas — rótulo nos cards/detalhe, nudge "Atribuir dono primeiro" no painel de perda e o schema de evento com `declared.owner` nullable **ficam de pé** —, mas ela **não vende mais**: some do `sellableOnly` (picker do contrato à vista) e o `createSampleMovement` recusa `SALE` sem dono, porque o vendedor do contrato é o dono do lote (**RC-D37**). Isso **reverte a rejeição da opção (D)** registrada em §F3.A (":1202", "fricção sem justificativa"): o Flavio decidiu que o café em nome da corretora deixou de ser caso operacional na criação. Regulariza-se atribuindo dono no detalhe do lote. O **auto-pin** perdeu uma de suas três vias: o sync do contrato de venda foi apagado junto com a D48.

> **Revisão "o dono nunca propaga" em 2026-07-28** (RC-D36 do ciclo de contratos, `Contratos-Plano-de-Trabalho.md` §5.10): a revisão acima dizia que "o dono não é mais recalculado/sobrescrito", mas isso valia **só para a liga fixada** — para a liga **sem pin**, `_buildBlendPropagation` seguia rodando `deriveBlendOwner` e reescrevendo o dono a cada edição de origem. Agora **acabou de vez**: editar o dono de um lote **nunca** mexe no dono de liga ancestral alguma, fixada ou não. O gatilho da propagação virou `harvestChanged || originLotChanged` — edição só-de-dono não monta propagação, não emite `409 BLEND_HARVEST_PROPAGATION_REQUIRED` e não toca liga nenhuma. `deriveBlendOwner` continua servindo o `createBlend` (unanimidade **na criação**) e saiu da propagação. O **auto-pin** fica (é o que distingue "carteira da corretora" de "sem dono"). O texto de 2026-07-15 passa a descrever o código.

> **Revisão "safra Mix" em 2026-07-15** (fecha o gargalo da safra, análogo ao dono): diferente do dono, a **safra da liga NÃO é escolhível nem editável** — deriva sempre das origens (`deriveBlendHarvest`) e ponto. Três frentes:
>
> 1. **Read-only na liga.** O campo Safra do modal de edição do detalhe (`app/samples/[sampleId]/page.tsx`) fica **read-only** quando `isBlend` (nota "Deriva dos lotes"), e uma **guarda no `updateRegistration`** rejeita 422 qualquer edição que **mude** a safra de uma liga (defense-in-depth; a propagação reativa, que constrói drafts em `_buildBlendPropagation`, não passa por essa guarda). Remove a armadilha em que uma safra digitada à mão na liga era volátil (sobrescrita na próxima edição de origem), já que não há "pin" de safra.
> 2. **Apresentação "Mix".** Quando a liga tem **2+ safras distintas** (`declaredHarvest` concatenado), a UI mostra um **badge `[Mix]`** em vez da string crua: **card = só o badge**; **detalhe / linhas de origem = `[Mix] 24/25 · 25/26`**. Safra única segue mostrando a safra normal. Helpers `splitHarvests`/`isMixHarvest` em `lib/sample-identification.ts` (o `summarizeHarvest` "mais nova + `+`" é aposentado); badge `MixHarvestBadge` (molde `BlendBadge`).
> 3. **Laudo revela o Mix (inverte o anti-vazamento SÓ na safra).** Antes o laudo **forçava escolher UMA safra** (422 `normalizeReportedHarvest`) pra nunca imprimir a concatenada — o "tell" de que era liga. Agora o laudo de liga multi-safra imprime **`Mix` + cada safra com a % da composição** (Q1). A **% é por sacas, descendo até os lotes-folha** (Q2): `buildHarvestBreakdown` (`src/reports/export-fields.js`) roda `loadBlendTree` (agora com `declared_harvest`), agrupa as **folhas** (`!isBlend`) por safra e soma `contributedSacks` — exato mesmo em liga-de-liga graças ao F7.7 (a liga-em-liga contribui 100%, então a soma das folhas bate com o total). **Sem revelar origens/produtores** — o breakdown só agrupa por safra. A **escolha forçada some**: `normalizeReportedHarvest`/`resolveReportedHarvestLenient` deixam de lançar/cair-pra-`options[0]` e retornam `null` em "multi sem escolha"; o `ReportHarvestSelectModal` é removido (código morto). **Etiqueta física** (`shippingPrintJob.payload.harvest`, `backend-api.js`) imprime **`Mix — 24/25, 25/26`** (Q4). **Laudos já enviados** têm a safra única gravada em `share.reportedHarvest` → seguem **congelados** mostrando essa safra; só os **novos** (`reportedHarvest=null`) mostram Mix (Q5).
>
> **Sem migration/coluna/evento** — a safra continua derivada; `reportedHarvest` (share + `REPORT_EXPORTED`) já existe e vira `null` nos envios novos (o schema `report-exported.payload` é non-nullable → a chave é **omitida** quando vazia, nunca gravada como `null`). Decisões travadas: Q1 (Mix+%, sem origens) / Q2 (% por sacas até as folhas) / Q3 (badge Mix) / Q4 (etiqueta "Mix — safras") / Q5 (laudos antigos congelados).

> **Revisão "sacas e lote read-only" em 2026-07-15** (fecha os buracos restantes de "campo derivado editável na liga", análogo à safra Mix e ao dono fixado). Auditoria de **todos os caminhos de edição** da liga revelou dois campos que **derivam da composição mas seguiam editáveis** — inconsistência silenciosa, já que a **composição é imutável após criar** (único escritor `createBlend`; `SampleBlendComponent` append-only; `onDelete: Restrict`) e não há como rebalancear as origens pra casar com um valor editado à mão. Duas frentes:
>
> 1. **Sacas → read-only na liga.** O total da liga é derivado (`declaredSacks = Σ contributedSacks`, no `createBlend`) e **nunca re-sincroniza** (a propagação reativa só toca safra/dono). Continuava editável em **dois** caminhos — o campo Sacas do modal de edição (`app/samples/[sampleId]/page.tsx`) e a reconciliação da **ficha/câmera** (`applySampleUpdates` em `sample-command-service.js`). Editar decoplava o total da soma das origens e **quebrava o laudo Mix** (o "% por sacas" é calculado das folhas, cuja soma é o total original — se a Sacas exibida divergir, deixa de bater com a composição). Agora: campo **read-only** no modal quando `isBlend` (nota "Deriva dos lotes"), a câmera **pula** `declaredSacks` na liga (espelha a safra, `&& !current.isBlend`), e uma **guarda no `updateRegistration`** rejeita **422 `BLEND_SACKS_READ_ONLY`** qualquer edição que **mude** as sacas de uma liga (posta **antes** do bloco de redução de sacas, pra a liga receber a mensagem certa e não o 409 de "não é possível reduzir"). A guarda é por **presença da chave** — segura porque `buildRegistrationUpdatePayload` faz o diff e só mantém o campo quando mudou; salvar outros campos (ex.: Local) numa liga não dispara.
> 2. ~~**Lote de origem → read-only na liga.**~~ **⚠️ REVOGADA em 2026-07-20** (ver o log daquela data): a origem da liga passou a ser **derivada** (`deriveBlendOriginLot`), **editável e pinável** (`blendOriginLotPinned`), e o guard `BLEND_ORIGIN_LOT_READ_ONLY` foi **removido do código**. O texto abaixo é o estado anterior. — Numa liga o `declared.originLot` era **`null` por design** (forçado no `createBlend`; a origem real vive em `SampleBlendComponent`). Mas o input do modal não era gated → dava pra gravar um **lote-fantasma**. Agora o campo é **read-only** no modal quando `isBlend` (nota "A liga não tem lote de origem próprio") e uma **guarda no `updateRegistration`** rejeita **422 `BLEND_ORIGIN_LOT_READ_ONLY`** (a câmera não reconcilia `originLot`, então não precisa de skip lá).
>
> **Dono NÃO é travado** — segue **editável** (é o recurso "dono fixado" / `blendOwnerPinned` da revisão acima; editar re-fixa). **Local** e a **classificação** seguem editáveis (autorais da liga). **Sem migration** — nada de coluna/evento novo; puro guard + UI. Escopo travado com o usuário (AskUserQuestion): travar Sacas + Lote de origem, manter Dono. **Tensão conhecida fora de escopo** (não é edição de liga): vender uma origem abaixo do que contribuiu deixa `contributedSacks` stale — over-commit entre ligas é intencional; a defesa é detecção ("Liga inviável" no `getBlendFeasibility` + `409 BLEND_HAS_BLOCKED_DESCENDANTS` ao vender a liga drenada).

> ## 🔴 Revisão "estado das superfícies" em 2026-07-22 (leia antes de seguir qualquer ponteiro de UI abaixo)
>
> O redesenho FV de `/samples` (RD15, `docs/Redesign-Plano-de-Trabalho.md` §2.7) reescreveu **todas** as telas da liga. As **regras de domínio deste doc continuam válidas** — o que envelheceu foram os componentes e contêineres. Tabela de tradução, boa para o doc inteiro:
>
> | O doc diz                                                                   | Realidade                                                                                                                                     |
> | --------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
> | `app/samples/[sampleId]/page.tsx`                                           | **redirect de 26 linhas**. O detalhe é `components/samples/SampleDetailView.tsx`                                                              |
> | `SampleCreatedSuccessModal` (modal sucesso)                                 | **deletado**. Sucesso = `SuccessCheckOverlay` **dentro** do painel de criação; ao sair o check, o drawer da liga abre sozinho                 |
> | Botão "Criar outra liga"                                                    | **morreu junto**. Criar de novo = reabrir o painel pelo CTA da página                                                                         |
> | `SampleMovementModal` (venda/perda/edição)                                  | **deletado**. Perda → `SampleLossSheet` (painel); venda → `/contratos` (`SaleContractEtapa2Modal`); edição de movimentação → **sem UI**       |
> | "Modal F3" central (dono/safra/local/obs)                                   | **removido em 2026-05-19**; o dono voltou em 2026-07-15, mas como **campo do `BlendConfirmationSheet`**                                       |
> | `ReportHarvestSelectModal`                                                  | **deletado** — o laudo de liga multi-safra imprime Mix + %, não escolhe safra                                                                 |
> | `BlendRevertModal` — "modal `.app-confirm-modal`"                           | **painel lateral** no molde R5 (o arquivo mantém o nome `…Modal.tsx`)                                                                         |
> | `BlendConfirmationSheet` — "cabeçalho verde", rodapé "Voltar" + "Continuar" | **side-sheet** `.fv-panel-sheet` com seta ← na borda; rodapé = total + **"Criar liga"**. O "Voltar" textual morreu no molde R5                |
> | Leque do "+" com 3 opções                                                   | **2** (Lote e Liga) — a Etiqueta de Aprovação saiu na AP29. E o leque **só existe no mobile**; no desktop quem cria liga é o CTA do cabeçalho |
> | Contador "N selecionadas" no header                                         | o `SelectionModeHeader` é `[X] · título · spacer`; o contador foi pra linha de meta abaixo, e é ele que abre o `SelectedSamplesDropdown`      |
>
> **Três regras do doc estão erradas, não só desatualizadas** (as correções estão marcadas no lugar onde cada uma vive):
>
> 1. **Lote de origem read-only** (revisão de 2026-07-15, item 2) — **revogada**: hoje a origem da liga é derivada, **editável e pinável**, e o guard `BLEND_ORIGIN_LOT_READ_ONLY` foi removido. Ver o log de 2026-07-20.
> 2. **"Operador pode editar a safra depois via detalhe da liga"** (F3.2 revogada) — **falso**: a safra é read-only, com guard 422 `BLEND_HARVEST_READ_ONLY`.
> 3. **A tabela "Resumo do formulário" e o "Fluxo de telas resumido"** (logo abaixo das decisões F) descreviam o modal F3 — foram reescritos no estado atual.
>
> **Âncoras `arquivo:linha` deste doc não são confiáveis** — os números deslocaram. Trate-as como **âncoras por símbolo** e faça `grep` pelo nome antes de editar (mesma regra que o `Revisao-Pagina-Lotes-Plano-de-Trabalho.md` já carrega).
...
| Dono (`ownerClientId` + `declaredOwner`)                                    | **Painel de confirmação** — `ClientLookupField`               | **Depende**  | Dono unânime das origens, quando há                                    | Escolher é **obrigatório** quando as origens divergem; grava `blendOwnerPinned`. "Carteira da corretora" é escolha explícita (rev. 2026-07-15).             |
| Safra (`declaredHarvest`)                                                   | **Não aparece** — derivada                                    | —            | `deriveBlendHarvest` das origens                                       | **Read-only depois**: guard 422 `BLEND_HARVEST_READ_ONLY`. 2+ safras = badge `[Mix]` (rev. "safra Mix").                                                    |
| Local (`declaredLocation`)                                                  | Não aparece (revogado em 2026-05-19)                          | —            | `null`                                                                 | Editável depois, pelo detalhe.                                                                                                                              |
| Observações (`notes`)                                                       | Não aparece (revogado em 2026-05-19)                          | —            | `null`                                                                 | Editável depois, pelo detalhe.                                                                                                                              |
| Lote de origem (`declaredOriginLot`)                                        | **Não aparece** — derivado                                    | —            | `deriveBlendOriginLot` das origens                                     | ~~F3.5: `null` por design~~ **revogada em 2026-07-20**: deriva da somatória, e editar depois **fixa** (`blendOriginLotPinned`). Guard removido.             |
...
**Fluxo de telas resumido** (reescrito em 2026-07-22 — o anterior descrevia o modal F3, removido em 2026-05-19, e o modal de sucesso, deletado no redesenho):

_Desktop_ — 1. `/samples` → **"Criar liga"** no cabeçalho, ao lado do "+ Novo lote". 2. Modo seleção: a `.fv-bulkbar` aparece abaixo da toolbar (contador · Revisar ▾ · Criar liga ≥2 · ✕) e a **caixa de seleção ocupa a célula do ⋯**; a linha inteira alterna, inelegível fica apagada e o clique explica o motivo. 3. "Revisar ▾" abre o `SelectedSamplesDropdown`. 4. "Criar liga" abre o **`BlendConfirmationSheet`** — painel lateral com a composição (contribuição por origem, `×` por linha, total corrente) **e o campo de dono**. 5. "Criar liga" no rodapé → backend cria → **check no próprio painel** → o **drawer da liga abre sozinho**.

_Mobile_ — mesma sequência, com o leque do FAB "+" no lugar do CTA (2 opções: Lote e Liga) e o `SelectionModeHeader` no topo; o contador "N selecionadas" fica na linha de meta, abaixo do header, e é ele que abre o dropdown de revisão.
...
- **Modo seleção** (F1.1 reescrita + F1.D):
  ...
  - **Itens inelegíveis** (`eligibility.eligible === false` — F1.B): bolinha cinza opaca + card acinzentado + tooltip mapeado de `eligibility.reason`. Mapeamento `reason → texto` em constante (`lib/samples/eligibility-labels.ts`).
...
- `lib/api-client.ts:createBlend` — type simplificado: agora aceita só `clientDraftId`, `components`, `ownerClientId?`, `ownerUnitId?`, `idempotencyKey?`. `harvest`/`location`/`notes` removidos da API client (não enviados pelo frontend). _(Assinatura vigente, depois das revisões de julho: `ownerUnitId` **saiu** e entraram `ownerFixed?`, `lotNumber?`, `lotNumberManual?` e `receivedDate?` — a liga ficou editável como o lote normal.)_
