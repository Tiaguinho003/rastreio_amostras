...
- **Como gerar:** pelo botão **"Gerar espelho" no Detalhes** do contrato — e só por ali (**RC-D42** revogou a D76: o leque "+" da lista deixou de oferecê-lo, e com ele saiu o modo de seleção da página). Abre a **Conferência (D134)**, revisão _read-only_ dos campos (não grava nada; o "Preço" vem de `effectiveUnitPrice` da view, mesma fonte do PDF; a data é formatada em `America/Sao_Paulo`, o fuso que o PDF imprime) antes de gerar a prévia. Quando o contrato não é elegível o botão **não aparece**; quando o **lado escolhido** não sai, a Conferência escreve o motivo e desabilita "Gerar espelho". Ela também **avisa** (não bloqueia) quando o dono do lote mudou desde a emissão — o papel imprime o vendedor congelado, que é quem vendeu (RC-D110).
...
- **Onde ver depois (RC-D106):** o Detalhes do contrato tem a seção **"Espelhos de corretagem"** — a prateleira dos entregues que ainda estão guardados, cada linha com lado · data · comissão, clicável, reabrindo o **mesmo** modal em modo guardado (`?logId=`). Reabrir e baixar **não** cria linha nova: é o mesmo documento. Sem endpoint novo — a prateleira sai da **timeline**, cujos itens `ESPELHO` carregam `logId`/`available`/`commission`/`superseded`/`stale`. A prateleira lista **só os disponíveis**; a timeline mostra **todos**.
