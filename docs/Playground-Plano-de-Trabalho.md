...
> **Status**: PROTÓTIPO IMPLEMENTADO (2026-07-13) — decisões PG1–PG37 fechadas; casca + canvas + mocks + motor stub em código (ver nota no §10); pendentes deliberados: Q-F11, Q-T2/desenho do algoritmo, DEP-1; 🖥️ validação visual pendente
...
- **Adicionar nodes (PG26)**: paleta lateral colapsável com os 5 nodes (arrasta pro canvas ou clica) **e** arrastar de uma porta de saída para o vazio abre o menu de nodes compatíveis no ponto, já conectando (`onConnectEnd` do React Flow) — o gesto-assinatura do n8n.
...
### 4.2 Catálogo de nodes (confirmado — PG15)
...
| **Especificação-alvo** | Descreve o resultado desejado (fluxo inverso)                | —        | 1      | Formulário de alvos com tolerância (PG19+PG20): peneiras por chave, catação/defeitos, sacas mínimas                                                                                                                                                                        |
| **Combinações**        | Saída do fluxo inverso: lista de combinações reais possíveis | 1        | —      | Nº máx. de lotes por combinação: default 3, teto 4 (PG21)                                                                                                                                                                                                                  |
...
- **Identidade dos nodes (PG34)**: **cor de acento por tipo** (borda esquerda + ícone): Lote = verde da marca, Mistura = âmbar, Resultado = azul, Especificação-alvo = roxo, Combinações = azul-escuro. O fluxo se lê de longe pelo colorido.
...
- ~~Q-F5~~ — resolvida → **PG19** (peneiras + catação/defeitos + sacas mínimas; safra fora).
- ~~Q-F6~~ — resolvida → **PG20** (alvo pontual + tolerância, score de aderência).
- ~~Q-F7~~ — resolvida → **PG21** (configurável, default 3, teto 4).
- ~~Q-F8~~ — resolvida → **PG22** (menos lotes primeiro, desempate por aderência).
...
- **Q-T2 (reformulada em 2026-07-13)** — Fluxo inverso server-side: direção do usuário = **busca por semelhança** (rápida e eficiente) em vez de enumeração combinatória com caps. Desenhar o algoritmo (pré-rank por proximidade ao alvo, composição das combinações, poda, caps, top-N) na **análise pré-implementação da F4**.
- ~~Q-P1~~ — resolvida → **PG24** (localStorage, 1 rascunho único).

## 10. Fases de implementação **[CONGELADAS em 2026-07-13]**

> Duas pendências deliberadas não bloqueiam o congelamento: **Q-F11** (fundos ficam fora do motor — composição por componente — até a análise própria, que pode mudar a classificação) e **Q-T2** (desenho do algoritmo de busca por semelhança é a análise pré-implementação da própria F4). A **DEP-1** (aspecto/padrão com valores fechados) só condiciona a entrada desses 2 campos no motor, não as fases.
...
| F4   | **Fluxo inverso**: análise pré-implementação do algoritmo de busca por semelhança (Q-T2) + node Especificação-alvo + Combinações + endpoint read-only                                    | Combinações coerentes em dataset real; busca rápida (limites definidos na análise)  |
