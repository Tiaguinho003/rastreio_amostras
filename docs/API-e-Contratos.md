...
8. `GET /api/v1/dashboard/recent-sends`
   Card "Últimos envios" (desktop, DSH-D5): últimos 40 envios do mais recente pro mais antigo, `kind ∈ {PHYSICAL_SAMPLE, REPORT, APPROVAL}`. **Envios de amostra** (`PHYSICAL_SAMPLE_SENT` + `REPORT_EXPORTED`, sobre `sample_event`): lote/`isBlend`, destinatário ATUAL (última `SEND_UPDATED` vence; laudo cai pro snapshot ou `destination`) e flag `cancelled` (pareamento por `payload.sendEventId`); exclui amostras `INVALIDATED`. **Envios de aprovação (AP16)**: 1 item por linha do `approval_label_log` ligada a contrato (`sale_contract_id` não nulo; avulsas históricas excluídas), com `contractNumber` + `buyer` (join manual do contrato) e `id` namespaced `approval:<id>`. As duas fontes são buscadas (top-40 cada) e **mescladas no handler** por `at` desc, cortando em 40. `Cache-Control: private, max-age=30, must-revalidate`.
