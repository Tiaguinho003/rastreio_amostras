...
8. `GET /api/v1/dashboard/recent-sends`
   Alimenta os dois cards de envio do dashboard desktop (DSH-D5; divididos em DSB-D5). Devolve `{ sampleItems, approvalItems }` — **duas listas independentes**, cada uma top-40 do mais recente pro mais antigo (sem merge/corte global). **`sampleItems`** = card "Amostras enviadas", `kind ∈ {PHYSICAL_SAMPLE, REPORT}` (`PHYSICAL_SAMPLE_SENT` + `REPORT_EXPORTED`, sobre `sample_event`): lote/`isBlend`, destinatário ATUAL (última `SEND_UPDATED` vence; laudo cai pro snapshot ou `destination`) e flag `cancelled` (pareamento por `payload.sendEventId`); exclui amostras `INVALIDATED`. **`approvalItems`** = card "Aprovações enviadas" (AP16), `kind = APPROVAL`: 1 item por linha do `approval_label_log` ligada a contrato (`sale_contract_id` não nulo; avulsas históricas excluídas), com `contractNumber` + `buyer` (join manual do contrato) e `id` namespaced `approval:<id>`; degrada para `[]` se o contract service não estiver configurado. `Cache-Control: private, max-age=30, must-revalidate`.
