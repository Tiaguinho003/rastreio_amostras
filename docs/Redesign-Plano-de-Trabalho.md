...
Commits: `53bb96d` (style: prettier no doc, escapou da rodada 3) · `393a5c4` (bloqueio + seta) · `3f7cc8c` (cards + botões + linha reta) · este (docs). Gates verdes por commit (typecheck, lint, format, unit 535, contracts 20; sem mudança de backend).

Checklist de conferência (desktop): abrir o detalhe → página escurece e **nada atrás é clicável** (tap fora fecha; com edição aberta, não fecha); criar cliente idem; seta ← meio pra fora fecha os dois painéis; cards de filial E de conta brancos hairline; "+ Adicionar filial/conta/anexo" verdes preenchidos; linha da aba ativa reta; no mobile a seta aparece à esquerda do título.
