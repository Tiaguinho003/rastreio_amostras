...
### Etapa 1 — Venda (passo 1 do wizard, página Contratos)

> **Conferida + implementada ✅ (Sessão 53).** Todos os campos da fase de registro da venda (modal `SampleMovementModal`) revisados campo a campo e refinados — ver as notas abaixo da tabela.
>
> **Wizard (D61/D62):** esta fase virou o **passo 1** do wizard da página Contratos (não nasce mais "no lote"). O passo 1 **só coleta** (não cria nada); mostra **Tipo · Lote · Documento (preview do nº)** no topo, **sem** o campo Observações; o "Cancelar" do passo 2 vira **"Voltar"** e reabre este modal preservando os dados. A criação (venda + contrato + emitir) acontece só no fim do passo 2.
...
> **Próxima fase a implementar** (ainda **sem backend nem frontend** — só conferência de campos S38). Deve **espelhar o wizard de 2 passos do à vista** (D62), mas **sem lote** e persistindo **100% no `SaleContract`** (`sampleId`/`movementId` nulos + `type=FUTURO`, D51) — não há `SampleMovement`, então a edição/criação não sincroniza lote nem usa `SALE_UPDATED`.

Mesmos campos da **Etapa 1 + Etapa 2** do Mercado à vista (wizard de 2 passos, D62), **exceto**:
...
> **Fases 0/A/B/C/D ✅ implementadas para o Mercado à vista** (ver "Status da implementação" acima). **Próximo: contrato Futuro** (sem lote, D42/D51/D62) — espelha o wizard à vista persistindo 100% no `SaleContract`.
