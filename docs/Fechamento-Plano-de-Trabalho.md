...
| D82 | **Escopo por papel**                                     | **ADMIN** vê **todos** os elegíveis, com a **quebra por corretor** (nome + cota). **COMMERCIAL** vê **só os fechamentos em que é corretor** (`Broker.userId` = usuário) e **só a própria cota** (não vê o total, os outros corretores nem o valor do negócio). Sem `Broker` vinculado → página vazia.                                                                                                                                                                                                                                                                                                                                                                                                      |
| D83 | **Estrutura: lista por fechamento, sem período**         | Só a **lista de fechamentos** (ADMIN detalha por corretor; COMMERCIAL só a cota). **Sem agregado por corretor** e **sem filtro de período** (todos os recebíveis). **Total geral** no topo + **busca**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
- **É**: um PDF on-demand, gerado a partir de **1** contrato **CONFIRMADO/FATURADO/PAGO**, endereçado a
  **uma** das partes (comprador **ou** vendedor), mostrando a comissão **daquele lado** (D70–D73).
...
`src/sale-contracts/sale-contract-support.js`) e **congelam no `CONFIRMADO`** — por isso só
`CONFIRMADO`+ são elegíveis (D73).
...
> **Desenho da Sessão 62 (2026-06-29).** Implementação = **Fase F** (a fazer). Decisões **D77–D83**.
> **Relatório derivado, sem schema novo.**
...
- **COMMERCIAL** — vê **só os fechamentos em que é corretor** (resolve `Broker.userId` = o usuário logado;
  lista os contratos em que esse broker está no `SaleContractBroker`) e **só a própria cota** (não vê a
  corretagem total, a cota dos outros corretores, nem o valor do negócio — preço/sacas/total). Total = soma
  das próprias cotas. Sem `Broker` vinculado → página vazia.

### Elegíveis (D80) e natureza (D81)

- **Elegíveis**: status ∈ **{CONFIRMADO, FATURADO, PAGO}** (congelados; corretagem já calculada); exclui
  `EM_ABERTO`/`CONFERIR`/`WASH_OUT`. Contratos **sem corretagem** (total 0) não entram. _(Mesma régua do
  Espelho, D73.)_
...
- **Lista por fechamento** (cards/linhas): nº do contrato + valor a receber; **ADMIN** detalha cada
  corretor (nome + cota), **COMMERCIAL** mostra só a sua cota.
- **Total geral** no topo (soma do escopo) + **busca** (nº/partes; corretor no ADMIN).
...
- **Fase F — Página Financeiro (a implementar).** Relatório **derivado, sem migration** (D77–D83).
  **Backend**: método no domínio de contratos (ex.: `SaleContractService.listBrokerReceivables`) que filtra
  elegíveis (status ∈ {CONFIRMADO,FATURADO,PAGO} e corretagem > 0), carrega os `SaleContractBroker` e
  calcula `valorAReceber = sellerBrokerageValue + buyerBrokerageValue` + `cota = valorAReceber / N` (divisão
  igual); para **COMMERCIAL** resolve `Broker.userId` e projeta **só a cota** (sem expor total/outros). Gate
  **ADMIN + COMMERCIAL**. **Rota** `GET /api/v1/financeiro` (role-adaptive). **Frontend**: página
  `/financeiro` (`useRequireAuth({ allowedRoles: ['ADMIN','COMMERCIAL'] })`) + nav (`AppShell` +
  `HeaderAvatarMenu`), lista **role-adaptive** (ADMIN com quebra por corretor; COMMERCIAL só a cota) + total
  geral + busca, CSS `fin-*`. **Sem schema novo.**
