...
- **Fluxo (D61, S55)**: a venda à vista nasce **só pela página "Contratos"** (FAB "+" → "Mercado à
  vista" → **seleção de lote** → modal `components/samples/SampleMovementModal.tsx`) → `POST
app/api/v1/samples/[sampleId]/movements/route.ts` → `createSampleMovement` em
  `src/samples/sample-command-service.js` → projetor em `src/samples/sample-query-service.js`. O
  detalhe da amostra (`SampleMovementsPanel`) virou **só-leitura** — sem botões de venda/perda.
...
> **Conferida + implementada ✅ (Sessão 53, 2026-06-28).** Todos os campos da fase de registro da venda (modal `SampleMovementModal`) revisados campo a campo e refinados — ver as notas abaixo da tabela.
...
> **Status da implementação (Sessões 40–52)** — commits em `main`, **não pushados**:
>
