...
| D34 | **Corretores: N por contrato**             | `SaleContractBroker` (saleContractId + `userId` FK **anulável** [métrica] + `nome` snapshot). Corretor que é usuário → entra nas métricas por usuário; não-usuário → só nome. _(Revisa D13. Rateio da corretagem entre corretores: adiado.)_ |
...
- ⚠️ **Corretor(es)** — N por contrato via `SaleContractBroker` (`userId` opcional p/ métrica + nome snapshot); figuram no contrato (D13/D34).
...
- **Fase A — Emissor + corretor.** Promover `COMPANY_INFO` a módulo compartilhado + **CNPJ** (D29).
  Reaproveitar o logo + **imagem da assinatura do dono** (D35). Corretores via `SaleContractBroker` (D34).
- **Fase B — Contrato no modal de Venda (campo a campo).** Criar a tabela `SaleContract` (D11) via
  **migration nova**; numeração `NNNN/AA` (D15) e status (D14); partes/banco/armazém com **snapshots**
  (D12/D24/D25/D26) + corretores via `SaleContractBroker` (D34); financeiros (D17/18/19);
