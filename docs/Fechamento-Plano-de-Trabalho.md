...
> **Status da implementação (Sessão 40)** — commits em `main`, **não pushados**:
>
...
> - ⏳ **Falta na Fase 0:** backend `ClientBankAccount` (nested em `/clients/[id]/bank-accounts`); backend `ClientAttachment` + **upload de PDF** (liberar `application/pdf` no allowlist com validação de magic bytes `%PDF` + rotas upload/download/delete); **frontend** (página "Cadastros" com abas — D60 + contas/anexos no detalhe do cliente + nav + `api-client`).
