...
**✅ Validado no device pelo Flavio em 2026-07-20** — retry no erro técnico, modo manual após queda de rede no detect, close-up detectada e evento de extração no histórico da amostra após confirmar: todos conferidos e funcionando.
...
**Ajuste pós-preview (header em linha)**: depois de ver o layout em coluna, o usuário preferiu a seta de voltar **fixa na borda esquerda** com o **título centralizado** no modal, na mesma linha (padrão header mobile, espelha o `.bottom-sheet-header`). Header → `position: relative` + flex centralizado; `.type-modal-header .type-modal-back` → `position: absolute` (`left` no padding do header, `translateY(-50%)`, `:active` compondo o translate). Não afeta o `ClassificationClassifierModal`. Gates: lint · format:check · typecheck · build.
