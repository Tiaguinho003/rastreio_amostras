...
2. **Caminho 2 — Câmera direto via tabbar (existente, sem contexto)**. Item "Câmera" da navbar mobile (`components/AppShell.tsx:60`) → `/camera` (sem `sampleId`, Flow A). Câmera abre com QR scanner ativo (`qr-scanner` @ 12 scans/s). Ao detectar QR, `resolveSampleByQr` (`lib/api-client.ts` → `GET /samples/resolve?qr=...`) retorna o sample, e abre `SampleLookupResultModal` (`app/camera/page.tsx:1241`) com **2 botões: "Ver detalhes" (→ `/samples/[id]`) e "Escanear novamente"**. Pra chegar na classificação, operador hoje precisa tapar "Ver detalhes" → "Classificar" no detalhe (atravessa Caminho 1 → 4 toques totais desde a navbar).

3. **Caminho 3 — Modal "Aguardando classificação" do dashboard (NOVO botão a adicionar)**. Modal `components/dashboard/OperationModal.tsx`, disparado pelo card "Classificações aguardando" em `components/dashboard/DashboardMobile.tsx:86-94`. Hoje cada card no modal é um `<Link href="/samples/[id]">` simples (`OperationModal.tsx:67-85`) — leva sempre ao detalhe da amostra. **A adicionar**: botão dedicado "Classificar" em cada card, que vai direto pra `/camera?sampleId=X` (Flow B), pulando o detalhe. Tap na "área comum" do card mantém o comportamento atual (vai pra detalhe).
...
- **Não encontrou amostra** (caminho legado Flow A, sem `sampleId` na URL) → `ClassificationNotFoundModal` → "Cadastrar nova" / "Sair".
...
| **2**   | Tabbar "Câmera" → QR scan da etiqueta         | ✅ Existente   | Abre `SampleLookupResultModal` com "Ver detalhes" / "Escanear novamente"; pra classificar, atravessa Caminho 1 |
| **3**   | Modal "Aguardando classificação" do dashboard | ⚠️ A adicionar | Hoje só tap no card vai pro detalhe; botão "Classificar" será adicionado e irá direto pra `/camera?sampleId=X` |
...
### Caminho 2 — Câmera direto via tabbar (existente, com QR scan)
...
**3 caminhos**: Caminho 1 (detalhe, existente) · Caminho 2 (câmera via tabbar + QR, existente) · Caminho 3 (modal de pendências, a adicionar).
...
**Bloco F1 fechado em 2026-05-25** — 16 cravadas, 0 pendentes. ✅
...
_(Vazia. Aparecerá quando uma decisão antiga conflitar com uma nova — formato `T0.A`, `T0.B`, etc., seguindo padrão do doc Liga.)_
